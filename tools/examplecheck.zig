...
const examples = [_]Example{
    .{
        ...
        .source_tokens = &.{ "bytesAlloc", "valueBatch u16", "chanceBatch p=.25", "ratioBatch 3/8", "sampleBatch dice", "index choice", "u32 index choice", "const pointer choice" },
    },
    .{ .path = "examples/reproducible_streams.zig", .step = "zig build run-reproducible-streams" },
    .{ .path = "examples/range_sampling.zig", .step = "zig build run-range-sampling", .source_tokens = &.{ "rangeBatch u16", "rangeBatchChecked f64", "openBatch f32", "openClosedBatch f32" } },
