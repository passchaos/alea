...
const examples = [_]Example{
    ...
    .{ .path = "examples/range_sampling.zig", .step = "zig build run-range-sampling", .source_tokens = &.{ "randomRange die", "randomRangeAtMost die", "sampleSingle die", "sampleSingleInclusive die", "rangeBatch u16", "rangeAtMostBatchChecked i16", "rangeBatchChecked f64", "durationRangeAtMostBatch", "vectorRangeBatch f32x4", "vectorRangeAtMostBatch i32x4", "vectorOpenBatch f32x4", "vectorOpenClosedBatch f32x4", "openBatch f32", "openClosedBatch f32", "Uniform(f64).new", "Uniform(u8).newInclusive", "mapped Uniform even die", "VectorUniform(f32x4).new", "VectorUniform(i32x4).newInclusive" } },
