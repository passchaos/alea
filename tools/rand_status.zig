...
test "status output keeps key local rand comparison tokens" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1228 follow-ups closed for current bar") != null);
...
test "json output keeps stable machine-readable status keys" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"current_conclusion\": \"S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1228 follow-ups closed for current bar\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"no_known_unblocked_gap\": true") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"remaining_blocker\": \"S4-M1229") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"latest_validate_local_evidence\": \"compare/results/s4-m1228-distribution-transform-lane-store-sweep.md\"") != null);
}
