...
fn runSelfTest(stdout: *std.Io.Writer) !void {
    ...
    if (!hasAll(text, &.{
        ...
        "~/Work/rand",
        "S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1173 follow-ups closed for current bar",
...
fn printStatus(stdout: *std.Io.Writer) !void {
    try stdout.print(
        ...
        \\- Current conclusion: S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1173 follow-ups closed for current bar
        \\- Local Rust gap: no known unblocked local Rust core RNG gap
        \\- Next bar: S4-M1174 post-S4-M1173 exact/default dense SIMD, broader runtime, or new local Rust gap
        \\- Details: compare/results/s4-m420-current-rand-status.md
        \\
    , .{});
}
...
test "status output keeps key local rand comparison tokens" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1173 follow-ups closed for current bar") != null);
...
test "json output keeps stable machine-readable status keys" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"current_conclusion\": \"S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1173 follow-ups closed for current bar\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"no_known_unblocked_gap\": true") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"remaining_blocker\": \"S4-M1174") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"latest_validate_local_evidence\": \"compare/results/s4-m1173-post-s4-m1172-validate-all.md\"") != null);
}
