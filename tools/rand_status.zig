...
fn runSelfTest(stdout: *std.Io.Writer) !void {
    ...
    if (!hasAll(text, &.{
        ...
        "~/Work/rand",
        "S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1158 follow-ups closed for current bar",
...
test "status output keeps key local rand comparison tokens" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1158 follow-ups closed for current bar") != null);
...
test "json output keeps stable machine-readable status keys" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"current_conclusion\": \"S4-M11 runtime branch plus S4-M1124/S4-M1127-S4-M1158 follow-ups closed for current bar\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"no_known_unblocked_gap\": true") != null);
    try std.testing.expect(std.mem.indexOf(u8, out, "\"remaining_blocker\": \"S4-M1159") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, out, "\"latest_validate_local_evidence\": \"compare/results/s4-m1158-skew-normal-location.md\"") != null);
}
