...
evidence because it adds `rand-bench-test`, `rand-bench-smoke`, `rand-bench-smoke-self-test`, `rand-status`, `surfacecheck`, and `runtimecheck`; the smoke wrapper supports `ALEA_RAND_BENCH_MANIFEST` / `ALEA_RAND_BENCH_EXPECTED_ROW` overrides for custom local Rust comparison checks.
