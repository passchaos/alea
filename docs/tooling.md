...
| `zig build toolingcheck` | Verify this tooling catalog covers every project-defined build step/checked-in tool file, that doccheck, validate, and validate-all keep their aggregate dependencies, and that surfacecheck/runtimecheck run helper tests before their executable checks. |
