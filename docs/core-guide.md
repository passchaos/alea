...
`Charset.allocFrom`, `Charset.numChoices` / `len`, `Charset.get` / `byteAt`, `Charset.probability` / `probabilityAt`, `Charset.probabilityIter` size hints, `charFrom`, `stringFrom`, `unicodeScalarFrom`,
...
ASCII charsets, custom `Charset` count, item, probability, probability-iterator, and size-hint diagnostics, allocation-returning strings,
