...
- `distributions.WeightedTree` for O(log n) dynamic weight update, push, pop,
  ...
  `initByIndex` / `updateAllByIndex` from index-weight accessors, and optional `weight` / `probability`, lazy `weightIter` / `probabilityIter`, checked `weightAt` / `probabilityAt` lookup, and bulk
...
- `distributions.WeightedIntTree` for unsigned integer weights when dynamic
  ...
  `updateAllByIndex` from index-weight accessors; optional `weight` / `probability`, lazy `weightIter` / `probabilityIter`, and checked `weightAt` / `probabilityAt` lookup mirrors generic trees, and weights wider than `u64` are
