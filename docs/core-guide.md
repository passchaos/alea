...
- `seq.Choice`, `seq.chooseIterFrom`, `seq.chooseIterCheckedFrom`,
  ...
  optional `WeightedChoice.weight` / `probability`, lazy `WeightedChoice.weightIter`, `WeightedChoice.weightAt` / `weightsInto`, `WeightedChoice.sampleIndexFrom` /
...
- `distributions.AliasTable` for O(1) repeated weighted index sampling,
  ...
  optional `weight` / `probability`, lazy `weightIter`, `weightAt` / `weights` reconstruction for diagnostics and parity with Rust weighted
