...
| `zig build run-string-generation` | `examples/string_generation.zig` | Predefined ASCII charsets, custom `Charset` probability and probability-iterator diagnostics, allocation-returning strings, Unicode scalar fill/owned/range batches, and caller-owned UTF-8 buffers. |
