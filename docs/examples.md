...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot weighted indexes and values, static alias tables, dynamic weighted trees, weighted choices, weighted no-replacement, fixed-size weighted item arrays, and fixed-size weighted index arrays. |
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, fixed-size item arrays, item subsets, partial shuffle, allocated/caller-owned reservoir sampling, reusable choices, and allocation-returning/caller-owned streaming iterator helpers, and fixed-size weighted iterator arrays. |
