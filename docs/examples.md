...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot f64/generic weighted indexes, values, and const pointers, static alias tables, dynamic weighted trees, weighted choices with value/index samples, fills, and owned usize/u32 index batches, allocation-returning weighted no-replacement item/usize-index/u32-index/pointer outputs, compact weighted IndexVec samples, fixed-size weighted item/usize-index/u32-index/pointer arrays, and caller-owned weighted usize/u32 index/value/pointer buffers. |
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, compact fixed-size u32 index arrays, IndexVec lazy/caller-owned/allocation-returning value, pointer, mutable-pointer, and u32 export mapping, caller-owned usize/u32 index buffers, fixed-size value/pointer arrays, allocation-returning and caller-owned item/value/pointer subset buffers, item subsets, partial shuffle selected/rest splits, allocated/caller-owned value and pointer reservoir sampling, reusable choices with value/index samples, fills, and owned index batches, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
