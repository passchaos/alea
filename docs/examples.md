...
| --- | --- | --- |
| `zig build run-basic` | `examples/basic.zig` | A broad first tour: engines, `Rng`, owned byte/value/bounded-uint/sample/scalar-and-vector-probability/standard-and-parameterized normal/exponential batches, distributions, strings, index and const-pointer choice, sequence helpers, and child streams. |
...
| `zig build run-range-sampling` | `examples/range_sampling.zig` | Integer half-open/inclusive ranges, strict float endpoint semantics, duration ranges, owned duration/range/inclusive-range/vector-range and scalar/vector strict-interval batches, reusable `Uniform`, vector ranges, point masses, and checked errors. |
