...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot f64/generic weighted indexes, values, and const pointers, static alias tables, dynamic weighted trees, weighted choices, allocation-returning weighted no-replacement item/index/pointer outputs, fixed-size weighted item/index/pointer arrays, and caller-owned weighted index/value/pointer buffers. |
