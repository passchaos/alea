...
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, caller-owned index buffers, fixed-size item arrays, caller-owned item subset buffers, item subsets, partial shuffle, allocated/caller-owned reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
