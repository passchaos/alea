...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot and repeated f64/generic weighted indexes, generic weighted values, f64 weighted values, and generic/f64 const plus f64 mutable pointers, static alias tables, dynamic weighted trees, weighted choices with value/pointer/index samples, fills, and owned value/pointer/usize/u32-index batches, allocation-returning weighted no-replacement item/usize-index/u32-index/pointer outputs, compact weighted IndexVec samples, fixed-size weighted item/usize-index/u32-index/pointer arrays, and caller-owned weighted usize/u32 index/value/const-pointer/pointer buffers. |
