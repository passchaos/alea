...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot weighted indexes, values, and const pointers, static alias tables, dynamic weighted trees, weighted choices, allocation-returning weighted no-replacement item/index/pointer outputs, fixed-size weighted item/index/pointer arrays, and caller-owned weighted index/value/pointer buffers. |
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, IndexVec lazy/caller-owned value and pointer mapping including mutable pointers, caller-owned usize/u32 index buffers, fixed-size value/pointer arrays, allocation-returning and caller-owned item/value/pointer subset buffers, item subsets, partial shuffle selected/rest splits, allocated value/pointer and caller-owned reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
