...
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, compact fixed-size u32 index arrays, IndexVec lazy/caller-owned/allocation-returning value, pointer, mutable-pointer, and u32 export mapping, caller-owned usize/u32 index buffers, fixed-size value/pointer arrays, allocation-returning and caller-owned item/value/pointer subset buffers, item subsets, partial shuffle selected/rest splits, allocated/caller-owned value and pointer reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
