...
| --- | --- | --- |
| `zig build run-basic` | `examples/basic.zig` | A broad first tour: engines, `Rng`, distributions, strings, const-pointer choice, sequence helpers, and child streams. |
...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot weighted indexes, values, and const pointers, static alias tables, dynamic weighted trees, weighted choices, weighted no-replacement, fixed-size weighted item/index/pointer arrays, and caller-owned weighted index/value/pointer buffers. |
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, IndexVec lazy/caller-owned value and pointer mapping including mutable pointers, caller-owned usize/u32 index buffers, fixed-size value/pointer arrays, allocation-returning and caller-owned item/value/pointer subset buffers, item subsets, partial shuffle selected/rest splits, allocated/caller-owned reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
| `zig build run-caller-owned-sampling` | `examples/caller_owned_sampling.zig` | Caller-owned index/item/iterator buffers, weighted scratch buffers, and allocation-predictable sampling workflows. |
