...
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, fixed-size item arrays, item subsets, partial shuffle, allocated/caller-owned reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size weighted iterator arrays. |
