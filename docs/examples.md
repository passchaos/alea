...
| `zig build run-sequence-sampling` | `examples/sequence_sampling.zig` | Index sampling, IndexVec lazy/caller-owned value and pointer mapping including mutable pointers, caller-owned usize/u32 index buffers, fixed-size item arrays, caller-owned item/value/pointer subset buffers, item subsets, partial shuffle selected/rest splits, allocated/caller-owned reservoir sampling, reusable choices, and fixed-size/allocation-returning/caller-owned streaming iterator helpers, and fixed-size/caller-owned weighted iterator helpers. |
