...
| `zig build run-weighted-sampling` | `examples/weighted_sampling.zig` | One-shot f64/generic weighted indexes, values, and const pointers, static alias tables, dynamic weighted trees, weighted choices with value/index samples and fills, allocation-returning weighted no-replacement item/usize-index/u32-index/pointer outputs, compact weighted IndexVec samples, fixed-size weighted item/usize-index/u32-index/pointer arrays, and caller-owned weighted usize/u32 index/value/pointer buffers. |
