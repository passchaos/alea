...
pub fn main(init: std.process.Init) !void {
    ...
    var rng_weighted_index_array_engine = alea.ScalarPrng.init(0x715d);
    const rng_weighted_index_array = try alea.Rng.weightedIndexArrayCheckedFrom(&rng_weighted_index_array_engine, 6, &float_weights);
    try stdout.print("Rng weighted index array: {any}\n", .{rng_weighted_index_array});
    ...
    var rng_weighted_u32_index_array_engine = alea.ScalarPrng.init(0x715e);
    const rng_weighted_u32_index_array = try alea.Rng.weightedIndexU32ArrayCheckedFrom(&rng_weighted_u32_index_array_engine, 6, &float_weights);
    try stdout.print("Rng weighted u32 index array: {any}\n", .{rng_weighted_u32_index_array});
    ...
    var index_weight_repeated_index_array_engine = alea.ScalarPrng.init(0x71c5);
    const index_weight_repeated_index_array = try alea.seq.weightedIndexArrayByIndexCheckedFrom(&index_weight_repeated_index_array_engine, u32, 6, items.len, indexWeight);
    try stdout.print("weighted index-weight repeated index array: {any}\n", .{index_weight_repeated_index_array});

    var index_weight_repeated_u32_index_array_engine = alea.ScalarPrng.init(0x71c6);
    const index_weight_repeated_u32_index_array = try alea.seq.weightedIndexU32ArrayByIndexCheckedFrom(&index_weight_repeated_u32_index_array_engine, u32, 6, items.len, indexWeight);
    try stdout.print("weighted index-weight repeated u32 index array: {any}\n", .{index_weight_repeated_u32_index_array});
    ...
    var index_weight_repeated_value_array_engine = alea.ScalarPrng.init(0x71c7);
    const index_weight_repeated_value_array = try alea.seq.chooseWeightedValueArrayByIndexCheckedFrom(&index_weight_repeated_value_array_engine, []const u8, u32, 6, &items, indexWeight);
    try stdout.print("weighted index-weight repeated value array: [{s}, {s}, {s}, {s}, {s}, {s}]\n", .{ index_weight_repeated_value_array[0], index_weight_repeated_value_array[1], index_weight_repeated_value_array[2], index_weight_repeated_value_array[3], index_weight_repeated_value_array[4], index_weight_repeated_value_array[5] });
    ...
    var index_weight_repeated_const_ptr_array_engine = alea.ScalarPrng.init(0x71c8);
    const index_weight_repeated_const_ptr_array = try alea.seq.chooseWeightedConstPtrArrayByIndexCheckedFrom(&index_weight_repeated_const_ptr_array_engine, []const u8, u32, 6, &items, indexWeight);
    try stdout.print("weighted index-weight repeated const ptr array: [{s}, {s}, {s}, {s}, {s}, {s}]\n", .{ index_weight_repeated_const_ptr_array[0].*, index_weight_repeated_const_ptr_array[1].*, index_weight_repeated_const_ptr_array[2].*, index_weight_repeated_const_ptr_array[3].*, index_weight_repeated_const_ptr_array[4].*, index_weight_repeated_const_ptr_array[5].* });
    ...
    var index_weight_repeated_mut_array_items = items;
    var index_weight_repeated_mut_array_engine = alea.ScalarPrng.init(0x71c9);
    const index_weight_repeated_mut_array = try alea.seq.chooseWeightedPtrArrayByIndexCheckedFrom(&index_weight_repeated_mut_array_engine, []const u8, u32, 6, &index_weight_repeated_mut_array_items, indexWeight);
    for (index_weight_repeated_mut_array) |slot| slot.* = "array";
    try stdout.print("weighted index-weight repeated mut ptr array items: [{s}, {s}, {s}, {s}]\n", .{ index_weight_repeated_mut_array_items[0], index_weight_repeated_mut_array_items[1], index_weight_repeated_mut_array_items[2], index_weight_repeated_mut_array_items[3] });
    ...
    var rng_weighted_value_array_engine = alea.ScalarPrng.init(0x715a);
    const rng_weighted_value_array = try alea.Rng.chooseWeightedValueArrayCheckedFrom(&rng_weighted_value_array_engine, []const u8, 6, &items, &float_weights);
    try stdout.print("Rng weighted value array: [{s}, {s}, {s}, {s}, {s}, {s}]\n", .{ rng_weighted_value_array[0], rng_weighted_value_array[1], rng_weighted_value_array[2], rng_weighted_value_array[3], rng_weighted_value_array[4], rng_weighted_value_array[5] });
    ...
    var rng_weighted_const_ptr_array_engine = alea.ScalarPrng.init(0x715b);
    const rng_weighted_const_ptr_array = try alea.Rng.chooseWeightedConstPtrArrayCheckedFrom(&rng_weighted_const_ptr_array_engine, []const u8, 6, &items, &float_weights);
    try stdout.print("Rng weighted const ptr array: [{s}, {s}, {s}, {s}, {s}, {s}]\n", .{ rng_weighted_const_ptr_array[0].*, rng_weighted_const_ptr_array[1].*, rng_weighted_const_ptr_array[2].*, rng_weighted_const_ptr_array[3].*, rng_weighted_const_ptr_array[4].*, rng_weighted_const_ptr_array[5].* });
    ...
    var rng_weighted_mut_ptr_array_engine = alea.ScalarPrng.init(0x715c);
    var rng_weighted_mut_scores_array = [_]u8{ 10, 20, 30, 40 };
    const rng_weighted_mut_ptr_array = try alea.Rng.chooseWeightedPtrArrayCheckedFrom(&rng_weighted_mut_ptr_array_engine, u8, 6, &rng_weighted_mut_scores_array, &float_weights);
    for (rng_weighted_mut_ptr_array) |score| score.* += 1;
    try stdout.print("Rng weighted mut ptr array scores: {any}\n", .{rng_weighted_mut_scores_array});
    ...
    var weighted_namespace_alias = try alea.distr.weighted.WeightedIndex(f64).new(allocator, &float_weights);
    defer weighted_namespace_alias.deinit();
    const alias_new_num_choices = alias_new.numChoices();
    const weighted_index_alias_num_choices = weighted_index_alias.numChoices();
    const weighted_namespace_num_choices = weighted_namespace_alias.numChoices();
    ...
    const alias_missing_probability = alias.probability(items.len) == null;
    ...
    const alias_weight_iter_hint = alias_weight_iter.sizeHint();
    ...
    const alias_probability_iter_hint = alias_probability_iter.sizeHint();
    ...
    try stdout.print("weighted namespace numChoices: {}\n", .{weighted_namespace_num_choices});
    ...
    try stdout.print("alias weightIter sizeHint: {}..{}\n", .{ alias_weight_iter_hint.lower, alias_weight_iter_hint.upper.? });
    try stdout.print("alias probabilityIter fill: {any}\n", .{alias_probability_iter_fill});
    try stdout.print("alias probabilityIter sizeHint: {}..{}\n", .{ alias_probability_iter_hint.lower, alias_probability_iter_hint.upper.? });
    ...
    var tree_probability_iter = tree.probabilityIter();
    var tree_probability_iter_fill: [items.len + 1]f64 = undefined;
    _ = tree_probability_iter.fill(&tree_probability_iter_fill);
    const tree_probability_iter_hint = tree_probability_iter.sizeHint();
    ...
    try stdout.print("dynamic tree probabilityIter fill: {any}\n", .{tree_probability_iter_fill});
    try stdout.print("dynamic tree probabilityIter sizeHint: {}..{}\n", .{ tree_probability_iter_hint.lower, tree_probability_iter_hint.upper.? });
    ...
    try tree.updateAll(&.{ 0, 0, 7, 0, 0 });
    try stdout.print("dynamic tree single-positive constantIndex: {?}\n", .{tree.constantIndex()});
    ...
    var int_tree_probability_iter = int_tree.probabilityIter();
    var int_tree_probability_iter_fill: [items.len]f64 = undefined;
    _ = int_tree_probability_iter.fill(&int_tree_probability_iter_fill);
    const int_tree_probability_iter_hint = int_tree_probability_iter.sizeHint();
    ...
    try stdout.print("integer tree probabilityIter fill: {any}\n", .{int_tree_probability_iter_fill});
    try stdout.print("integer tree probabilityIter sizeHint: {}..{}\n", .{ int_tree_probability_iter_hint.lower, int_tree_probability_iter_hint.upper.? });
    ...
    try int_tree.updateAll(&.{ 0, 0, 7, 0 });
    try stdout.print("integer tree single-positive constantIndex: {?}\n", .{int_tree.constantIndex()});
    ...
    const choice_weight_iter_hint = choice_weight_iter.sizeHint();
    try stdout.print("WeightedChoice.weightIter fill: {any}\n", .{choice_weight_iter_fill});
    try stdout.print("WeightedChoice.weightIter sizeHint: {}..{}\n", .{ choice_weight_iter_hint.lower, choice_weight_iter_hint.upper.? });
    ...
    var choice_value_iter_engine = alea.ScalarPrng.init(0x71cc);
    var choice_value_iter = (try alea.seq.chooseWeightedIterFrom(allocator, &choice_value_iter_engine, []const u8, f64, &items, &float_weights)).?;
    defer choice_value_iter.deinit();
    var choice_value_iter_fill: [6]*const []const u8 = undefined;
    choice_value_iter.fill(&choice_value_iter_fill);
    try stdout.print("seq.chooseWeightedIterFrom fill: [{s}, {s}, {s}, {s}, {s}, {s}]\n", .{ choice_value_iter_fill[0].*, choice_value_iter_fill[1].*, choice_value_iter_fill[2].*, choice_value_iter_fill[3].*, choice_value_iter_fill[4].*, choice_value_iter_fill[5].* });
