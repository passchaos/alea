...
pub fn fromSeed(seed_value: anytype) Xoshiro256PlusPlus {
...
pub fn fromRng(source: anytype) Xoshiro256PlusPlus {
    var self: Xoshiro256PlusPlus = .{ .state = undefined };
    inline for (0..4) |i| self.state[i] = source_helpers.nextU64(source);
    if (self.isZeroState()) return init(0);
    return self;
}
...
pub fn tryFromRng(source: anytype) !Xoshiro256PlusPlus {
    var self: Xoshiro256PlusPlus = .{ .state = undefined };
    inline for (0..4) |i| self.state[i] = try source_helpers.tryNextU64(source);
    if (self.isZeroState()) return init(0);
    return self;
}
...
pub fn tryNext(self: *Xoshiro256PlusPlus) !u64 {
    return self.next();
}

pub fn tryNextU64(self: *Xoshiro256PlusPlus) !u64 {
    return self.tryNext();
}

pub fn tryNextU32(self: *Xoshiro256PlusPlus) !u32 {
...
pub fn tryFillBytes(self: *Xoshiro256PlusPlus, buf: []u8) !void {
    self.fillBytes(buf);
}
...
pub fn tryFork(self: *Xoshiro256PlusPlus) !Xoshiro256PlusPlus {
