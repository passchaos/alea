...
pub fn vectorLogLogistic(rng: Rng, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    return vectorLogLogisticFrom(rng, VectorType, scale, shape);
}

pub fn vectorLogLogisticFrom(source: anytype, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    const sampler = VectorLogLogistic(VectorType).init(scale, shape) catch unreachable;
    return sampler.sampleFrom(source);
}

pub fn vectorLogLogisticChecked(rng: Rng, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    return vectorLogLogisticCheckedFrom(rng, VectorType, scale, shape);
}

pub fn vectorLogLogisticCheckedFrom(source: anytype, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    const sampler = try VectorLogLogistic(VectorType).init(scale, shape);
    return sampler.sampleFrom(source);
}

pub fn fillVectorLogLogistic(rng: Rng, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    fillVectorLogLogisticFrom(rng, VectorType, dest, scale, shape);
}

pub fn fillVectorLogLogisticFrom(source: anytype, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    const sampler = VectorLogLogistic(VectorType).init(scale, shape) catch unreachable;
    sampler.fillFrom(source, dest);
}

pub fn fillVectorLogLogisticChecked(rng: Rng, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    return fillVectorLogLogisticCheckedFrom(rng, VectorType, dest, scale, shape);
}

pub fn fillVectorLogLogisticCheckedFrom(source: anytype, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    if (dest.len == 0) return;
    const sampler = try VectorLogLogistic(VectorType).init(scale, shape);
    sampler.fillFrom(source, dest);
}

fn logLogisticFromOpenUniformVector(comptime VectorType: type, uniform_vec: VectorType, scale: vectorChild(VectorType), inverse_shape: vectorChild(VectorType)) VectorType {
    const scale_vec: VectorType = @splat(scale);
    const inverse_shape_vec: VectorType = @splat(inverse_shape);
    const one_vec: VectorType = @splat(1);
    return scale_vec * @exp(@log(uniform_vec / (one_vec - uniform_vec)) * inverse_shape_vec);
}

fn logLogisticShapeOneFromOpenUniformVector(comptime VectorType: type, uniform_vec: VectorType, scale: vectorChild(VectorType)) VectorType {
    const scale_vec: VectorType = @splat(scale);
    const one_vec: VectorType = @splat(1);
    return scale_vec * uniform_vec / (one_vec - uniform_vec);
}

pub fn VectorLogLogistic(comptime VectorType: type) type {
    const Child = vectorChild(VectorType);
    requireFloat(Child);
    ...
    return struct {
        ...
        sampler: LogLogistic(Child),

        pub fn init(scale: Child, shape: Child) Error!Self {
            return .{ .sampler = try LogLogistic(Child).init(scale, shape) };
        }
...
pub fn vectorPareto(rng: Rng, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    return vectorParetoFrom(rng, VectorType, scale, shape);
}

pub fn vectorParetoFrom(source: anytype, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    const sampler = VectorPareto(VectorType).init(scale, shape) catch unreachable;
    return sampler.sampleFrom(source);
}

pub fn vectorParetoChecked(rng: Rng, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    return vectorParetoCheckedFrom(rng, VectorType, scale, shape);
}

pub fn vectorParetoCheckedFrom(source: anytype, comptime VectorType: type, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    const sampler = try VectorPareto(VectorType).init(scale, shape);
    return sampler.sampleFrom(source);
}

pub fn fillVectorPareto(rng: Rng, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    fillVectorParetoFrom(rng, VectorType, dest, scale, shape);
}

pub fn fillVectorParetoFrom(source: anytype, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    const sampler = VectorPareto(VectorType).init(scale, shape) catch unreachable;
    sampler.fillFrom(source, dest);
}

pub fn fillVectorParetoChecked(rng: Rng, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    return fillVectorParetoCheckedFrom(rng, VectorType, dest, scale, shape);
}

pub fn fillVectorParetoCheckedFrom(source: anytype, comptime VectorType: type, dest: []VectorType, scale: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    if (dest.len == 0) return;
    const sampler = try VectorPareto(VectorType).init(scale, shape);
    sampler.fillFrom(source, dest);
}

fn paretoFromOpenUniformVector(comptime VectorType: type, uniform_vec: VectorType, scale: vectorChild(VectorType), inverse_shape: vectorChild(VectorType)) VectorType {
    const scale_vec: VectorType = @splat(scale);
    const neg_inverse_shape_vec: VectorType = @splat(-inverse_shape);
    return scale_vec * @exp(@log(uniform_vec) * neg_inverse_shape_vec);
}

pub fn VectorPareto(comptime VectorType: type) type {
    const Child = vectorChild(VectorType);
    requireFloat(Child);
    ...
    return struct {
        ...
        sampler: Pareto(Child),

        pub fn init(scale: Child, shape: Child) Error!Self {
            return .{ .sampler = try Pareto(Child).init(scale, shape) };
        }
...
pub fn vectorInverseGaussian(rng: Rng, comptime VectorType: type, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    return vectorInverseGaussianFrom(rng, VectorType, mean, shape);
}

pub fn vectorInverseGaussianFrom(source: anytype, comptime VectorType: type, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    const sampler = VectorInverseGaussian(VectorType).init(mean, shape) catch unreachable;
    return sampler.sampleFrom(source);
}

pub fn vectorInverseGaussianChecked(rng: Rng, comptime VectorType: type, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    return vectorInverseGaussianCheckedFrom(rng, VectorType, mean, shape);
}

pub fn vectorInverseGaussianCheckedFrom(source: anytype, comptime VectorType: type, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!VectorType {
    const sampler = try VectorInverseGaussian(VectorType).init(mean, shape);
    return sampler.sampleFrom(source);
}

pub fn fillVectorInverseGaussian(rng: Rng, comptime VectorType: type, dest: []VectorType, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    fillVectorInverseGaussianFrom(rng, VectorType, dest, mean, shape);
}

pub fn fillVectorInverseGaussianFrom(source: anytype, comptime VectorType: type, dest: []VectorType, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) void {
    const sampler = VectorInverseGaussian(VectorType).init(mean, shape) catch unreachable;
    sampler.fillFrom(source, dest);
}

pub fn fillVectorInverseGaussianChecked(rng: Rng, comptime VectorType: type, dest: []VectorType, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    return fillVectorInverseGaussianCheckedFrom(rng, VectorType, dest, mean, shape);
}

pub fn fillVectorInverseGaussianCheckedFrom(source: anytype, comptime VectorType: type, dest: []VectorType, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) Error!void {
    if (dest.len == 0) return;
    const sampler = try VectorInverseGaussian(VectorType).init(mean, shape);
    sampler.fillFrom(source, dest);
}

fn inverseGaussianFromNormalVector(comptime VectorType: type, normal_vec: VectorType, uniform_vec: VectorType, mean: vectorChild(VectorType), shape: vectorChild(VectorType)) VectorType {
    const Child = vectorChild(VectorType);
    ...
    return @select(Child, uniform_vec <= threshold, x, mean_squared / x);
}

pub fn VectorInverseGaussian(comptime VectorType: type) type {
    const Child = vectorChild(VectorType);
    requireFloat(Child);
    ...
    return struct {
        ...
        sampler: InverseGaussian(Child),

        pub fn init(mean: Child, shape: Child) Error!Self {
            return .{ .sampler = try InverseGaussian(Child).init(mean, shape) };
        }
...
test "degenerate log-logistic helpers do not consume random stream" {
    ...
    var control = alea.ScalarPrng.init(0x5150_d186);
    const rng = Rng.init(&engine);

    try std.testing.expectEqual(@as(f64, 0), logLogisticFrom(&engine, f64, 0, 3));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(f64, 0), try logLogisticChecked(rng, f64, 0, 1));
    try std.testing.expectEqual(control.next(), engine.next());

    var scalar_buf: [5]f64 = undefined;
    fillLogLogisticFrom(&engine, f64, &scalar_buf, 0, 3);
    for (scalar_buf) |sample| try std.testing.expectEqual(@as(f64, 0), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    try fillLogLogisticChecked(rng, f64, &scalar_buf, 0, 1);
    for (scalar_buf) |sample| try std.testing.expectEqual(@as(f64, 0), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    const sampler = try LogLogistic(f64).init(0, 3);
    try std.testing.expectEqual(@as(f64, 0), sampler.scaleValue());
    ...
    const shape_one = try LogLogistic(f64).init(0, 1);
    ...
    try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), try vectorLogLogisticChecked(rng, @Vector(4, f64), 0, 1));
    try std.testing.expectEqual(control.next(), engine.next());

    var vector_buf: [3]@Vector(4, f64) = undefined;
    fillVectorLogLogisticFrom(&engine, @Vector(4, f64), &vector_buf, 0, 3);
    for (vector_buf) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    try fillVectorLogLogisticChecked(rng, @Vector(4, f64), &vector_buf, 0, 1);
    for (vector_buf) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    const vector_sampler = try VectorLogLogistic(@Vector(4, f64)).init(0, 3);
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.scaleValue());
    try std.testing.expectEqual(@as(f64, 3), vector_sampler.shapeValue());
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.expectedValue().?);
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.varianceValue().?);
...
test "degenerate rayleigh helpers do not consume random stream" {
    ...
    var control = alea.ScalarPrng.init(0x5150_d1c7);
    const rng = Rng.init(&engine);

    try std.testing.expectEqual(@as(f64, 0), rayleighFrom(&engine, f64, 0));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(f64, 0), try rayleighChecked(rng, f64, 0));
    try std.testing.expectEqual(control.next(), engine.next());

    var scalar_buf: [5]f64 = undefined;
    fillRayleighFrom(&engine, f64, &scalar_buf, 0);
    for (scalar_buf) |sample| try std.testing.expectEqual(@as(f64, 0), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    try fillRayleighChecked(rng, f64, &scalar_buf, 0);
    for (scalar_buf) |sample| try std.testing.expectEqual(@as(f64, 0), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    const sampler = try Rayleigh(f64).init(0);
    try std.testing.expectEqual(@as(f64, 0), sampler.scaleValue());
    try std.testing.expectEqual(@as(f64, 0), sampler.expectedValue());
    try std.testing.expectEqual(@as(f64, 0), sampler.varianceValue());
    try std.testing.expectEqual(@as(f64, 0), sampler.medianValue());
    ...
    try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), vectorRayleighFrom(&engine, @Vector(4, f64), 0));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), try vectorRayleighChecked(rng, @Vector(4, f64), 0));
    try std.testing.expectEqual(control.next(), engine.next());

    var vector_buf: [3]@Vector(4, f64) = undefined;
    fillVectorRayleighFrom(&engine, @Vector(4, f64), &vector_buf, 0);
    for (vector_buf) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    try fillVectorRayleighChecked(rng, @Vector(4, f64), &vector_buf, 0);
    for (vector_buf) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(0)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    const vector_sampler = try VectorRayleigh(@Vector(4, f64)).init(0);
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.scaleValue());
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.expectedValue());
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.varianceValue());
    try std.testing.expectEqual(@as(f64, 0), vector_sampler.medianValue());
...
test "distribution vector helpers preserve support and stream shape" {
    ...
    try std.testing.expectEqual(log_logistic_vec, direct_log_logistic_vec);
    inline for (0..4) |lane| try std.testing.expect(log_logistic_vec[lane] > 0);
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    const vector_log_logistic_sampler = try VectorLogLogistic(@Vector(4, f64)).init(2, 3);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_log_logistic_sampler.scaleValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 3), vector_log_logistic_sampler.shapeValue(), 1e-12);
    const vector_log_logistic_mean = @as(f64, 4) * std.math.pi / (3 * @sqrt(@as(f64, 3)));
    try std.testing.expectApproxEqAbs(vector_log_logistic_mean, vector_log_logistic_sampler.expectedValue().?, 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 16) * std.math.pi / (3 * @sqrt(@as(f64, 3))) - vector_log_logistic_mean * vector_log_logistic_mean, vector_log_logistic_sampler.varianceValue().?, 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_log_logistic_sampler.medianValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2) * std.math.pow(f64, 0.5, 1.0 / 3.0), vector_log_logistic_sampler.modeValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 0), vector_log_logistic_sampler.minValue(), 0);
    try std.testing.expect(vector_log_logistic_sampler.maxValue() == null);
    const sampled_log_logistic_vec = vector_log_logistic_sampler.sample(rng);
    const direct_sampled_log_logistic_vec = vector_log_logistic_sampler.sampleFrom(&direct_engine);
    try std.testing.expectEqual(sampled_log_logistic_vec, direct_sampled_log_logistic_vec);
    inline for (0..4) |lane| try std.testing.expect(sampled_log_logistic_vec[lane] > 0);
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    var log_logistic_buf_vec: [3]@Vector(4, f64) = undefined;
    var direct_log_logistic_buf_vec: [3]@Vector(4, f64) = undefined;
    try fillVectorLogLogisticChecked(rng, @Vector(4, f64), &log_logistic_buf_vec, 2, 3);
    try fillVectorLogLogisticCheckedFrom(&direct_engine, @Vector(4, f64), &direct_log_logistic_buf_vec, 2, 3);
    try std.testing.expectEqualSlices(@Vector(4, f64), &log_logistic_buf_vec, &direct_log_logistic_buf_vec);
    for (log_logistic_buf_vec) |vec| inline for (0..4) |lane| try std.testing.expect(vec[lane] > 0);
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());
    vector_log_logistic_sampler.fill(rng, &log_logistic_buf_vec);
    vector_log_logistic_sampler.fillFrom(&direct_engine, &direct_log_logistic_buf_vec);
    try std.testing.expectEqualSlices(@Vector(4, f64), &log_logistic_buf_vec, &direct_log_logistic_buf_vec);
    for (log_logistic_buf_vec) |vec| inline for (0..4) |lane| try std.testing.expect(vec[lane] > 0);
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    const vector_log_logistic_shape_one = try VectorLogLogistic(@Vector(4, f64)).init(2, 1);
    try std.testing.expect(vector_log_logistic_shape_one.expectedValue() == null);
    try std.testing.expect(vector_log_logistic_shape_one.varianceValue() == null);
    const log_logistic_shape_one_vec = vector_log_logistic_shape_one.sample(rng);
    const direct_log_logistic_shape_one_vec = vector_log_logistic_shape_one.sampleFrom(&direct_engine);
    try std.testing.expectEqual(log_logistic_shape_one_vec, direct_log_logistic_shape_one_vec);
    ...
    try std.testing.expectEqual(pareto_vec, direct_pareto_vec);
    inline for (0..4) |lane| try std.testing.expect(pareto_vec[lane] >= 2 and std.math.isFinite(pareto_vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    const vector_pareto_sampler = try VectorPareto(@Vector(4, f64)).init(2, 3);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_pareto_sampler.scaleValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 3), vector_pareto_sampler.shapeValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 3), vector_pareto_sampler.expectedValue().?, 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 3), vector_pareto_sampler.varianceValue().?, 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2) * std.math.pow(f64, 2, 1.0 / 3.0), vector_pareto_sampler.medianValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_pareto_sampler.modeValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_pareto_sampler.minValue(), 1e-12);
    try std.testing.expect(vector_pareto_sampler.maxValue() == null);
    const sampled_pareto_vec = vector_pareto_sampler.sample(rng);
    const direct_sampled_pareto_vec = vector_pareto_sampler.sampleFrom(&direct_engine);
    try std.testing.expectEqual(sampled_pareto_vec, direct_sampled_pareto_vec);
    inline for (0..4) |lane| try std.testing.expect(sampled_pareto_vec[lane] >= 2 and std.math.isFinite(sampled_pareto_vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    var pareto_buf_vec: [3]@Vector(4, f64) = undefined;
    var direct_pareto_buf_vec: [3]@Vector(4, f64) = undefined;
    try fillVectorParetoChecked(rng, @Vector(4, f64), &pareto_buf_vec, 2, 3);
    try fillVectorParetoCheckedFrom(&direct_engine, @Vector(4, f64), &direct_pareto_buf_vec, 2, 3);
    ...
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());
    vector_pareto_sampler.fill(rng, &pareto_buf_vec);
    vector_pareto_sampler.fillFrom(&direct_engine, &direct_pareto_buf_vec);
    try std.testing.expectEqualSlices(@Vector(4, f64), &pareto_buf_vec, &direct_pareto_buf_vec);
    for (pareto_buf_vec) |vec| inline for (0..4) |lane| try std.testing.expect(vec[lane] >= 2 and std.math.isFinite(vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());
    ...
    try std.testing.expect(vector_pareto_shape_one.varianceValue() == null);
    const pareto_shape_one_vec = vector_pareto_shape_one.sample(rng);
    const direct_pareto_shape_one_vec = vector_pareto_shape_one.sampleFrom(&direct_engine);
    ...
    const inverse_gaussian_vec = try vectorInverseGaussianChecked(rng, @Vector(4, f64), 1, 2);
    const direct_inverse_gaussian_vec = try vectorInverseGaussianCheckedFrom(&direct_engine, @Vector(4, f64), 1, 2);
    try std.testing.expectEqual(inverse_gaussian_vec, direct_inverse_gaussian_vec);
    inline for (0..4) |lane| try std.testing.expect(inverse_gaussian_vec[lane] > 0 and std.math.isFinite(inverse_gaussian_vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    const vector_inverse_gaussian_sampler = try VectorInverseGaussian(@Vector(4, f64)).init(1, 2);
    try std.testing.expectApproxEqAbs(@as(f64, 1), vector_inverse_gaussian_sampler.meanValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 2), vector_inverse_gaussian_sampler.shapeValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 1), vector_inverse_gaussian_sampler.expectedValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 0.5), vector_inverse_gaussian_sampler.varianceValue(), 1e-12);
    try std.testing.expectApproxEqAbs(@as(f64, 0), vector_inverse_gaussian_sampler.minValue(), 0);
    try std.testing.expect(vector_inverse_gaussian_sampler.maxValue() == null);
    const sampled_inverse_gaussian_vec = vector_inverse_gaussian_sampler.sample(rng);
    const direct_sampled_inverse_gaussian_vec = vector_inverse_gaussian_sampler.sampleFrom(&direct_engine);
    try std.testing.expectEqual(sampled_inverse_gaussian_vec, direct_sampled_inverse_gaussian_vec);
    inline for (0..4) |lane| try std.testing.expect(sampled_inverse_gaussian_vec[lane] > 0 and std.math.isFinite(sampled_inverse_gaussian_vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());

    var inverse_gaussian_buf_vec: [3]@Vector(4, f64) = undefined;
    var direct_inverse_gaussian_buf_vec: [3]@Vector(4, f64) = undefined;
    try fillVectorInverseGaussianChecked(rng, @Vector(4, f64), &inverse_gaussian_buf_vec, 1, 2);
    try fillVectorInverseGaussianCheckedFrom(&direct_engine, @Vector(4, f64), &direct_inverse_gaussian_buf_vec, 1, 2);
    ...
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());
    vector_inverse_gaussian_sampler.fill(rng, &inverse_gaussian_buf_vec);
    vector_inverse_gaussian_sampler.fillFrom(&direct_engine, &direct_inverse_gaussian_buf_vec);
    try std.testing.expectEqualSlices(@Vector(4, f64), &inverse_gaussian_buf_vec, &direct_inverse_gaussian_buf_vec);
    for (inverse_gaussian_buf_vec) |vec| inline for (0..4) |lane| try std.testing.expect(vec[lane] > 0 and std.math.isFinite(vec[lane]));
    try std.testing.expectEqual(facade_engine.next(), direct_engine.next());
...
test "invalid distribution vector helpers do not consume random stream" {
    ...
    try std.testing.expectError(error.InvalidParameter, vectorLogLogisticCheckedFrom(&engine, @Vector(4, f64), -1, 3));
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try std.testing.expectError(error.InvalidParameter, fillVectorLogLogisticCheckedFrom(&engine, @Vector(4, f64), &uniform_buf, -1, 3));
    try std.testing.expectEqual(control.next(), engine.next());
...
test "zero-length distribution vector fills do not validate or consume random stream" {
    ...
    try std.testing.expectEqual(control.next(), engine.next());
    try fillVectorLogLogisticCheckedFrom(&engine, @Vector(4, f64), &empty, -1, 3);
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try std.testing.expectEqual(control.next(), engine.next());
    try fillVectorLogLogisticChecked(rng, @Vector(4, f64), &empty, -1, 3);
    try std.testing.expectEqual(control.next(), engine.next());
...
test "invalid distribution facade tail fills do not consume random stream" {
    ...
    try std.testing.expectError(error.InvalidParameter, fillLogLogisticChecked(rng, f64, &out, -1, 1));
    try std.testing.expectEqual(control.next(), engine.next());
...
test "degenerate uniform distribution helpers do not consume random stream" {
    const alea = @import("root.zig");
    var engine = alea.ScalarPrng.init(0x5150_d1f3);
    var control = alea.ScalarPrng.init(0x5150_d1f3);
    const rng = Rng.init(&engine);

    try std.testing.expectEqual(@as(f64, 1.5), uniformFrom(&engine, f64, 1.5, 1.5));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(f32, -2.25), try uniformChecked(rng, f32, -2.25, -2.25));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(f64, 3.75), uniformInclusiveFrom(&engine, f64, 3.75, 3.75));
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try fillUniformChecked(rng, f64, &floats, -3.5, -3.5);
    for (floats) |sample| try std.testing.expectEqual(@as(f64, -3.5), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    fillUniformInclusiveFrom(&engine, f64, &floats, 6.5, 6.5);
    for (floats) |sample| try std.testing.expectEqual(@as(f64, 6.5), sample);
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try std.testing.expectEqual(@as(@Vector(8, f32), @splat(-9.5)), try vectorUniformChecked(rng, @Vector(8, f32), -9.5, -9.5));
    try std.testing.expectEqual(control.next(), engine.next());

    try std.testing.expectEqual(@as(@Vector(4, f64), @splat(10.5)), vectorUniformInclusiveFrom(&engine, @Vector(4, f64), 10.5, 10.5));
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    fillVectorUniformFrom(&engine, @Vector(4, f64), &vec_floats, 13.5, 13.5);
    for (vec_floats) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(13.5)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    try fillVectorUniformChecked(rng, @Vector(4, f64), &vec_floats, -14.5, -14.5);
    for (vec_floats) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(-14.5)), sample);
    try std.testing.expectEqual(control.next(), engine.next());

    fillVectorUniformInclusiveFrom(&engine, @Vector(4, f64), &vec_floats, 15.5, 15.5);
    for (vec_floats) |sample| try std.testing.expectEqual(@as(@Vector(4, f64), @splat(15.5)), sample);
    try std.testing.expectEqual(control.next(), engine.next());
...
test "invalid tail scalar helpers do not consume random stream" {
    ...
    try std.testing.expectError(error.InvalidParameter, logLogisticCheckedFrom(&engine, f64, -1, 3));
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try std.testing.expectError(error.InvalidParameter, rayleighCheckedFrom(&engine, f64, -1));
    try std.testing.expectEqual(control.next(), engine.next());
}
...
test "zero-length tail distribution fills do not validate or consume random stream" {
    ...
    try std.testing.expectEqual(control.next(), engine.next());
    try fillLogLogisticCheckedFrom(&engine, f64, &out, -1, 1);
    try std.testing.expectEqual(control.next(), engine.next());
    ...
    try std.testing.expectEqual(control.next(), engine.next());
    try fillRayleighChecked(rng, f64, &out, -1);
    try std.testing.expectEqual(control.next(), engine.next());
...
test "non-uniform samplers can be reused with sample iterators" {
    ...
    for (direct_log_logistic_buf) |value| try std.testing.expect(value > 0);
    try std.testing.expectError(error.InvalidParameter, fillLogLogisticCheckedFrom(&direct_engine, f64, &direct_log_logistic_buf, -1, 3));
    ...
    try std.testing.expectError(error.InvalidParameter, logLogisticCheckedFrom(&direct_engine, f64, -1, 3));
    ...
    for (direct_rayleigh_buf) |value| try std.testing.expect(value >= 0);
    try std.testing.expectError(error.InvalidParameter, fillRayleighCheckedFrom(&direct_engine, f64, &direct_rayleigh_buf, -1));
    ...
    try std.testing.expectError(error.InvalidParameter, rayleighCheckedFrom(&direct_engine, f64, -1));
    ...
    try std.testing.expectError(error.InvalidParameter, LogLogistic(f64).init(-1, 1));
...
test "checked fill helpers preserve valid-parameter stream shape" {
    ...
    inline for (.{ alea.ScalarPrng, alea.DefaultPrng }) |Engine| {
        ...
        var vector_log_logistic_unchecked: [4]@Vector(4, f64) = undefined;
        var vector_log_logistic_checked: [4]@Vector(4, f64) = undefined;
        ...
        var vector_pareto_unchecked: [4]@Vector(4, f64) = undefined;
        var vector_pareto_checked: [4]@Vector(4, f64) = undefined;
        ...
        var vector_inverse_gaussian_unchecked: [4]@Vector(4, f64) = undefined;
        var vector_inverse_gaussian_checked: [4]@Vector(4, f64) = undefined;
