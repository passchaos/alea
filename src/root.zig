...
pub fn sampleWeightedIndices(comptime Weight: type, io: std.Io, allocator: std.mem.Allocator, weights: []const Weight, amount: usize) ![]usize {
    ...
    var engine = try secure(io);
    const random_source = Rng.init(&engine);
    return try seq.sampleWeightedIndices(allocator, random_source, Weight, weights, count);
}
...
pub fn sampleWeightedIndicesU32(comptime Weight: type, io: std.Io, allocator: std.mem.Allocator, weights: []const Weight, amount: usize) ![]u32 {
    ...
    var engine = try secure(io);
    const random_source = Rng.init(&engine);
    return try seq.sampleWeightedIndicesU32(allocator, random_source, Weight, weights, count);
}
...
fn rootChooseIteratorWeightedExactCount(comptime T: type, io: std.Io, iterator: anytype, remaining: usize) !?T {
...
pub fn chooseIteratorWeighted(comptime T: type, io: std.Io, iterator: anytype) !?T {
    ...
    if (rootIteratorExactRemaining(iterator)) |remaining| {
        ...
        return rootChooseIteratorWeightedExactCount(T, io, iterator, remaining);
    }
    const Pending = struct {
        item: T,
        weight: f64,
...
fn rootSampleIteratorFrom(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize, exact_remaining: ?usize, comptime checked: bool) ![]T {
    if (checked) {
...
pub fn sampleIterator(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize) ![]T {
    ...
    return rootSampleIteratorFrom(T, io, allocator, iterator, amount, exact_remaining, false);
}
...
pub fn sampleIteratorChecked(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize) ![]T {
    ...
    return rootSampleIteratorFrom(T, io, allocator, iterator, amount, exact_remaining, true);
}
...
pub fn sampleIteratorWeighted(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize) ![]T {
    if (amount != 0 and comptime rootValueTypeHasEmptyEnum(T)) return error.EmptyRange;
    const exact_remaining = if (amount != 0) rootIteratorExactRemaining(iterator) else null;
    if (exact_remaining) |remaining| {
        if (remaining == 0) return allocator.alloc(T, 0);
    }
    return try rootSampleIteratorWeightedAlloc(T, io, allocator, iterator, amount, false, exact_remaining);
}
...
pub fn sampleIteratorWeightedChecked(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize) ![]T {
    ...
    return try rootSampleIteratorWeightedAlloc(T, io, allocator, iterator, amount, true, exact_remaining);
}
...
pub fn sampleIteratorWeightedArray(comptime T: type, io: std.Io, comptime N: usize, iterator: anytype) !?[N]T {
    ...
    const candidates = (try rootSampleIteratorWeightedCandidateArray(T, io, N, iterator, exact_remaining)) orelse return null;
    var out: [N]T = undefined;
...
pub fn sampleIteratorWeightedArrayChecked(comptime T: type, io: std.Io, comptime N: usize, iterator: anytype) ![N]T {
    ...
    const candidates = (try rootSampleIteratorWeightedCandidateArray(T, io, N, iterator, exact_remaining)) orelse return error.InvalidParameter;
    var out: [N]T = undefined;
    inline for (0..N) |i| out[i] = candidates[i].item;
    return out;
}
...
fn rootSampleIteratorWeightedIntoExactCover(comptime T: type, iterator: anytype, out: []T, remaining: usize, comptime checked: bool) !usize {
    var index: usize = 0;
...
fn rootSampleIteratorWeightedAlloc(comptime T: type, io: std.Io, allocator: std.mem.Allocator, iterator: anytype, amount: usize, comptime checked: bool, exact_remaining: ?usize) ![]T {
    if (amount == 0) return allocator.alloc(T, 0);
    const max_possible = if (exact_remaining) |remaining| blk: {
...
fn rootSampleIteratorWeightedCandidateArray(comptime T: type, io: std.Io, comptime N: usize, iterator: anytype, exact_remaining: ?usize) !?[N]RootWeightedIteratorCandidate(T) {
    if (N == 0) return .{};
    if (exact_remaining) |remaining| {
        if (remaining == N) {
...
test "root random helpers validate deterministic cases before entropy" {
    ...
    var empty_ptr_batch_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.EmptyRange, choosePtrBatchChecked(u8, failing, empty_ptr_batch_checked_alloc.allocator(), 3, &.{}));
    ...
    try std.testing.expectError(error.EmptyInput, sampleWeightedIndices(f64, failing, std.testing.allocator, &.{}, 1));
    var empty_weighted_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesChecked(f64, failing, empty_weighted_checked_alloc.allocator(), &.{}, 1));
    try std.testing.expectError(error.EmptyInput, sampleWeightedIndicesU32(f64, failing, std.testing.allocator, &.{}, 1));
    var empty_weighted_u32_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesU32Checked(f64, failing, empty_weighted_u32_checked_alloc.allocator(), &.{}, 1));
    const zero_weighted_indices = try sampleWeightedIndices(f64, failing, std.testing.allocator, &.{ 0, 0, 0 }, 2);
    defer std.testing.allocator.free(zero_weighted_indices);
    try std.testing.expectEqual(@as(usize, 0), zero_weighted_indices.len);
    var zero_weighted_indices_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesChecked(f64, failing, zero_weighted_indices_checked_alloc.allocator(), &.{ 0, 0, 0 }, 2));
    const zero_weighted_indices_u32 = try sampleWeightedIndicesU32(f64, failing, std.testing.allocator, &.{ 0, 0, 0 }, 2);
    defer std.testing.allocator.free(zero_weighted_indices_u32);
    try std.testing.expectEqual(@as(usize, 0), zero_weighted_indices_u32.len);
    var zero_weighted_indices_u32_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesU32Checked(f64, failing, zero_weighted_indices_u32_checked_alloc.allocator(), &.{ 0, 0, 0 }, 2));
    const single_weighted_indices = try sampleWeightedIndices(f64, failing, std.testing.allocator, &.{ 0, 5, 0 }, 3);
    defer std.testing.allocator.free(single_weighted_indices);
    try std.testing.expectEqualSlices(usize, &.{1}, single_weighted_indices);
    const single_weighted_indices_checked = try sampleWeightedIndicesChecked(f64, failing, std.testing.allocator, &.{ 0, 5, 0 }, 1);
    defer std.testing.allocator.free(single_weighted_indices_checked);
    try std.testing.expectEqualSlices(usize, &.{1}, single_weighted_indices_checked);
    var weighted_indices_checked_invalid_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesChecked(f64, failing, weighted_indices_checked_invalid_alloc.allocator(), &.{ 1, 2, 3 }, 4));
    const single_weighted_indices_u32 = try sampleWeightedIndicesU32(f64, failing, std.testing.allocator, &.{ 0, 5, 0 }, 3);
    defer std.testing.allocator.free(single_weighted_indices_u32);
    try std.testing.expectEqualSlices(u32, &.{1}, single_weighted_indices_u32);
    const single_weighted_indices_u32_checked = try sampleWeightedIndicesU32Checked(f64, failing, std.testing.allocator, &.{ 0, 5, 0 }, 1);
    defer std.testing.allocator.free(single_weighted_indices_u32_checked);
    try std.testing.expectEqualSlices(u32, &.{1}, single_weighted_indices_u32_checked);
    var weighted_indices_u32_checked_invalid_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, sampleWeightedIndicesU32Checked(f64, failing, weighted_indices_u32_checked_invalid_alloc.allocator(), &.{ 1, 2, 3 }, 4));
    ...
    const EmptyEnumWeight = struct {
        fn positive(_: *const EmptyEnum) f64 {
            return 1;
        }
    };
    var weighted_by_empty_enum_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    var weighted_by_empty_enum_failed = false;
    if (sampleWeightedBy(EmptyEnum, f64, failing, weighted_by_empty_enum_alloc.allocator(), weighted_empty_enum_items, 1, EmptyEnumWeight.positive)) |values| {
        weighted_by_empty_enum_alloc.allocator().free(values);
    } else |err| {
        try std.testing.expectEqual(error.EmptyRange, err);
        weighted_by_empty_enum_failed = true;
    }
    try std.testing.expect(weighted_by_empty_enum_failed);
    var weighted_by_empty_enum_checked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    var weighted_by_empty_enum_checked_failed = false;
    if (sampleWeightedByChecked(EmptyEnum, f64, failing, weighted_by_empty_enum_checked_alloc.allocator(), weighted_empty_enum_items, 1, EmptyEnumWeight.positive)) |values| {
        weighted_by_empty_enum_checked_alloc.allocator().free(values);
    } else |err| {
        try std.testing.expectEqual(error.EmptyRange, err);
        weighted_by_empty_enum_checked_failed = true;
    }
    try std.testing.expect(weighted_by_empty_enum_checked_failed);
    ...
    try std.testing.expectEqual(@as(usize, 1), exact_single_choice.remaining_calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), exact_two_choice.remaining_calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), exact_two_stable.remaining_calls);
    ...
    var by_index_u32_checked_empty_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.EmptyInput, weightedIndexU32BatchByIndexChecked(f64, failing, by_index_u32_checked_empty_alloc.allocator(), 3, 3, RootByIndexWeights.zero));
    ...
    try std.testing.expectError(error.InvalidWeight, weightedIndexU32BatchByIndex(f64, failing, std.testing.allocator, 3, 3, RootByIndexWeights.invalid));
    ...
    var by_index_u32_invalid_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    ...
    if (chooseWeightedValueArray(EmptyEnum, failing, 1, weighted_empty_enum_items, &.{1})) |_| {
        ...
        try std.testing.expectEqual(error.EmptyRange, err);
        choose_weighted_empty_array_failed = true;
    }
    try std.testing.expect(choose_weighted_empty_array_failed);
    var choose_weighted_empty_array_checked_failed = false;
    if (chooseWeightedValueArrayChecked(EmptyEnum, failing, 1, weighted_empty_enum_items, &.{1})) |_| {
        return error.TestExpectedError;
    } else |err| {
        try std.testing.expectEqual(error.EmptyRange, err);
        choose_weighted_empty_array_checked_failed = true;
    }
    try std.testing.expect(choose_weighted_empty_array_checked_failed);
    ...
    var bad_scalar_less_unchecked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.EmptyRange, unicodeScalarRangeLessThanBatch(failing, bad_scalar_less_unchecked_alloc.allocator(), 3, 0x41, 0x41));
    var bad_scalar_invalid_unchecked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, unicodeScalarRangeLessThanBatch(failing, bad_scalar_invalid_unchecked_alloc.allocator(), 3, 0xD800, 0xE000));
    ...
    var bad_scalar_at_most_unchecked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.EmptyRange, unicodeScalarRangeAtMostBatch(failing, bad_scalar_at_most_unchecked_alloc.allocator(), 3, 0x42, 0x41));
    var bad_scalar_at_most_invalid_unchecked_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    try std.testing.expectError(error.InvalidParameter, unicodeScalarRangeAtMostBatch(failing, bad_scalar_at_most_invalid_unchecked_alloc.allocator(), 3, 0xD800, 0xE000));
    ...
    const ExactCountWeightedChoiceIter = struct {
    ...
    const weighted_exact_single_positive_entries = [_]WeightedIter.Entry{
        .{ .item = 1, .weight = 0 },
        .{ .item = 2, .weight = 5 },
        .{ .item = 3, .weight = 0 },
    };
    var weighted_exact_single_positive = ExactCountWeightedChoiceIter{ .entries = &weighted_exact_single_positive_entries };
    try std.testing.expectEqual(@as(?u8, 2), try chooseIteratorWeighted(u8, failing, &weighted_exact_single_positive));
    try std.testing.expectEqual(@as(usize, 3), weighted_exact_single_positive.calls);
    var weighted_exact_single_positive_checked = ExactCountWeightedChoiceIter{ .entries = &weighted_exact_single_positive_entries };
    try std.testing.expectEqual(@as(u8, 2), try chooseIteratorWeightedChecked(u8, failing, &weighted_exact_single_positive_checked));
    try std.testing.expectEqual(@as(usize, 3), weighted_exact_single_positive_checked.calls);
    const weighted_exact_two_positive_entries = [_]WeightedIter.Entry{
        .{ .item = 1, .weight = 1 },
        .{ .item = 2, .weight = 1 },
    };
    var weighted_exact_two_positive = ExactCountWeightedChoiceIter{ .entries = &weighted_exact_two_positive_entries };
    try std.testing.expectError(error.EntropyUnavailable, chooseIteratorWeighted(u8, failing, &weighted_exact_two_positive));
    try std.testing.expectEqual(@as(usize, 2), weighted_exact_two_positive.calls);
    const weighted_exact_invalid_entries = [_]WeightedIter.Entry{
        .{ .item = 1, .weight = 0 },
        .{ .item = 2, .weight = std.math.nan(f64) },
    };
    var weighted_exact_invalid = ExactCountWeightedChoiceIter{ .entries = &weighted_exact_invalid_entries };
    try std.testing.expectError(error.InvalidWeight, chooseIteratorWeighted(u8, failing, &weighted_exact_invalid));
    try std.testing.expectEqual(@as(usize, 2), weighted_exact_invalid.calls);
    ...
    var sample_iter_known_empty_alloc = std.testing.FailingAllocator.init(std.testing.allocator, .{ .fail_index = 0 });
    var sample_iter_known_empty = SliceIter{ .items = &.{} };
    const sample_iter_known_empty_out = try sampleIterator(u8, failing, sample_iter_known_empty_alloc.allocator(), &sample_iter_known_empty, 4);
    ...
    const ExactShortSampleIter = struct {
        next_value: u8 = 1,
        end: u8 = 3,
        calls: usize = 0,
    ...
    var sample_iter_exact_short = ExactShortSampleIter{};
    const sample_iter_exact_short_out = try sampleIterator(u8, failing, std.testing.allocator, &sample_iter_exact_short, 8);
    defer std.testing.allocator.free(sample_iter_exact_short_out);
    try std.testing.expectEqualSlices(u8, &.{ 1, 2 }, sample_iter_exact_short_out);
    try std.testing.expectEqual(@as(usize, 2), sample_iter_exact_short.calls);
    ...
    const ExactLongSampleIter = struct {
        items: []const u8,
        index: usize = 0,
        calls: usize = 0,
    ...
    var sample_iter_exact_long = ExactLongSampleIter{ .items = &.{ 1, 2, 3 } };
    try std.testing.expectError(error.EntropyUnavailable, sampleIterator(u8, failing, std.testing.allocator, &sample_iter_exact_long, 2));
    try std.testing.expectEqual(@as(usize, 3), sample_iter_exact_long.calls);
    var sample_iter_exact_long_checked = ExactLongSampleIter{ .items = &.{ 1, 2, 3 } };
    try std.testing.expectError(error.EntropyUnavailable, sampleIteratorChecked(u8, failing, std.testing.allocator, &sample_iter_exact_long_checked, 2));
    try std.testing.expectEqual(@as(usize, 3), sample_iter_exact_long_checked.calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), weighted_sample_exact_count.remaining_calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), weighted_sample_exact_count_checked.remaining_calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), weighted_sample_exact_sparse.remaining_calls);
    ...
    try std.testing.expectEqual(@as(usize, 1), weighted_sample_exact_sparse_checked.remaining_calls);
    ...
    const ExactLongWeightedSampleIter = struct {
        entries: []const WeightedIter.Entry,
        index: usize = 0,
        calls: usize = 0,
    ...
    var weighted_sample_exact_long = ExactLongWeightedSampleIter{ .entries = &weighted_entropy_entries };
    try std.testing.expectError(error.EntropyUnavailable, sampleIteratorWeighted(u8, failing, std.testing.allocator, &weighted_sample_exact_long, 1));
    try std.testing.expectEqual(@as(usize, 2), weighted_sample_exact_long.calls);
    var weighted_sample_exact_long_checked = ExactLongWeightedSampleIter{ .entries = &weighted_entropy_entries };
    try std.testing.expectError(error.EntropyUnavailable, sampleIteratorWeightedChecked(u8, failing, std.testing.allocator, &weighted_sample_exact_long_checked, 1));
    try std.testing.expectEqual(@as(usize, 2), weighted_sample_exact_long_checked.calls);
    ...
    const ExactCountWeightedIntoIter = struct {
        entries: []const WeightedIter.Entry,
        index: usize = 0,
        calls: usize = 0,
        remaining_calls: usize = 0,
    ...
    var weighted_into_exact_count = ExactCountWeightedIntoIter{ .entries = &weighted_entropy_entries };
    ...
    var weighted_into_exact_sparse = ExactCountWeightedIntoIter{ .entries = &weighted_into_exact_sparse_entries };
    ...
    var weighted_array_exact_single = SingleExactWeightedIntoIter{ .entry = .{ .item = 42, .weight = 5 } };
    try std.testing.expectEqualSlices(u8, &.{42}, &(try sampleIteratorWeightedArray(u8, failing, 1, &weighted_array_exact_single)).?);
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_single.calls);
    var weighted_array_exact_single_checked = SingleExactWeightedIntoIter{ .entry = .{ .item = 42, .weight = 5 } };
    try std.testing.expectEqualSlices(u8, &.{42}, &(try sampleIteratorWeightedArrayChecked(u8, failing, 1, &weighted_array_exact_single_checked)));
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_single_checked.calls);
    var weighted_array_exact_zero = SingleExactWeightedIntoIter{ .entry = .{ .item = 42, .weight = 0 } };
    try std.testing.expectEqual(@as(?[1]u8, null), try sampleIteratorWeightedArray(u8, failing, 1, &weighted_array_exact_zero));
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_zero.calls);
    var weighted_array_exact_zero_checked = SingleExactWeightedIntoIter{ .entry = .{ .item = 42, .weight = 0 } };
    try std.testing.expectError(error.InvalidParameter, sampleIteratorWeightedArrayChecked(u8, failing, 1, &weighted_array_exact_zero_checked));
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_zero_checked.calls);
    const ExactCountWeightedArrayIter = struct {
        entries: []const WeightedIter.Entry,
        index: usize = 0,
        calls: usize = 0,
        remaining_calls: usize = 0,
    ...
    var weighted_array_exact_count = ExactCountWeightedArrayIter{ .entries = &weighted_entropy_entries };
    try std.testing.expectEqualSlices(u8, &.{ 1, 2 }, &(try sampleIteratorWeightedArray(u8, failing, 2, &weighted_array_exact_count)).?);
    try std.testing.expectEqual(@as(usize, 2), weighted_array_exact_count.calls);
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_count.remaining_calls);
    var weighted_array_exact_count_checked = ExactCountWeightedArrayIter{ .entries = &weighted_entropy_entries };
    try std.testing.expectEqualSlices(u8, &.{ 1, 2 }, &(try sampleIteratorWeightedArrayChecked(u8, failing, 2, &weighted_array_exact_count_checked)));
    try std.testing.expectEqual(@as(usize, 2), weighted_array_exact_count_checked.calls);
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_count_checked.remaining_calls);
    const weighted_array_exact_count_zero_entries = [_]WeightedIter.Entry{
        .{ .item = 1, .weight = 1 },
        .{ .item = 2, .weight = 0 },
    };
    var weighted_array_exact_count_zero = ExactCountWeightedArrayIter{ .entries = &weighted_array_exact_count_zero_entries };
    try std.testing.expectEqual(@as(?[2]u8, null), try sampleIteratorWeightedArray(u8, failing, 2, &weighted_array_exact_count_zero));
    try std.testing.expectEqual(@as(usize, 2), weighted_array_exact_count_zero.calls);
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_count_zero.remaining_calls);
    var weighted_array_exact_count_zero_checked = ExactCountWeightedArrayIter{ .entries = &weighted_array_exact_count_zero_entries };
    try std.testing.expectError(error.InvalidParameter, sampleIteratorWeightedArrayChecked(u8, failing, 2, &weighted_array_exact_count_zero_checked));
    try std.testing.expectEqual(@as(usize, 2), weighted_array_exact_count_zero_checked.calls);
    try std.testing.expectEqual(@as(usize, 1), weighted_array_exact_count_zero_checked.remaining_calls);
    var weighted_array_known_short = WeightedIter{ .items = &weighted_entropy_entries };
    try std.testing.expectEqual(@as(?[3]u8, null), try sampleIteratorWeightedArray(u8, failing, 3, &weighted_array_known_short));
    try std.testing.expectEqual(@as(usize, 0), weighted_array_known_short.index);
