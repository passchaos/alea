...
pub fn main(init: std.process.Init) !void {
    ...
    try stdout.print("\nrange throughput\n", .{});
    try benchRange(io, stdout, "alea bounded u32", bytes / 8);
    try stdout.print("\nsequence throughput\n", .{});
    try benchSeq(io, stdout, "alea sample indices", 1_000_000, 10_000);
    try benchSeqIndexVec(io, stdout, "alea sample index vec", 1_000_000, 10_000);
    try benchSeqU32(io, stdout, "alea sample indices u32", 1_000_000, 10_000);
    try stdout.flush();
}
...
fn benchRange(io: std.Io, stdout: *std.Io.Writer, name: []const u8, count: usize) !void {
...
fn benchSeq(io: std.Io, stdout: *std.Io.Writer, name: []const u8, length: usize, amount: usize) !void {
...
fn benchSeqIndexVec(io: std.Io, stdout: *std.Io.Writer, name: []const u8, length: usize, amount: usize) !void {
    var best_thousand_per_s: f64 = 0;
    var best_checksum: usize = 0;
    var trial: usize = 0;
    while (trial < trials) : (trial += 1) {
...
fn benchSeqU32(io: std.Io, stdout: *std.Io.Writer, name: []const u8, length: u32, amount: u32) !void {
    var best_thousand_per_s: f64 = 0;
    var best_checksum: u64 = 0;
    var trial: usize = 0;
    while (trial < trials) : (trial += 1) {
