...
- collection helpers for one-shot and repeated usize/u32 index, value, const-pointer, and mutable-pointer choice, `choose`, `shuffle`, fixed-size u32 index and item/pointer arrays, allocation-returning and caller-owned item/pointer subsets with both `chooseMultiple*` and Rust-discoverable `sampleItems*` naming, partial shuffle selected/rest splits, compact `IndexVec` index samples with lazy/caller-owned/allocation-returning value and pointer mapping plus u32 export mapping, f64 and generic repeated weighted index/value/pointer batches, accessor-based weighted fixed-size index/value/pointer arrays, allocation-returning, caller-owned, and fixed-size index/u32-index/IndexVec samples from item or index weight accessors, plus value/const-pointer/mutable-pointer choices, allocation-returning samples, and caller-owned no-replacement buffers for item-embedded weights, compact weighted IndexVec samples, allocation-returning weighted u32 index slices, fixed-size weighted u32 index arrays, caller-owned weighted u32 index buffers,
