...
- collection helpers for one-shot and repeated usize/u32 index, value, const-pointer, and mutable-pointer choice, `choose`, `shuffle`, fixed-size u32 index and item/pointer arrays, allocation-returning and caller-owned item/pointer subsets, partial shuffle selected/rest splits, compact `IndexVec` index samples with lazy/caller-owned/allocation-returning value and pointer mapping plus u32 export mapping, f64 and generic repeated usize/u32 weighted index batches, compact weighted IndexVec samples, allocation-returning weighted u32 index slices, fixed-size weighted u32 index arrays, caller-owned weighted u32 index buffers,
