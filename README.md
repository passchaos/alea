...
- collection helpers for one-shot index choice, `choose`, const/mutable pointer choice, `shuffle`, fixed-size item/pointer arrays, allocation-returning and caller-owned item/pointer subsets, partial shuffle selected/rest splits, compact `IndexVec` index samples with lazy/caller-owned value and pointer mapping, weighted indexes,
