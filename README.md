...
- scalar helpers for integers, floats, durations, ranges, booleans including Rust-discoverable `randomBool` / `randomRatio` aliases, caller-owned bytes, and allocation-returning byte buffers
...
- reusable uniform with Rust-discoverable `new` / `newInclusive` and one-shot `sampleSingle` / `sampleSingleInclusive` aliases, Bernoulli with Rust-discoverable `new` / `fromRatio` / `p()` aliases, non-uniform distribution, opt-in bounded f32
