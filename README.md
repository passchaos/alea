...
- collection helpers for `choose`, const/mutable pointer choice, `shuffle`, fixed-size item/pointer arrays, caller-owned item and pointer subsets, partial shuffle selected/rest splits, compact `IndexVec` index samples with lazy/caller-owned value and pointer mapping, weighted indexes,
  repeated choice iterators, one-shot weighted item/const-pointer/mutable-pointer helpers, weighted choice samplers, weighted fixed-size pointer arrays, caller-owned weighted pointer buffers, weighted sampling without
  replacement, iterator and weighted iterator sampling with and without
  replacement, fixed-size and caller-owned iterator sampling, allocated and caller-owned reservoir sampling, adaptive and caller-owned usize/u32 index sampling, and checked
