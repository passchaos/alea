# S4-M605 Root By-Index Weighted Mutable-Pointer Batch Prevalidation

## Gap

Root `chooseWeightedPtrBatchByIndex` and
`chooseWeightedPtrBatchByIndexChecked` allocated output buffers before checking
invalid weights and deterministic empty/single by-index weighted-index states.
Those failures should be reported before random-output allocation and before root
secure-engine construction.

This milestone aligns the allocation-returning by-index weighted mutable-pointer
batch helpers with the validated weighted-index root helpers.
...
- Empty/all-zero index weights still return nullable `null` pointer batches for
  unchecked requests and `error.EmptyInput` for checked requests before entropy.
- Single-positive index weights still allocate and fill repeated mutable pointers
...
S4-M605 is closed for the current bar: root by-index weighted mutable-pointer
