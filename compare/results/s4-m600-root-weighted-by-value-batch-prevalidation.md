# S4-M600 Root Item-Accessor Weighted Value Batch Prevalidation

## Gap

Root `chooseWeightedBatchBy` and `chooseWeightedBatchByChecked` allocated output
buffers before checking invalid weights and deterministic empty/single
...
This milestone aligns the allocation-returning item-accessor weighted value batch
...
- Empty/all-zero item weights still return nullable `null` batches for unchecked
  requests and `error.EmptyInput` for checked requests before entropy.
- Single-positive item weights still allocate and fill repeated values before
...
S4-M600 is closed for the current bar: root item-accessor weighted value batch
