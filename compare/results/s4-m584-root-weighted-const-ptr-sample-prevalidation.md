# S4-M584 Root Parallel-Weighted Const-Pointer Sample Prevalidation

## Gap

The root parallel-weighted `sampleWeightedPtrs` and
`sampleWeightedPtrsChecked` helpers always constructed a secure engine for
non-zero requests before delegating to `seq.sampleWeightedPtrs*`. The value
...
This milestone aligns the parallel-weighted root const-pointer sample helpers
...
S4-M584 is closed for the current bar: root parallel-weighted no-replacement
const-pointer sample helpers now validate deterministic no-entropy paths before
