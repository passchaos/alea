# S4-M631 Root Item-Accessor Weighted Value Sample Empty-Type Prevalidation

## Gap

Root item-accessor weighted no-replacement value sample helpers could allocate
output buffers or proceed toward secure-engine construction for non-zero requests
whose value type is uninhabited. Such requests cannot produce values and should
fail before allocation or entropy.

This milestone aligns item-accessor weighted value sampling with generic value
batch and parallel-weighted value sample empty-type validation.
...
- Empty input errors still keep their existing precedence.
...
S4-M631 is closed for the current bar: root item-accessor weighted value sample
helpers now prevalidate non-zero uninhabited value types before random-output
