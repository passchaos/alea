...
| `Bernoulli`, `BernoulliError`, `from_ratio`, `p` | Covered by `Bernoulli`, vector Bernoulli, `fromRatio`, `p()`, aliases, and diagnostics; see S4-M225, S4-M227, S4-M228, S4-M263. |
...
| `WeightedIndex`, `WeightedIndexIter`, `Weight`, `weighted::WeightedIndex`, `weighted::Error`, `weight`, `update_weights`, `weights`, `total_weight` | Covered by root/prelude `WeightedIndex`, `AliasTable`, `distributions.weighted.WeightedIndex`, optional single `weight`, `weights` / `weightIter`, `updateWeights`, `totalWeight`, typed `typedWeight*` / `weightValue*`, `typedWeights*` / `weightsValue*`, `typedTotalWeight` / `totalWeightValue`, typed iterator diagnostics, weighted error aliases/variants, weighted error message helpers, root/prelude weighted error aliases, root/prelude weighted sampler aliases, and weighted iterator clone/format helpers; Rust `Weight` trait bounds are implementation machinery; see S4-M191, S4-M192, S4-M221, S4-M265, S4-M271, S4-M281, S4-M284, S4-M1172, S4-M1174, S4-M1176, S4-M1180, S4-M1182, and S4-M1195. |
...
| `seq::index::{IndexVec, IndexVecIter, IndexVecIntoIter, sample, sample_weighted, sample_array, len, is_empty, index, iter, into_vec}` | Covered by top-level `seq.IndexVec`, index sample helpers, weighted index helpers, fixed-size array helpers, length/empty diagnostics, checked `index`, iterators, and consuming `intoVec`; the intermediate namespace is audited in S4-M287. |
...
| `block::{Generator, BlockRng, reconstruct, reset_and_skip, word_offset, remaining_results, next_word, next_u64_from_u32, fill_bytes}` | Audited in S4-M286 as Rust implementation scaffolding; Alea engines expose explicit state/fill APIs instead. |
