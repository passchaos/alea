...
| `Beta`, `Binomial`, `Cauchy`, `ChiSquared`, `FisherF`, `Frechet`, `Gamma`, `Geometric`, `StandardGeometric`, `Gumbel`, `Hypergeometric`, `InverseGaussian`, `NormalInverseGaussian`, `Pareto`, `Pert`, `PertBuilder`, `Poisson`, `Poisson::MAX_LAMBDA`, `SkewNormal`, `StudentT`, `Triangular`, `Weibull`, `Zeta`, `Zipf` | Covered by concrete Alea samplers and docs. Local `new(...)` discovery is covered where semantics match; `Poisson::MAX_LAMBDA` maps to `poisson_max_lambda`, `poissonMaxLambda`, `Poisson.max_lambda`, `Poisson.MAX_LAMBDA`, and vector `VectorPoisson` constants; `PertBuilder` maps to Alea's range-first `PertBuilder(T)` with `with_shape` / `withShape`, `with_mean` / `withMean`, and `with_mode` / `withMode` workflows; `SkewNormal` location/scale/shape accessors map to Alea `locationValue` / `scaleValue` / `shapeValue` and `locationParameter` / `scaleParameter` / `shapeParameter` aliases because exact Rust method names would collide with Zig public fields; `Geometric` failure-count semantics map to `GeometricFailures`; see S4-M292, S4-M298, S4-M1197, and `distribution-parity-matrix.md`. |
...
| `WeightedAliasIndex` and `weights` | Covered by `AliasTable(Weight)`, root/prelude `WeightedAliasIndex(Weight)`, and `distributions.weighted.WeightedAliasIndex(Weight)` for O(1) repeated weighted sampling and diagnostics, including `weights` / `weightIter` export, typed `weightsValue*` / `typedWeights*` preservation of the original `Weight`, typed iterator diagnostics, iterator clone/format diagnostics, weighted error messages, and root/prelude weighted error discovery; see S3-M4, S4-M1172, S4-M1174, S4-M1176, S4-M1180, S4-M1182, and `distribution-parity-matrix.md`. |
| `WeightedTreeIndex`, `new`, `Default`, `Clone`, `Debug`, `PartialEq`, `is_empty`, `len`, `is_valid`, `get`, `pop`, `push`, `update`, and `try_sample` | Covered by `WeightedTree(Weight)` / root/prelude `WeightedTreeIndex(Weight)` / `distributions.weighted.WeightedTreeIndex(Weight)` and `WeightedIntTree(Weight)` / root/prelude `WeightedIntTreeIndex(Weight)` / `distributions.weighted.WeightedIntTreeIndex(Weight)` dynamic weighted samplers; `new`, `initEmpty` / `default`, `clone`, `eql`, `{f}` formatting, `isEmpty`, `len`, `isValid`, `get`, typed `getValue` / `weightValue`, typed `typedTotalWeight` / `totalWeightValue`, typed weight iterators/exports, `pop` plus typed `popValue`, `push`, `update`, and `trySample` / checked sample methods expose the corresponding local dynamic-tree workflows; see S3-M4, S4-M1167-S4-M1172, S4-M1187, S4-M1189, and `distribution-parity-matrix.md`. |
