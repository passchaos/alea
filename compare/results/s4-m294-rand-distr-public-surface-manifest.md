...
| `Normal`, `StandardNormal`, `LogNormal` | Covered by `Normal(T)`, `StandardNormal(T)`, `LogNormal(T)`, vector/opt-in profiles, `new`, `fromMeanCv`, and `fromZScore`; see S4-M289, S4-M292, S4-M293 and `distribution-parity-matrix.md`. |
| `Exp`, `Exp1` | Covered by `Exp(T)` / `Exp1(T)` aliases over `Exponential(T)` / `StandardExponential(T)`; see S4-M290. |
| `Beta`, `Binomial`, `Cauchy`, `ChiSquared`, `FisherF`, `Frechet`, `Gamma`, `Geometric`, `StandardGeometric`, `Gumbel`, `Hypergeometric`, `InverseGaussian`, `NormalInverseGaussian`, `Pareto`, `Pert`, `Poisson`, `SkewNormal`, `StudentT`, `Triangular`, `Weibull`, `Zeta`, `Zipf` | Covered by concrete Alea samplers and docs. Local `new(...)` discovery is covered where semantics match; `Geometric` failure-count semantics map to `GeometricFailures`; see S4-M292 and `distribution-parity-matrix.md`. |
| `*Error` aliases such as `NormalError`, `ExpError`, `GammaError`, `PoissonError`, `ZipfError` | Covered by aliases over `distributions.Error`; see S4-M289. |
