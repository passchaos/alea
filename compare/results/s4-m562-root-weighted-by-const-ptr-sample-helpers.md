# S4-M562 Root Item-Accessor Weighted Const-Pointer Sample Helpers

## Gap

S4-M561 added root item-accessor weighted value sample helpers, but callers that
wanted allocation-returning no-replacement weighted `*const T` samples from
item-local weights still had to construct a secure engine manually and call
`seq.sampleWeightedPtrsBy*`.
...
primitive; this milestone closes the root ergonomics gap for borrowed samples
...
- `sampleWeightedPtrsBy`
- `sampleWeightedPtrsByChecked`

Both helpers take an item slice and a comptime item-weight accessor. The
unchecked helper returns an allocated slice of const pointers with up to
...
- All-zero unchecked item weights return an empty allocated pointer slice.
...
- `examples/basic.zig` demonstrates `byPtrSample=` in the dedicated
...
Runnable example excerpt showing the guarded root item-accessor weighted const-pointer sample token:

```text
$ zig build run-basic | grep "root weighted by no-replacement helpers"
root weighted by no-replacement helpers: bySample=[green, blue], byPtrSample=[green, blue]
```
...
S4-M562 is closed for the current bar: root system-entropy callers can allocate
no-replacement weighted const-pointer samples directly from an item slice and
