...
infinite-degree standard-normal behavior and zero-length checked-fill semantics.
...
- `src/distributions.zig` updates scalar `studentT` / `studentTChecked` to
  construct `StudentT(T)` once and call facade `sample` directly, with direct
  standard-normal behavior for infinite degrees of freedom.
- `src/distributions.zig` updates scalar `fillStudentT` / `fillStudentTChecked`
  ...
  infinite-degree standard-normal and checked zero-length fast paths.
