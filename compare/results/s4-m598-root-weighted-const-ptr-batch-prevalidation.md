# S4-M598 Root Weighted Const-Pointer Batch Prevalidation

## Gap

Root `chooseWeightedConstPtrBatch` and `chooseWeightedConstPtrBatchChecked`
allocated their output buffers before checking length mismatches, invalid
weights, and deterministic empty/single weighted-index states. Those failures
should be reported before random-output allocation and before root secure-engine
construction.

This milestone aligns the allocation-returning weighted const-pointer batch
...
- Single-positive weights still allocate and fill repeated pointers before
...
S4-M598 is closed for the current bar: root weighted const-pointer batch helpers
