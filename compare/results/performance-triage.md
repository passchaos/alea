...
| Extra continuous distribution breadth | No direct local `rand_distr` rows because these are Alea product-breadth additions | Added Laplace, Logistic, and Rayleigh reusable/direct samplers; local throughput rows are about 139M, 150M, and 167M samples/s | Adopted for feature breadth beyond the local Rust distribution list |
