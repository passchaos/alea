...
| Probability bool bulk/vector sampling | No direct Rust row; fills a Zig-native API gap around repeated `chance(p)` workloads | Added `chanceFrom`, `fillChance`, `fillChanceFrom`, and `vectorChance`/`vectorChanceFrom`; benchmark rows track p=0.25 bool throughput | Adopted for ergonomic high-volume probability sampling and vector API coverage. |
