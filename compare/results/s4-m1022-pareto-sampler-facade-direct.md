# S4-M1022 Pareto Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `Pareto` facade sample/fill helpers still routed through
`sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw
strict-open uniforms and apply the Pareto inverse-power transform directly; the
facade paths can now do the same through facade `Rng` while preserving collapsed
`scale == 0` and `shape == inf` point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while Alea's Pareto
...
- `src/distributions.zig` updates `Pareto(T).sample` and `Pareto(T).fill` to draw
  strict-open uniforms directly through facade `Rng` and apply the shape-one or
  generic Pareto transform, with no-consume point-mass fast paths.
- `src/distributions.zig` updates `VectorPareto(VectorType).sample` and
  `VectorPareto(VectorType).fill` to draw vector strict-open uniforms directly
...
$ zig test src/distributions.zig --test-filter "degenerate pareto helpers do not consume random stream"
1/2 distributions.test.degenerate pareto helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1022 is closed for the current bar: reusable scalar/vector Pareto facade
