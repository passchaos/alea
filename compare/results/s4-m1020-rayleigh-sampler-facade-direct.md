# S4-M1020 Rayleigh Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `Rayleigh` facade sample/fill helpers still routed through
`sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw
strict-open uniforms and apply the Rayleigh square-root/log transform directly;
the facade paths can now do the same through facade `Rng` while preserving
collapsed `scale == 0` point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while prior Alea
`rand_distr` surface evidence records Rayleigh as product-breadth beyond the
...
- `src/distributions.zig` updates `Rayleigh(T).sample` and `Rayleigh(T).fill` to
  draw strict-open uniforms directly through facade `Rng` and apply the Rayleigh
  transform, with a `scale == 0` no-consume fast path.
- `src/distributions.zig` updates `VectorRayleigh(VectorType).sample` and
  `VectorRayleigh(VectorType).fill` to draw vector strict-open uniforms directly
  through facade `Rng` and call `rayleighFromOpenUniformVector`, with the same
...
$ zig test src/distributions.zig --test-filter "degenerate rayleigh helpers do not consume random stream"
1/2 distributions.test.degenerate rayleigh helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1020 is closed for the current bar: reusable scalar/vector Rayleigh facade
