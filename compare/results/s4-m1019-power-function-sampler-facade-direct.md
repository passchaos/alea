# S4-M1019 PowerFunction Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `PowerFunction` facade sample/fill helpers still routed
through `sampleFrom` / `fillFrom` wrappers. The direct-source paths already use
cached method variants for point masses, uniform, square-root, and generic power
transforms; the facade paths can now do the same through facade `Rng` while
preserving collapsed `min == max` and `shape == inf` point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while prior Alea
`rand_distr` surface evidence records PowerFunction as product-breadth beyond the
...
- `src/distributions.zig` updates `PowerFunction(T).sample` and
  `PowerFunction(T).fill` to dispatch cached method variants directly through
  facade `Rng`, including point-mass no-consume fast paths.
- `src/distributions.zig` updates `VectorPowerFunction(VectorType).sample` and
  `VectorPowerFunction(VectorType).fill` to draw vector ranges or strict-open
  uniforms directly through facade `Rng` and call the matching vector transform
  helpers, with the same point-mass no-consume fast paths.
- Direct-source `sampleFrom` / `fillFrom` remain unchanged for explicit
  direct-source workflows and top-level helpers.

## Validation

Focused PowerFunction tests:
...
$ zig test src/distributions.zig --test-filter "degenerate power-function helpers do not consume random stream"
1/2 distributions.test.degenerate power-function helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1019 is closed for the current bar: reusable scalar/vector PowerFunction
