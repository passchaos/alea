# S4-M633 Root Item-Accessor Weighted Value Array Empty-Type Prevalidation

## Gap

Root item-accessor weighted no-replacement fixed-size value array helpers could
...
This milestone aligns item-accessor weighted fixed-size value array sampling with
weighted value sample empty-type validation.
...
- Empty input and count errors still keep their existing precedence.
...
S4-M633 is closed for the current bar: root item-accessor weighted fixed-size
value array helpers now prevalidate non-zero uninhabited value types before
