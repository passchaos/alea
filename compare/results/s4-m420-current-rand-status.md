...
S4-M1233 fixes `std.Random` adapter `nextU32` byte-stream shape after S4-M1232
hardened owned byte allocation for fallible byte sources. The retained status and validation evidence include:
...
rand_distr standard-normal: 39.5 M samples/s checksum=-3.640
...
Wasmtime profilelongcheck run. `compare/results/s4-m1233-fromrandom-nextu32-byte-shape.md` records the latest focused `std.Random` adapter byte-shape hardening after S4-M1232.
S4-M11 is closed for the current bar; S4-M1220 through S4-M1233 are also closed for their current bars, but exact/default-compatible dense SIMD
normal/exponential kernels are still not known to beat scalar lane-fill in the
real vector-slice harness.

## Current Post-S4-M1233 Bar

The long-term product goal is not complete. The next bar is S4-M1234: pursue
...
while the post-S4-M1233 S4-M1234 bar remains the active follow-up.
