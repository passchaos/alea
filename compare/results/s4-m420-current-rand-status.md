...
S4-M1166 refreshed status output after updating `latest_validate_local_evidence` in
`rand-status-json` to the post-S4-M1165 validate-all evidence. The passing
...
Retained local comparison output:
...
while the post-S4-M1166 S4-M1167 bar continues to block whole-goal completion.
