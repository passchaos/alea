...
S4-M1236 aligns direct-source `nextU64From` and `tryNextU64From` with source-native `nextU64` after S4-M1235
aligned fallible raw-u32 fallback behavior. The retained status and validation evidence include:
...
Wasmtime profilelongcheck run. `compare/results/s4-m1236-next-u64-native-fallback.md` records the latest focused direct-source native-u64 fallback hardening after S4-M1235.
S4-M11 is closed for the current bar; S4-M1220 through S4-M1236 are also closed for their current bars, but exact/default-compatible dense SIMD
normal/exponential kernels are still not known to beat scalar lane-fill in the
real vector-slice harness.

## Current Post-S4-M1236 Bar

The long-term product goal is not complete. The next bar is S4-M1237: pursue
...
while the post-S4-M1236 S4-M1237 bar remains the active follow-up.
