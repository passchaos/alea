...
- S4-M1190 refreshes the full `validate-all` aggregate after S4-M1189;
- S4-M1191 repairs rand-status post-bar and roadmap evidence-map drift;
- S4-M1192 repairs the next rand-status post-bar drift instance;
- S4-M1193 refreshes the local `validate-local` aggregate after those status repairs;
- S4-M1194 adds weighted namespace aliases for local `rand_distr::weighted` discovery names;
- S4-M1195 adds root/prelude weighted sampler aliases;
- S4-M1196 refreshes the local rand weighted manifest after those aliases;
- S4-M1197 exposes public Poisson max-lambda constants matching local `rand_distr::Poisson::MAX_LAMBDA`;
- S4-M1198 refreshes the full `validate-all` aggregate after that public API change;
...
while the post-S4-M1203 S4-M1204 bar remains the active follow-up until S4-M1204 closes status drift.
