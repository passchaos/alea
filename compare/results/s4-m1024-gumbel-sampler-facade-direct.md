# S4-M1024 Gumbel Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `Gumbel` facade sample/fill helpers still routed through
`sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw
open-closed uniforms and apply the Gumbel transform directly; the facade paths
can now do the same through facade `Rng` while preserving collapsed `scale == 0`
point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while Alea's Gumbel
...
- `src/distributions.zig` updates `Gumbel(T).sample` and `Gumbel(T).fill` to draw
  open-closed uniforms directly through facade `Rng` and apply the Gumbel
  transform, with a `scale == 0` no-consume fast path.
- `src/distributions.zig` updates `VectorGumbel(VectorType).sample` and
  `VectorGumbel(VectorType).fill` to draw vector open-closed uniforms directly
  through facade `Rng` and call `gumbelFromOpenClosedUniformVector`, with the
  same point-mass no-consume fast path.
- Direct-source `sampleFrom` / `fillFrom` remain unchanged for explicit
  direct-source workflows and top-level helpers.

## Validation

Focused Gumbel tests:
...
$ zig test src/distributions.zig --test-filter "degenerate cauchy and gumbel helpers do not consume random stream"
1/2 distributions.test.degenerate cauchy and gumbel helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1024 is closed for the current bar: reusable scalar/vector Gumbel facade
