# S4-M604 Root By-Index Weighted Const-Pointer Batch Prevalidation

## Gap

Root `chooseWeightedConstPtrBatchByIndex` and
`chooseWeightedConstPtrBatchByIndexChecked` allocated output buffers before
checking invalid weights and deterministic empty/single by-index weighted-index
states. Those failures should be reported before random-output allocation and
before root secure-engine construction.

This milestone aligns the allocation-returning by-index weighted const-pointer
...
- Single-positive index weights still allocate and fill repeated const pointers
...
S4-M604 is closed for the current bar: root by-index weighted const-pointer batch
