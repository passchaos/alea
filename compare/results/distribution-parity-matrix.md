...
| `Slice::Choose` and fixed-size slice samples | Covered by `Rng.chooseIndex`, `Rng.choose`, `Rng.chooseConstPtr`, `Rng.choosePtr`, `seq.Choice`, `chooseIter`, `chooseArray`, and `sampleArray`; item-slice/emptiness diagnostics plus single, fixed-size value/const-pointer/mutable-pointer arrays, allocated value/const-pointer/mutable-pointer subset slices, caller-owned value/const-pointer/mutable-pointer subset buffers, allocated value/const-pointer/mutable-pointer reservoir samples, caller-owned value/const-pointer/mutable-pointer reservoir buffers, compact `IndexVec` value/const-pointer/mutable-pointer mapping, and fixed-size iterator, caller-owned iterator reservoir exports are available |
...
| WeightedChoice / weighted slice choice | Covered: one-shot `weightedIndex` plus `chooseWeighted` / `chooseWeightedConstPtr` / `chooseWeightedPtr`, allocation-returning `sampleWeighted` / `sampleWeightedPtrs`, compact `sampleWeightedIndexVec`, caller-owned `sampleWeightedIndicesInto` / `sampleWeightedIndicesU32Into` / `sampleWeightedInto` / `sampleWeightedPtrsInto`, fixed-size `sampleWeightedIndexArray` / `sampleWeightedArray` / `sampleWeightedPtrArray` / `sampleIteratorWeightedArray` / `sampleIteratorWeightedInto`, reusable `WeightedChoice`, item/emptiness diagnostics, weight/probability diagnostics and exports, weight updates, single/bulk weight introspection, `chooseIteratorWeighted`, `sampleIteratorWeighted` |
