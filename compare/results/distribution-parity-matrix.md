...
| WeightedAliasIndex | Covered: `AliasTable(Weight)` for O(1) repeated weighted sampling, including `len`, `isEmpty`, `totalWeight`, `weightAt`, `probabilityAt`, deterministic `constantIndex`, compact `sampleU32` / `fillU32` output when populations fit `u32`, owned `indices` / `indicesU32` repeated-sample batches, and allocation-returning or caller-buffer weight/probability reconstruction |
