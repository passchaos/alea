...
| `Slice::Choose` and fixed-size slice samples | Covered by `seq.Choice`, `chooseIter`, `chooseArray`, and `sampleArray`; item-slice/emptiness diagnostics plus single, fixed-size-array, allocated-bulk, caller-owned reservoir, and caller-owned iterator reservoir exports are available |
...
| WeightedChoice / weighted slice choice | Covered: one-shot `chooseWeighted` / `chooseWeightedPtr`, reusable `WeightedChoice`, item/emptiness diagnostics, weight/probability diagnostics and exports, weight updates, single/bulk weight introspection, `chooseIteratorWeighted`, `sampleIteratorWeighted` |
