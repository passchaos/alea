...
| `Uniform` integer/float ranges | Covered by `Uniform(T)`, `Open01`, `OpenClosed01`, scalar/vector range helpers, caller-owned range/strict-interval fills, owned `Rng.rangeBatch` / `vectorRangeBatch` / `openBatch` / `openClosedBatch` batches, endpoint/inclusivity, and range/strict-interval moment accessors |
...
| `Bernoulli` | Covered by `Bernoulli`, `chance`, `ratio`, checked variants, caller-owned and allocation-returning chance/ratio batches, and probability/moment/mode/support accessors |
...
| `Slice::Choose` and fixed-size slice samples | Covered by `Rng.chooseIndex`, `Rng.chooseIndexU32`, `Rng.choose`, `Rng.chooseConstPtr`, `Rng.choosePtr`, `seq.Choice` value/pointer/index samples, fills, and owned value/pointer/index batches, `chooseIter`, `chooseArray`, `sampleArray`, and `sampleArrayU32`; item-slice/emptiness diagnostics plus single, fixed-size value/const-pointer/mutable-pointer arrays, allocated value/const-pointer/mutable-pointer subset slices, caller-owned value/const-pointer/mutable-pointer subset buffers, allocated value/const-pointer/mutable-pointer reservoir samples, caller-owned value/const-pointer/mutable-pointer reservoir buffers, compact `IndexVec` lazy/caller-owned/allocation-returning value/const-pointer/mutable-pointer/u32 export mapping, and fixed-size iterator, caller-owned iterator reservoir exports are available |
