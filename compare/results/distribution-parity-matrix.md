...
| WeightedChoice / weighted slice choice | Covered: one-shot `weightedIndex` / `Rng.weightedIndexU32` / `weightedIndexU32` plus `chooseWeighted` / `chooseWeightedConstPtr` / `chooseWeightedPtr`, accessor-based `chooseWeightedBy` / `chooseWeightedConstPtrBy` / `chooseWeightedPtrBy` and `sampleWeightedIndexArrayBy` / `sampleWeightedIndexArrayU32By` / `sampleWeightedArrayBy` / `sampleWeightedPtrArrayBy` / `sampleWeightedMutPtrArrayBy`, `sampleWeightedIndicesBy` / `sampleWeightedIndicesU32By` / `sampleWeightedIndexVecBy`, `sampleWeightedBy` / `sampleWeightedPtrsBy` / `sampleWeightedMutPtrsBy`, plus caller-owned `sampleWeighted*ByInto` helpers for item-embedded or derived weights, allocation-returning `sampleWeighted` / `sampleWeightedPtrs` / `sampleWeightedIndicesU32`, compact `sampleWeightedIndexVec`, caller-owned `sampleWeightedIndicesInto` / `sampleWeightedIndicesU32Into` / `sampleWeightedInto` / `sampleWeightedPtrsInto`, fixed-size `sampleWeightedIndexArray` / `sampleWeightedIndexArrayU32` / `sampleWeightedArray` / `sampleWeightedPtrArray` / `sampleIteratorWeightedArray` / `sampleIteratorWeightedInto`, reusable accessor-constructible `WeightedChoice` value/pointer/index samples, fills, updates, and owned value/pointer/index batches, item/emptiness diagnostics, weight/probability diagnostics and exports, weight updates, single/bulk weight introspection, `chooseIteratorStable`, `chooseIteratorWeighted`, `sampleIteratorWeighted` |
