...
| `Slice::Choose` and fixed-size slice samples | Covered by `seq.Choice`, `chooseIter`, `chooseArray`, and `sampleArray`; item-slice/emptiness diagnostics plus single, fixed-size-array, allocated-bulk, and caller-owned reservoir exports are available |
