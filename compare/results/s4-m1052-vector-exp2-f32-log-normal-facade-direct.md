# S4-M1052 Vector Exp2 F32 LogNormal Facade Direct Paths

## Gap

Reusable vector `VectorLogNormalExp2F32` facade `sample` / `fill` still routed
through `sampleFrom` / `fillFrom`. The vector exp2-f32 top-level facade helpers
already drive facade `Rng` directly and preserve their snapshot-sensitive output
contract, so the reusable vector sampler facade can call those helpers directly.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while Alea's vector
exp2-f32 log-normal surface is an explicit opt-in throughput/approximation
...
- `src/distributions.zig` updates `VectorLogNormalExp2F32.sample` to call
  `vectorLogNormalExp2F32` through facade `Rng` directly.
- `src/distributions.zig` updates `VectorLogNormalExp2F32.fill` to call
  `fillVectorLogNormalExp2F32` through facade `Rng` directly.
...
S4-M1052 is closed for the current bar: reusable vector exp2-f32 LogNormal facade
