...
S4-M885 draws reusable WeightedChoice scalar value samples through direct
alias-table sampling (`compare/results/s4-m885-weightedchoice-value-sample-direct-table.md`):
...
S4-M887 draws distribution-layer Choose scalar value samples through direct
uniform index generation
(`compare/results/s4-m887-distribution-choose-value-sample-direct-index.md`):
...
S4-M896 routes AliasTable checked index aliases through direct alias-table branches
(`compare/results/s4-m896-aliastable-index-checked-direct.md`):
...
S4-M907 routes AliasTable unchecked index facade aliases through direct
alias-table sampling
(`compare/results/s4-m907-aliastable-index-facade-direct.md`):
...
S4-M911 routes checked facade fills through direct filling
(`compare/results/s4-m911-checked-facade-fill-direct.md`):
Distribution Choose, reusable Choice, and reusable WeightedChoice checked facade
...
S4-M919 routes distribution Choose owned facades through direct allocation and
filling (`compare/results/s4-m919-distribution-choose-owned-facade-direct.md`):
...
S4-M923 routes reusable Choice checked iterator facade constructors directly
(`compare/results/s4-m923-choice-checked-iterator-facade-direct.md`):
Choice pointer/value/index/u32 checked iterator facade helpers now construct
iterators directly instead of routing through direct-source checked iterator
wrappers. This improves reusable checked iterator ergonomics but does not resolve
S4-M11.

S4-M924 routes reusable WeightedChoice checked iterator facade constructors
directly (`compare/results/s4-m924-weightedchoice-checked-iterator-facade-direct.md`):
WeightedChoice pointer/value/index/u32 checked iterator facade helpers now
construct iterators directly instead of routing through direct-source checked
iterator wrappers. This improves reusable weighted checked iterator ergonomics
but does not resolve S4-M11.

S4-M925 routes distribution Choose iterator facade constructors directly
(`compare/results/s4-m925-distribution-choose-iterator-facade-direct.md`):
...
S4-M927 routes reusable WeightedChoice iterator facade constructors directly
(`compare/results/s4-m927-weightedchoice-iterator-facade-direct.md`):
...
S4-M932 routes reusable Choice checked direct-source owned helpers directly
(`compare/results/s4-m932-choice-checked-owned-from-direct.md`):
Choice pointer/value/index/u32 checked direct-source allocation-returning helpers
now allocate and fill directly instead of routing through unchecked direct-source
...
S4-M934 routes distribution Choose checked direct-source fixed-array helpers
directly (`compare/results/s4-m934-distribution-choose-checked-array-from-direct.md`):
Choose pointer/index/u32 checked direct-source fixed-array helpers now construct
...
S4-M936 routes reusable WeightedChoice checked direct-source fixed-array helpers
directly (`compare/results/s4-m936-weightedchoice-checked-array-from-direct.md`):
WeightedChoice pointer/index/u32 checked direct-source fixed-array helpers now
...
S4-M968 routes scalar/vector Binomial facade fills directly
(`compare/results/s4-m968-binomial-fill-facade-direct.md`):
...
S4-M971 routes scalar binomial Poisson-approx facade helpers directly
(`compare/results/s4-m971-binomial-poisson-approx-facade-direct.md`):
`binomialPoissonApprox` and `binomialPoissonApproxChecked` now execute the
...
S4-M984 routes scalar top-level Poisson Ahrens-Dieter facade helpers directly
(`compare/results/s4-m984-poisson-ad-top-level-facade-direct.md`):
...
S4-M992 routes top-level vector Uniform facade helpers directly
(`compare/results/s4-m992-vector-uniform-top-level-facade-direct.md`):
`vectorUniform`, `vectorUniformChecked`, `fillVectorUniform`,
`fillVectorUniformChecked`, `vectorUniformInclusive`,
...
S4-M997 routes reusable VectorGamma facade helpers directly
(`compare/results/s4-m997-vector-gamma-sampler-facade-direct.md`):
`VectorGamma.sample` and `VectorGamma.fill` now dispatch through facade `Rng`
...
S4-M999 routes reusable scalar/vector ChiSquared facade helpers directly
(`compare/results/s4-m999-chi-squared-sampler-facade-direct.md`):
`ChiSquared.sample` / `fill` and `VectorChiSquared.sample` / `fill` now dispatch
...
S4-M1008 routes reusable scalar/vector FisherF facade helpers directly
(`compare/results/s4-m1008-fisher-f-sampler-facade-direct.md`): `FisherF.sample`
/ `fill` and `VectorFisherF.sample` / `fill` now draw cached numerator and
...
S4-M1043 routes reusable scalar LogNormal facade helpers directly
(`compare/results/s4-m1043-log-normal-sampler-facade-direct.md`): `LogNormal.sample`
...
S4-M1050 routes reusable scalar approximate-f32 LogNormal facade helpers directly
(`compare/results/s4-m1050-approx-f32-log-normal-facade-direct.md`):
`LogNormalApproxF32.sample` / `fill` now call approximate-f32 log-normal facade
...
S4-M1054 routes reusable scalar/vector HalfNormal facade helpers directly
(`compare/results/s4-m1054-half-normal-sampler-facade-direct.md`):
`HalfNormal.sample` / `fill` and `VectorHalfNormal.sample` / `fill` now call
...
S4-M1058 routes sequence-layer reusable WeightedChoice owned pointer batches
directly (`compare/results/s4-m1058-seq-weightedchoice-owned-ptrs-facade-direct.md`):
...
S4-M1063 routes sequence-layer reusable WeightedChoice checked u32 index samples
directly (`compare/results/s4-m1063-seq-weightedchoice-checked-u32-index-sample-facade-direct.md`):
`seq.WeightedChoice(T).sampleIndexU32Checked` now calls the AliasTable checked u32
...
S4-M1070 routes sequence-layer reusable WeightedChoice checked u32 index fills
directly (`compare/results/s4-m1070-seq-weightedchoice-checked-u32-index-fill-facade-direct.md`):
`seq.WeightedChoice(T).fillIndicesU32Checked` now calls the AliasTable checked
...
S4-M1074 routes top-level sequence partialShuffleChecked directly
(`compare/results/s4-m1074-seq-partial-shuffle-checked-facade-direct.md`):
...
S4-M1076 routes top-level sequence partialShuffleSplitChecked directly
(`compare/results/s4-m1076-seq-partial-shuffle-split-checked-facade-direct.md`):
`seq.partialShuffleSplitChecked` now validates and calls facade
...
S4-M1078 routes top-level sequence partialShuffleTailChecked directly
(`compare/results/s4-m1078-seq-partial-shuffle-tail-checked-facade-direct.md`):
`seq.partialShuffleTailChecked` now validates and calls facade
`partialShuffleTail` directly instead of routing through
`partialShuffleTailCheckedFrom`. This improves sequence checked tail partial
shuffle facade ergonomics but does not resolve S4-M11.

S4-M1079 routes top-level sequence partialShuffleTailSplit directly
(`compare/results/s4-m1079-seq-partial-shuffle-tail-split-facade-direct.md`):
`seq.partialShuffleTailSplit` now calls facade `partialShuffleTail` directly and
builds the selected/rest split instead of routing through
`partialShuffleTailSplitFrom`. This improves sequence tail partial shuffle split
facade ergonomics but does not resolve S4-M11.

S4-M1080 routes top-level sequence partialShuffleTailSplitChecked directly
(`compare/results/s4-m1080-seq-partial-shuffle-tail-split-checked-facade-direct.md`):
`seq.partialShuffleTailSplitChecked` now validates and calls facade
...
S4-M1084 routes top-level sequence reservoirSamplePtrsInto directly
(`compare/results/s4-m1084-seq-reservoir-ptrs-into-facade-direct.md`):
`seq.reservoirSamplePtrsInto` now validates and performs the const-pointer
reservoir replacement loop through facade `Rng` directly instead of routing
through `reservoirSamplePtrsIntoFrom`. This improves sequence caller-owned
reservoir pointer facade ergonomics but does not resolve S4-M11.

S4-M1085 routes top-level sequence reservoirSamplePtrs directly
(`compare/results/s4-m1085-seq-reservoir-ptrs-facade-direct.md`):
`seq.reservoirSamplePtrs` now allocates and fills through facade
`reservoirSamplePtrsInto` directly instead of routing through
`reservoirSamplePtrsFrom`. This improves sequence allocation-returning reservoir
pointer facade ergonomics but does not resolve S4-M11.

S4-M1086 routes top-level sequence reservoirSamplePtrsChecked directly
(`compare/results/s4-m1086-seq-reservoir-ptrs-checked-facade-direct.md`):
`seq.reservoirSamplePtrsChecked` now validates and calls facade
`reservoirSamplePtrs` directly instead of routing through
`reservoirSamplePtrsCheckedFrom`. This improves sequence checked
allocation-returning reservoir pointer facade ergonomics but does not resolve
S4-M11.

S4-M1087 routes top-level sequence reservoirSampleMutPtrsInto directly
(`compare/results/s4-m1087-seq-reservoir-mut-ptrs-into-facade-direct.md`):
`seq.reservoirSampleMutPtrsInto` now validates and performs the mutable-pointer
reservoir replacement loop through facade `Rng` directly instead of routing
...
S4-M1089 routes top-level sequence reservoirSampleMutPtrsChecked directly
(`compare/results/s4-m1089-seq-reservoir-mut-ptrs-checked-facade-direct.md`):
`seq.reservoirSampleMutPtrsChecked` now validates and calls facade
`reservoirSampleMutPtrs` directly instead of routing through
...
S4-M1092 routes distribution-layer reusable Choose value fills directly
(`compare/results/s4-m1092-distribution-choose-value-fill-facade-direct.md`):
`distributions.Choose(T).fillValues` now maps direct facade-generated indexes
into value output instead of routing through `fillValuesFrom`. This improves
distribution Choose value-fill facade ergonomics but does not resolve S4-M11.

S4-M1093 routes distribution-layer reusable Choose index fills directly
(`compare/results/s4-m1093-distribution-choose-index-fill-facade-direct.md`):
...
S4-M1095 routes reusable UnicodeCharset scalar fills directly
(`compare/results/s4-m1095-unicode-charset-fill-facade-direct.md`):
...
S4-M1098 routes reusable UnicodeCharset scalar sampling directly
(`compare/results/s4-m1098-unicode-charset-sample-facade-direct.md`):
...
S4-M1106 routes reusable UnicodeCharset allocation-returning UTF-8 strings
directly (`compare/results/s4-m1106-unicode-charset-sample-string-facade-direct.md`):
...
S4-M1118 routes checked bounded `Rng` Unicode scalar range helpers directly
(`compare/results/s4-m1118-rng-checked-unicode-scalar-range-facade-direct.md`):
...
S4-M1121 routes checked bounded `Rng` Unicode scalar range fills directly
(`compare/results/s4-m1121-rng-checked-fill-unicode-scalar-range-facade-direct.md`):
...
S4-M1128 closes the adjacent checksum-preserving f64x4 standard-exponential
direct-source fill specialization (`compare/results/s4-m1128-f64x4-standard-exponential-direct-fill.md`): focused vector tests pass, focused
`vectorbench` keeps the same checksum, and direct standard-exponential /
rate-one exponential rows now match or slightly exceed the benchmark-local scalar
helper. This is a narrow performance closure, not whole-goal completion;
S4-M1129 remains active.

S4-M1129 refreshes current status tooling after S4-M1127/S4-M1128 direct-source
...
S4-M1131 refreshes current status after the S4-M1130 validate-all evidence (`compare/results/s4-m1131-post-s4-m1130-rand-status-refresh.md`): `rand-status` now includes S4-M1130 as closed for the current bar and points to S4-M1132. This is status synchronization, not whole-goal completion.

S4-M1132 refreshes f32x8 direct-source vectorbench evidence (`compare/results/s4-m1132-f32x8-direct-source-probe.md`): checksum-preserving f32x8 flat-slice/repair candidates still trail direct exact/default baselines, while faster table/approx-log rows keep different output mappings. This is useful negative evidence, not whole-goal completion; S4-M1133 remains active.

S4-M1133 routes rate-one vector exponential fills through standard-exponential fills (`compare/results/s4-m1133-vector-exponential-rate-one-delegate.md`), aligning implementation with S4-M1128 standard-fill evidence while preserving focused f64x4/f32x8 checksums. This is a narrow exact/default call-shape closure, not whole-goal completion; S4-M1134 remains active.
...
S4-M1153 aligns Triangular non-finite bound semantics with local `rand_distr::Triangular::new` (`compare/results/s4-m1153-triangular-nonfinite.md`): ordered `-inf`/`+inf` bounds and modes are accepted, NaN/unordered parameters remain invalid, and accepted non-finite paths preserve uniform draw shape before producing NaN. This is a local `rand_distr` compatibility closure, not whole-goal completion.

S4-M1154 aligns PERT infinite-shape semantics with local `rand_distr::Pert` (`compare/results/s4-m1154-pert-infinite-shape.md`): finite-range shape-infinity now derives Beta `(inf, inf)`, preserves NaN output and draw shape, rejects endpoint/mean-derived shape-infinity cases, and retains Alea collapsed-range point masses as a documented extension. This is a local `rand_distr` compatibility closure, not whole-goal completion.
...
S4-M1163 aligns AliasTable per-weight maximum validation with local `rand_distr::weighted::WeightedAliasIndex::new` (`compare/results/s4-m1163-alias-table-max-weight.md`): `AliasTable(Weight)` / `WeightedIndex(Weight)` now reject individual weights greater than `maxInt/len` or `floatMax/len` before alias-table scaling, including `updateAt` and `updateMany`, while accepting valid max-boundary tables such as one max weight or two half-max weights. This is a local `rand_distr` compatibility closure, not whole-goal completion.
...
S4-M1176 adds root/prelude weighted error aliases (`compare/results/s4-m1176-root-weighted-error-aliases.md`): root and prelude now expose `WeightedError` and `weightedErrorMessage` for local Rust weighted diagnostics discovery workflows. This is a local Rust diagnostics ergonomics closure, not whole-goal completion.

S4-M1177 refreshes full portability-sensitive validation after S4-M1176 (`compare/results/s4-m1177-post-s4-m1176-validate-all.md`): `zig build validate-all` passes with native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report ending in `profilelongcheck ok`. This is validation evidence, not whole-goal completion.

S4-M1178 refreshes weighted public-surface manifests (`compare/results/s4-m1178-weighted-manifest-refresh.md`): the local `rand` and `rand_distr` manifests now cite recent weighted iterator clone/format, weighted error message, and root/prelude weighted error alias closures. This is audit evidence, not whole-goal completion.

S4-M1179 refreshes local comparison validation after S4-M1178 (`compare/results/s4-m1179-post-s4-m1178-validate-local.md`): `zig build validate-local` passes with native validation, local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, and current status output. This is validation evidence, not whole-goal completion.

S4-M1180 adds typed static weighted diagnostics (`compare/results/s4-m1180-typed-static-weighted-diagnostics.md`): `AliasTable(Weight)` / `WeightedIndex(Weight)` now preserve typed weights and expose `weightValue*`, `weightsValue*`, `totalWeightValue`, and typed iterator helpers matching local Rust `WeightedIndex` typed accessor workflows. This is a local Rust diagnostics ergonomics closure, not whole-goal completion.

S4-M1181 refreshes full portability-sensitive validation after S4-M1180 (`compare/results/s4-m1181-post-s4-m1180-validate-all.md`): `zig build validate-all` passes with native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report ending in `profilelongcheck ok`. This is validation evidence, not whole-goal completion.
...
S4-M1192 repairs the follow-up rand-status post-bar drift (`compare/results/s4-m1192-rand-status-post-bar-drift.md`): status JSON now advances to `S4-M1193 post-S4-M1192 next product bar` without self-reference. This is status/tooling hygiene, not whole-goal completion.

S4-M1193 refreshes local comparison validation after S4-M1192 (`compare/results/s4-m1193-post-s4-m1192-validate-local.md`): `zig build validate-local` passes with native validation, local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, and current status output. This is validation evidence, not whole-goal completion.

S4-M1194 adds weighted namespace aliases (`compare/results/s4-m1194-weighted-namespace-aliases.md`): `distributions.weighted.WeightedAliasIndex`, `WeightedTreeIndex`, and `WeightedIntTreeIndex` now map local `rand_distr::weighted` discovery names onto Alea concrete weighted samplers. This is local Rust public-surface ergonomics evidence, not whole-goal completion.
...
S4-M1202 refreshes f64x4 dense-SIMD vectorbench evidence (`compare/results/s4-m1202-f64x4-vectorbench-refresh.md`): focused standard-normal and standard-exponential f64x4 rows still show no exact/default production replacement for scalar ziggurat lane-fill; table/approx rows remain opt-in or benchmark-only. This is research evidence, not whole-goal completion; S4-M1203 was the next raised product bar and is now closed by the parameterized vectorbench refresh below.

S4-M1203 refreshes parameterized dense-SIMD vectorbench evidence (`compare/results/s4-m1203-parameterized-vectorbench-refresh.md`): focused f32x8/f64x4 normal/exponential rows with affine/rate transforms still show no exact/default production replacement for scalar ziggurat lane-fill; table/approx/native rows remain opt-in or benchmark-only. This is research evidence, not whole-goal completion; S4-M1204 was the next raised product bar and is now closed by the status drift repair below.
...
S4-M1206 adds a roadmap evidence-path guard (`compare/results/s4-m1206-roadmap-evidence-path-guard.md`): roadmapcheck now rejects milestone/evidence path mismatches and the known S4-M1204/S4-M1205 drift is repaired. This is status/guard hygiene, not whole-goal completion; S4-M1207 was the next raised product bar and is now closed by the validate-all refresh below.

S4-M1207 refreshes full validation after S4-M1206 (`compare/results/s4-m1207-post-s4-m1206-validate-all.md`): `zig build validate-all` passed, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report ending with `profilelongcheck ok`. This is validation evidence, not whole-goal completion; S4-M1208 was the next raised product bar and is now closed by the validate-local refresh below.

S4-M1208 refreshes local comparison validation after S4-M1207 (`compare/results/s4-m1208-post-s4-m1207-validate-local.md`): `zig build validate-local` passed, including local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, current status output, and focused profile/stat checks. This is validation evidence, not whole-goal completion; S4-M1209 was the next raised product bar and is now closed by the validate-all refresh below.

S4-M1209 refreshes full validation after S4-M1208 (`compare/results/s4-m1209-post-s4-m1208-validate-all.md`): `zig build validate-all` passed, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report ending with `profilelongcheck ok`. This is validation evidence, not whole-goal completion; S4-M1210 was the next raised product bar and is now closed by the inverse-CDF tail probe below.
...
S4-M1214 refreshes exponential vectorbench evidence (`compare/results/s4-m1214-exponential-vectorbench-probe.md`): approx-log and table-CDF exponential rows beat exact/default direct ziggurat lane-fill, but their checksums differ and their output/distribution contracts are explicit opt-ins rather than default replacements. This is dense-SIMD research evidence, not whole-goal completion; S4-M1215 was the next raised product bar and is now closed by the validate-local refresh below.

S4-M1215 refreshes local comparison validation after S4-M1214 (`compare/results/s4-m1215-post-s4-m1214-validate-local.md`): `zig build validate-local` passed, including local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, current status output, and focused profile/stat checks. This is validation evidence, not whole-goal completion; S4-M1216 was the next raised product bar and is now closed by the validate-all refresh below.

S4-M1216 refreshes full validation after S4-M1215 (`compare/results/s4-m1216-post-s4-m1215-validate-all.md`): `zig build validate-all` passed, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report ending with `profilelongcheck ok`. This is validation evidence, not whole-goal completion; S4-M1217 was the next raised product bar and is now closed by the minimum vectorbench gate below.

S4-M1217 refreshes the minimum real-harness dense-SIMD vectorbench gate (`compare/results/s4-m1217-minimum-vectorbench-gate.md`): the eight focused f32x8/f64x4 standard and parameterized normal/exponential rows still do not identify an exact/default-compatible dense SIMD replacement for scalar ziggurat lane-fill. Table-CDF and approx-log rows remain checksum/output-contract-changing opt-ins. This is dense-SIMD research evidence, not whole-goal completion; S4-M1218 was the next raised product bar and is now closed by the validate-local refresh below.
...
lanes while preserving scalar stream shape. S4-M1226 refactors float fill lane stores
(`compare/results/s4-m1226-float-fill-lane-store-refactor.md`) by deriving vectorized fill widths from the vector type and centralizing lane copies while covering packed f32 and f64 strict-open stream shapes. S4-M1227 refactors distribution lane stores (`compare/results/s4-m1227-distribution-lane-store-refactor.md`) by applying shared vector load/store helpers to staged distribution transforms. S4-M1228 completes the remaining distribution transform lane-store sweep (`compare/results/s4-m1228-distribution-transform-lane-store-sweep.md`) across inverse-Gaussian and uniform-transform families. S4-M1229 completes the vector sampler lane-count sweep (`compare/results/s4-m1229-vector-sampler-lane-count-sweep.md`) by routing remaining vector wrappers through `vectorInfo(VectorType).len`. S4-M1230 refactors the rng normal affine lane-store helper (`compare/results/s4-m1230-rng-normal-affine-lane-store.md`) through the shared core lane helpers. S4-M1231 hardens rng byte-fill / reader refill correctness (`compare/results/s4-m1231-rng-reader-tryfill-hardening.md`) by sharing fallible byte-fill fallback behavior and covering buffered reader refills. S4-M1232 hardens owned byte allocation fallbacks (`compare/results/s4-m1232-owned-byte-allocation-fallback.md`) by routing `bytesAllocFrom` through the shared fallible byte-fill path. S4-M1233 fixes `std.Random` adapter `nextU32` byte-stream shape (`compare/results/s4-m1233-fromrandom-nextu32-byte-shape.md`) by matching native `std.Random.int(u32)` byte consumption. S4-M1234 hardens root alphanumeric string preallocation (`compare/results/s4-m1234-root-string-preallocation.md`) by allocating/reserving before secure entropy and preserving visible list contents on entropy failure. S4-M1235 aligns fallible direct-source raw-u32 fallback behavior (`compare/results/s4-m1235-try-next-u32-native-fallback.md`) by preserving source-native `nextU32` stream shape when `tryNextU32` is absent. S4-M1236 aligns direct-source native-u64 fallback behavior (`compare/results/s4-m1236-next-u64-native-fallback.md`) by preserving source-native `nextU64` for facade raw helpers. S4-M1237 aligns direct-source byte-helper fallback behavior (`compare/results/s4-m1237-fillbytes-direct-source-fallback.md`) by preserving source-native `fillBytes` and native-u64 fallbacks for byte helpers. S4-M1238 aligns generic direct-source native-u64 fallback behavior (`compare/results/s4-m1238-nextfrom-native-u64-fallback.md`) by preserving source-native `nextU64` across generic infallible helpers. S4-M1239 aligns seed/fork native-u64 fallback behavior (`compare/results/s4-m1239-fromrng-native-u64-fallback.md`) by preserving source-native `nextU64` / `tryNextU64` across `fromRng` workflows. S4-M1240 aligns `Rng.init` raw-alias source fallback behavior (`compare/results/s4-m1240-rng-init-raw-alias-source.md`) by wrapping `nextU64` / `fillBytes`-only sources through shared facade vtable dispatch. S4-M1241 aligns nextU32-only direct-source fallback behavior (`compare/results/s4-m1241-next-u32-only-source-fallback.md`) by widening native u32 draws for u64-shaped raw/facade helpers. S4-M1242 is closed by the first concrete post-S4-M1241 improvement: S4-M1243 fixes stable iterator choice semantics (`compare/results/s4-m1243-stable-iterator-choice-semantics.md`) so `chooseIteratorStable` always uses sequential reservoir sampling independent of iterator size hints, matching local Rust `choose_stable` reproducibility semantics. S4-M1244 remains active for
...
S4-M1244 completes the polymorphic unit-struct sampler surface for parameter-free
distributions (`compare/results/s4-m1244-polymorphic-standard-normal-exponential.md`):
`StandardNormal{}` and `StandardExponential{}` (with `Exp1{}` Rust-naming alias)
now follow the same polymorphic unit-struct pattern as `StandardUniform`, `Open01`,
...
S4-M1251 adds a Von Mises-Fisher (vMF) spherical directional distribution on
the n-dimensional unit sphere Sⁿ⁻¹ for comptime n≥2
(`compare/results/s4-m1251-von-mises-fisher-spherical-distribution.md`):
`VonMisesFisher(T, n)` is parameterized by a unit mean direction μ and
concentration κ≥0. For n=2 (circle S¹) sampling delegates to the existing
Von Mises angle sampler then converts (cosθ,sinθ) to a unit vector. For n≥3
Wood (1994) Ulrich-transform rejection sampling is used: constants
(b,x0,c) = `vmfWoodConstants(κ,dim)` are computed once at init; w is drawn
via Beta rejection (shape (dim-2)/2, (dim-2)/2 for the cosine marginal); a
uniform point on Sⁿ⁻² is drawn (normal variates normalized) for the
tangent-plane component; the sample is assembled in the e1-centered frame as
(w, sqrt(1-w²)·v) and rotated to the mean direction μ via a Householder
reflection H = I − 2vvᵀ/(vᵀv) where v = e1 − μ (handles ±e1 near-degenerate
cases with a 1e-12 tolerance). Fast paths: κ=0 delegates to
`unitSphereSurfaceFrom` for uniform sampling; κ>1e12 returns the mean
direction as a point mass. Statistical accessors: `meanResultantLength` is
exact for n=2 (Bessel ratio I₁(κ)/I₀(κ)) and n=3 (coth κ − 1/κ) and uses a
first-order continued-fraction approximation for higher dimensions;
`expectedValue` returns ρ·μ and `varianceValue` returns 1 − ρ² (scalar
concentration). A complete scalar/fill free-function family includes
`vonMisesFisher{,From,Checked,CheckedFrom}` and
`fillVonMisesFisher{,From,Checked,CheckedFrom}`; `initNormalized` skips
unit-norm validation for caller-guaranteed unit directions;
`VonMisesFisherError` aliases `error.InvalidParameter` (κ<0) and
`error.InvalidMeanDirection` (μ not unit). Nine tests cover constructor
validation, 2D concentration, 3D unit-norm and sphere-surface properties,
κ=0 uniformity (mean resultant length <0.05), high-κ degeneracy (collapses
to μ), free-function/fill bulk entry points, Householder arbitrary-mean
correctness (mean direction matches μ within 0.05 at κ=50), 4D unit vectors,
and f32 support. All 642 tests pass; `zig build validate` passes all gates.

S4-M1252 adds a Watson axial spherical directional distribution on the
n-dimensional unit sphere Sⁿ⁻¹ for comptime n≥2
(`compare/results/s4-m1252-watson-axial-spherical-distribution.md`):
`Watson(T, n)` is parameterized by a unit mean axis μ and concentration κ
(κ>0 bipolar concentration around ±μ, κ<0 girdle concentration around the
equator perpendicular to μ, κ=0 uniform sphere). The axial Watson distribution
is antipodally symmetric (x↔−x), so the signed first moment E[x] is identically
zero; `meanResultantLength()` returns the meaningful axial concentration
statistic `meanCosineSquared() = E[(μᵀx)²]` which ranges from 0 (girdle limit)
through 1/n (uniform) to 1 (bipolar point mass). Sampling uses
Beta((n-1)/2,(n-1)/2) rejection for the cosine w coordinate with log-ratio
envelope tests (`κ·(w²−1)` bipolar, `κ·w²` girdle) and a uniform Sⁿ⁻² tangent
point; the Householder reflection is factored into a shared
`householderReflectFromE1` helper reused from VonMises-Fisher. Fast paths:
κ=0 delegates to `unitSphereSurfaceFrom`, κ>1e12 returns ±μ via a fair coin
flip (axial bipolar degeneracy), κ<−1e12 samples Sⁿ⁻² uniformly in the
equatorial tangent plane (girdle degeneracy). Mean cosine-squared moments use
a truncated Kummer confluent hypergeometric ₁F₁ power series (up to 60 terms,
break at term<1e-15·sum) for moderate |κ|, with asymptotic expansions
1−(n−1)/(2κ) for κ→+∞ and (n−1)/(−2κ) (1/(−2κ) for n=2) for κ→−∞ to avoid
series divergence at large |z|. The standard free-function family is provided:
`watson`, `watsonFrom`, `watsonChecked`, `watsonCheckedFrom`,
`fillWatson`, `fillWatsonFrom`, `fillWatsonChecked`, `fillWatsonCheckedFrom`;
`initNormalized` skips unit-norm validation for caller-guaranteed unit axes;
`WatsonError` aliases `error.InvalidParameter` and `error.InvalidMeanDirection`.
Ten tests cover constructor validation, 3D bipolar concentration
(mean cos²>0.9 at κ=20), κ=0 uniformity (mean cos²≈1/3 within 2σ), girdle
concentration near equator (|cos θ|<0.1 for 99% of samples at κ=-20), high-κ
bipolar ±μ degeneracy, free-function/fill bulk paths, 4D unit vectors, 2D circle
...
S4-M1254 (`compare/results/s4-m1254-nakagami-m-fading-distribution.md`) adds a Nakagami-m distribution, the canonical multipath fading
envelope model for wireless/RF communications. Nakagami-m fills a practical
gap in continuous fading-family distributions alongside Rayleigh (m=1) and
Rice (which it approximates for m > 1):

- Shape parameter m ≥ 0.5 controls fading severity: m = 0.5 gives the
  half-normal (one-sided Gaussian, "worse-than-Rayleigh" severe fading),
  m = 1 exactly recovers Rayleigh(√(Ω/2)), m > 1 approximates Rician
  fading with an equivalent K-factor, and m → ∞ converges to a Gaussian
  concentrated near √Ω with variance Ω/(4m).
- Spread parameter Ω = E[X²] > 0 is the second moment, matching the
  second-moment parameterization used across RF/wireless literature.

Sampling is rejection-free via X = √(Gamma(m, Ω/m)) directly leveraging the
existing Gamma sampler — no new special functions are required beyond
`std.math.lgamma` for moment computation (already used throughout alea for
Student-t, Fisher-F, Beta, and Chi-squared). Moment formulas use the log-gamma
ratio exp(lgamma(m+1/2) − lgamma(m)) to avoid overflow, with exact fast paths
for the m=1 Rayleigh case:

- Expected value: (Γ(m+½)/Γ(m))·√(Ω/m), or √(πΩ)/2 when m=1.
- Variance: Ω·(1 − (Γ(m+½)/Γ(m))²/m), or Ω·(4−π)/4 when m=1.
- Mode: √(Ω·(m−1)/m) for m ≥ 1 (0 for m < 1), or √(Ω/2) when m=1.
- equivalentRayleighScale: √(Ω/2) for comparison with Rayleigh/Rice σ.

Fast paths handle Ω = 0 (point mass at 0) and m=1 (exact Rayleigh moments).
The standard 8-function free-function family (nakagami/nakagamiFrom/
nakagamiChecked/nakagamiCheckedFrom/fillNakagami/fillNakagamiFrom/
fillNakagamiChecked/fillNakagamiCheckedFrom) is provided alongside the
`Nakagami(T)` struct with init/new/accessors/sample/sampleFrom/fill/fillFrom.

Tests cover constructor validation (m < 0.5 rejected, Ω < 0 rejected, NaN/Inf
rejected, m=0.5 boundary accepted), m=1 Rayleigh moment equivalence (mean,
variance, mode, equivalentRayleighScale all match), Ω=0 point mass at 0,
non-negativity and finiteness of 1200 samples across shape values
m ∈ {0.5, 0.75, 1, 2, 5, 10}, m=0.5 half-normal moment correctness with
5000-sample Monte Carlo verification (mean ≈ √(2/π) within 0.04, variance
≈ 1−2/π within 0.05), large-m concentration at m=50, Ω=4 (mean ≈ 2 within
0.01, variance < 0.03, mode ≈ 2 within 0.03, Monte Carlo mean within 0.05),
free-function/fill correctness with error rejection, and f32 support.
All 668 tests pass; `zig build validate` passes all gates.

The next product bar after S4-M1254 covers noncentral chi/chi-squared,
Inverse Gamma, Exponentially Modified Gaussian, Generalized Pareto,
Hoyt (Nakagami-q) fading, additional directional/multivariate distributions
(Bingham, Matrix von Mises-Fisher, Kent), Rice/Nakagami vector/SIMD sampling,
...
S4-M1255 (`compare/results/s4-m1255-inverse-gamma-distribution.md`) adds an
Inverse Gamma distribution on (0,∞) with shape α > 0 and scale β > 0. If
X ~ Gamma(α, θ) with shape α and scale θ (alea's parameterization), then
1/X ~ InverseGamma(α, 1/θ). The Inverse Gamma PDF is

    f(x; α, β) = β^α / Γ(α) · x^{−(α+1)} · exp(−β/x)

Inverse Gamma is the canonical conjugate prior for the variance parameter of
a normal distribution in Bayesian statistics, and is the workhorse prior for
noise variance in regression, Gaussian processes, hierarchical models, MCMC,
and finance (inverse volatility priors). Scaled-inverse-χ²(ν, s²), the BUGS/
Stan parameterization, is exactly InverseGamma(ν/2, ν·s²/2).

Sampling is rejection-free via the reciprocal-Gamma identity: X = 1/Gamma(α,
1/β) using the existing `gammaFrom` helper. Statistical accessors:

- `expectedValue()`: β/(α−1) for α > 1; +∞ for α ≤ 1 (mean does not exist).
- `varianceValue()`: β²/((α−1)²(α−2)) for α > 2; +∞ for α ≤ 2 (variance
  does not exist).
- `modeValue()`: β/(α+1) for all α > 0 (always exists).
- `minValue()`: 0; `maxValue()`: null (∞).

The standard 8-function free-function family (inverseGamma/inverseGammaFrom/
inverseGammaChecked/inverseGammaCheckedFrom/fillInverseGamma/
fillInverseGammaFrom/fillInverseGammaChecked/fillInverseGammaCheckedFrom)
is provided alongside the `InverseGamma(T)` struct.

Tests cover constructor validation (α ≤ 0 and β ≤ 0 rejected, NaN/Inf
rejected), analytic moments for two parameter sets (IG(3,2): mean=1,
variance=1, mode=0.5; IG(5,4): mean=1, variance=1/3, mode=2/3), infinite
mean/variance for small shape (α=0.5 both infinite, α=1.5 finite mean /
infinite variance), positivity/finiteness of 500 samples, reciprocal-Gamma
identity via 2000-sample Monte Carlo (E[1/X] ≈ α/β = 1.5 within 0.1),
5000-sample MC mean/variance verification for IG(5,4) within sampling
tolerance, free-function/fill correctness with error rejection, and f32
support. All 676 tests pass; `zig build validate` passes all gates.

The next product bar after S4-M1255 covers noncentral chi/chi-squared,
Exponentially Modified Gaussian (ExGaussian), Generalized Pareto, Scaled
Inverse Chi-Squared (parameterization adapter), Hoyt (Nakagami-q) fading,
additional directional/multivariate distributions (Bingham, Matrix von
Mises-Fisher, Kent), Rice/Nakagami/InverseGamma vector/SIMD sampling,
copula methods, SIMD spherical bulk sampling paths, weighted/alias
sampling advances, string generation expansion, longer statistical
...
S4-M1256 (`compare/results/s4-m1256-exgaussian-distribution.md`) adds an
Exponentially Modified Gaussian distribution — the convolution X = μ + σ·Z + E
where Z ∼ N(0,1) and E ∼ Exp(1/τ) with rate λ = 1/τ. ExGaussian is the
canonical model for reaction-time (RT) distributions in cognitive psychology,
neuroscience, and psycholinguistics, where RTs are theoretically decomposed
into a Gaussian decision/encoding component plus an exponentially distributed
residual or tail component (e.g., residual perceptual processing, response
execution, or attentional lapses). It is also used in chromatography (peak
shapes), queuing theory, and finance for skewed, positive-tailed distributions
that are otherwise nearly Gaussian.

The PDF is

    f(x; μ, σ, τ) = (1/τ) · exp((μ − x)/τ + σ²/(2τ²)) · Φ((x − μ)/σ − σ/τ)

where Φ is the standard normal CDF. Rust `rand_distr 0.6.0` does not provide
ExGaussian directly; this adds a compositional distribution beyond the local
Rust surface.

Sampling is rejection-free by the defining convolution: one standard normal
draw plus one exponential draw, summed with affine scaling. No custom
rejection or approximation is required. Statistical accessors:

- `expectedValue()`: μ + τ.
- `varianceValue()`: σ² + τ².
- `standardDeviationValue()`: √(σ² + τ²).
- `skewnessValue()`: 2τ³/(σ² + τ²)^{3/2} (always positive — right skew).
- `modeValue()`: μ (returned as a stable reference; exact mode is a
  transcendental root requiring numerical root-finding, which is intentionally
  avoided in favor of a documented, deterministic anchor).
- `rateValue()`: 1/τ (exponential rate parameter for rate-parameterization users).
- `locationValue()`/`muValue()`: μ; `gaussianScaleValue()`/`sigmaValue()`: σ;
  `exponentialMeanValue()`/`tauValue()`: τ.
- `minValue()`: −∞ (no lower bound; X can take arbitrarily negative values
  because of the Gaussian component); `maxValue()`: null (∞).
- σ = 0 is accepted and produces a shifted exponential: X = μ + Exp(1/τ)
  on (μ, ∞).
- τ > 0 is required; τ = 0 or τ < 0 is rejected.

The standard 8-function free-function family (exGaussian/exGaussianFrom/
exGaussianChecked/exGaussianCheckedFrom/fillExGaussian/fillExGaussianFrom/
fillExGaussianChecked/fillExGaussianCheckedFrom) is provided alongside the
`ExGaussian(T)` struct, plus the Checked free-function variants that return
a typed error union. A bulk `fillFrom` path fills a slice of arbitrary length
with independent draws from a custom entropy source.

Tests cover constructor validation (τ ≤ 0 rejected, NaN/Inf rejected, σ ≥ 0
accepted including σ = 0), analytic moments for three parameter sets
(μ=0,σ=1,τ=1: mean=1, var=2, sd=√2, skew=2/(2√2)=1/√2≈0.707; μ=10,σ=2,τ=3:
mean=13, var=13, sd=√13≈3.606; μ=0,σ=0,τ=2: mean=2, var=4, sd=2, skew=1),
positivity-of-tail via 500 samples at τ≫σ (skew and >μ proportion verified),
Monte Carlo mean/variance on 8000 samples within tolerance, σ=0 shifted-
exponential verification (all samples ≥ μ), free-function/fill correctness
with error rejection, and f32 support. All 684 tests pass; `zig build validate`
passes all gates.

The next product bar after S4-M1256 covers noncentral chi/chi-squared,
Generalized Pareto, Scaled Inverse Chi-Squared (parameterization adapter),
Hoyt (Nakagami-q) fading, additional directional/multivariate distributions
(Bingham, Matrix von Mises-Fisher, Kent), Rice/Nakagami/InverseGamma/ExGaussian
vector/SIMD sampling, copula methods, SIMD spherical bulk sampling paths,
...
S4-M1257 (`compare/results/s4-m1257-generalized-pareto-distribution.md`) adds a
Generalized Pareto Distribution (GPD) on (μ, ∞) for ξ ≥ 0 or [μ, μ − σ/ξ] for
ξ < 0, with location μ ∈ ℝ, scale σ > 0, and shape ξ (tail index). GPD is the
canonical peaks-over-threshold (POT) model in extreme value theory and is used
pervasively in hydrology (flood/drought exceedances), finance (value-at-risk,
tail risk), insurance (large-loss modeling), and environmental risk assessment.
GPD unifies three continuous tail regimes:

- **ξ = 0** (Gumbel domain): reduces identically to the shifted Exponential(μ, σ)
  distribution with mean μ+σ and variance σ².
- **ξ > 0** (Fréchet / heavy tail): Pareto-like polynomial decay; support
  extends to +∞. Mean exists only for ξ < 1; variance exists only for ξ < 1/2.
  Classical Pareto (Type I) is a location/scale shift of GPD with ξ = 1/α.
- **ξ < 0** (Weibull / bounded short tail): bounded support with finite upper
  endpoint at μ − σ/ξ; includes the Uniform(μ, μ+σ) distribution at ξ = −1.

The PDF is f(x; μ, σ, ξ) = (1/σ)(1 + ξ(x−μ)/σ)^{−1/ξ−1} for ξ ≠ 0, and
f(x; μ, σ, 0) = (1/σ)exp(−(x−μ)/σ) for ξ = 0. The inverse CDF is closed-form,
yielding rejection-free O(1) sampling: X = μ + σ·(U^{−ξ} − 1)/ξ for ξ ≠ 0
and X = μ − σ·ln(U) for ξ = 0 where U ∈ (0,1].

Statistical accessors:

- `expectedValue()`: μ + σ/(1−ξ) for ξ < 1; null (+∞ infinite mean) for ξ ≥ 1.
...
- `modeValue()`: μ for all ξ (PDF is monotone decreasing from a finite
  maximum at μ, as is true for all three GPD regimes).
- `minValue()`: μ; `maxValue()`: μ − σ/ξ for ξ < 0, else null (∞).
- σ = 0 accepted as a degenerate point mass at μ.

The standard 8-function free-function family is provided. Tests cover
constructor validation (σ < 0 rejected, NaN/Inf rejected), analytic moments
across all three regimes (ξ=0 exponential, ξ=0.2 heavy with finite variance,
ξ=0.6 heavy with infinite variance, ξ=1.5 infinite mean, ξ=−0.25 bounded with
upper bound 4μ+4σ), support containment for 200 samples per regime, 5000-sample
Monte Carlo mean/variance for ξ=0 within sampling tolerance, free-function/fill
correctness with error rejection, and f32 support. All 702 tests pass (684 +
18 new across S4-M1257/M1258/M1259); `zig build validate` passes all gates.

### S4-M1258 — Scaled Inverse Chi-Squared distribution

S4-M1258 (`compare/results/s4-m1258-scaled-inverse-chi-squared-distribution.md`)
adds a Scaled Inverse Chi-Squared distribution on (0, ∞) with degrees of freedom
ν > 0 and squared scale τ² > 0, the Gelman BDA/BUGS/Stan parameterization of the
conjugate prior for a normal variance:

    σ² ~ ScaledInvChiSq(ν, τ²)   ⟺   σ² ~ InverseGamma(ν/2, ντ²/2)

where the second argument is the scale parameter in alea's existing
InverseGamma shape-scale parameterization. Providing ScaledInvChiSq as a
first-class distribution avoids forcing Bayesian-statistics users to hand-apply
the α = ν/2, β = ντ²/2 mapping. Sampling is trivial delegation to the existing
`inverseGammaPointFrom` helper — no new algorithms or special functions.

PDF: f(x; ν, τ²) = (ντ²/2)^{ν/2}/Γ(ν/2) · x^{−(ν/2+1)} · exp(−ντ²/(2x)).

Statistical accessors:

- `expectedValue()`: ντ²/(ν−2) for ν > 2; null (∞) for ν ≤ 2.
- `varianceValue()`: 2ν²τ⁴/((ν−2)²(ν−4)) for ν > 4; null for ν ≤ 4.
- `modeValue()`: ντ²/(ν+2) for all ν > 0.
- `scaleValue()`: τ = √(τ²); `scaleSquaredValue()`: τ².
- `minValue()`: 0; `maxValue()`: null (∞).

Tests cover constructor validation (ν ≤ 0 rejected, τ² ≤ 0 rejected, NaN/Inf
rejected), analytic moments for (ν=5, τ²=1): mean=5/3, variance=50/9,
mode=5/7; (ν=3, τ²=4): mean=12, infinite variance; ν=2: infinite mean;
positivity of 200 samples; 5000-sample Monte Carlo mean for (ν=10, τ²=1) within
sampling tolerance; free-function/fill correctness with error rejection; and
f32 support. ScaledInvChiSq is absent from local Rust `rand_distr`; the only
prior coverage was via hand-composed InverseGamma.

### S4-M1259 — Hoyt (Nakagami-q) fading distribution

S4-M1259 (`compare/results/s4-m1259-hoyt-nakagami-q-fading-distribution.md`) adds
a Hoyt (Nakagami-q) fading distribution on [0, ∞) with fading parameter
q ∈ (0,1] and spread Ω = E[X²] > 0. Hoyt is the canonical complementary fading
model to Nakagami-m for multipath wireless/RF channels where the scattered
components have unequal power (e.g., one strong specular path plus weaker
isotropic scatter). The parameter limits are exact: q = 1 recovers
Rayleigh(σ=√(Ω/2)) identically, while q → 0 converges to the one-sided
(half) Gaussian near-severe fading regime.

Sampling is rejection-free via the defining two-Gaussian product form:
X = σ·√(Z₁² + (q·Z₂)²) for Z₁,Z₂ ~ N(0,1) i.i.d. and σ = √(Ω/(1+q²)),
requiring exactly two standard normal draws per sample. This also adds a
standalone `besselI1(z)` implementation (modified Bessel function of the
first kind, order 1) using Abramowitz & Stegun polynomial/asymptotic
approximations with ~1e-7 relative accuracy, complementing the existing
`besselI0(z)` for use in Hoyt's mean formula and future Bessel-family
distributions.

Statistical accessors use closed Bessel forms:

- `expectedValue()`: σ·√(π/2)·(1+q)/2·exp(K)·(I₀(K) + I₁(K)) with
  K = (1−q)²/(4q). Verified exact at q=1 (Rayleigh limit): σ·√(π/2).
- `varianceValue()`: Ω − (E[X])² (second moment is Ω by definition).
- `modeValue()`: σ·q (exact at q=1: mode = σ ✓, matching Rayleigh).
- `sigmaValue()`: σ = √(Ω/(1+q²)).
- `minValue()`: 0; `maxValue()`: null (∞); Ω=0 degenerate point mass at 0.

Tests cover constructor validation (q ≤ 0, q > 1, Ω < 0, NaN/Inf all rejected),
exact q=1 Rayleigh moment equivalence (mean, variance, mode, sigmaValue),
non-negativity/finiteness across q ∈ {0.2, 0.5, 0.8, 1.0} for 200 samples each,
Ω=0 degenerate point mass at 0, 5000-sample Monte Carlo for q=1 (Rayleigh)
verifying mean ≈ √(π/2) within 0.06 and second moment ≈ Ω=2 within 0.1,
free-function/fill correctness with error rejection, and f32 support.

All three distributions (Generalized Pareto, Scaled Inverse Chi-Squared, Hoyt)
extend beyond local Rust `rand_distr` 0.6.0, which provides neither GPD nor
