...
| WeightedTreeIndex | `WeightedTree(Weight)`, `WeightedIntTree(Weight)`, plus `initByIndex` / `updateAllByIndex` index-weight accessor helpers | unit tests, Zig/Rust update+sample benchmark rows for integer and f64 weights |
...
index-weighted reusable `WeightedChoice.initByIndex` / `updateByIndex`, and S4-M147 adds
index-weighted dynamic `WeightedTree` / `WeightedIntTree` construction and refresh, but later stages should
