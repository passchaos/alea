...
| WeightedTreeIndex | `WeightedTree(Weight)`, `WeightedIntTree(Weight)`, plus `initBy` / `updateAllBy` item-accessor helpers, `initByIndex` / `updateAllByIndex` index-weight accessor helpers, and compact `sampleU32` / `fillU32` output | unit tests, Zig/Rust update+sample benchmark rows for integer and f64 weights |
...
compact `u32` dynamic tree sample/fill output, but later stages should
