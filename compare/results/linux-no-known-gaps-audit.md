...
| WeightedTreeIndex | `WeightedTree(Weight)`, `WeightedIntTree(Weight)`, plus `initBy` / `updateAllBy` item-accessor helpers and `initByIndex` / `updateAllByIndex` index-weight accessor helpers | unit tests, Zig/Rust update+sample benchmark rows for integer and f64 weights |
...
index-weighted dynamic `WeightedTree` / `WeightedIntTree` construction and refresh, and S4-M148 adds
item-accessor dynamic `WeightedTree` / `WeightedIntTree` construction and refresh, but later stages should
