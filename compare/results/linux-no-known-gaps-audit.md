...
index/u32-index batches, S4-M151 adds `sampleIndex` / `fillIndices` dynamic tree aliases, S4-M152 adds repeated dynamic tree index/u32-index iterators, S4-M153 adds compact `u32` `AliasTable` sample/fill output, and S4-M154 adds owned repeated `AliasTable` index/u32-index batches, but later stages should
