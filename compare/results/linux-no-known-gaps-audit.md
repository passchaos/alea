...
| WeightedAliasIndex | `AliasTable(Weight)`, including `sampleIndex` / `fillIndices` aliases, compact `sampleU32` / `fillU32` output, and owned `indices` / `indicesU32` batches | unit tests, benchmark rows for weighted index paths; f32, f64, and u32 `WeightedAliasIndex` rows are exceeded by `AliasTable` evidence |
