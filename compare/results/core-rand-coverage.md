...
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `ScalarPrng` reaches about 446-449M standard normal, about 423M standard exponential, about 68M direct Poisson fill, about 157M cached Gamma, about 121-122M direct LogNormal, and direct-source bulk fills for core discrete, compound, weighted, collection, and multivariate workflows; staged/vectorized LogNormal bulk fill now reaches about 127M facade fill and about 133M direct-source fill; vectorized InverseGaussian bulk fill now reaches about 217M facade and about 238M direct-source fill, and staged NormalInverseGaussian bulk fill reaches about 143M facade and about 159M direct-source fill; Geometric failures and StandardGeometric now cover local `rand_distr` semantics, Hypergeometric HIN closes the benchmarked `(100, 30, 10)` gap, and the H2PE production path closes the benchmarked large `(5000, 2500, 500)` gap with a `distcheck` mean gate; vector-slice normal/exponential rows are about 497M normal f32x8, 497M normal f64x4, 449M exponential f32x8, and 448M exponential f64x4 lanes/s; `fillOpenClosed(f64)` bulk is now about 778M samples/s after the 128-word raw buffer follow-up, at the local Rust evidence noise boundary; strict-interval, reusable-sampler, weighted, collection, and multivariate batch APIs have direct-source/bulk coverage; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
