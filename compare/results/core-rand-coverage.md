...
| S4-M3 | SIMD/vector sampling design | Design and prototype Zig-native vector/SIMD sampling APIs for high-volume scalar distributions without copying Rust `std::simd` surface shapes. | In progress: `Rng` now has Zig-native vector APIs, direct/static vector helpers, direct/static vector fill helpers, packed-lane prototypes for bool/integer/`f32` vectors, vectorized `f32` and `f64` uniform/open/open-closed helpers and bulk fills, exponent-bit f64 uniform/open/range generation, direct-source scalar/vector bulk `fillFrom` / `fillRangeFrom`, single-pass vector-scaled float range fills, vector-slice fill, vector range/chance/ratio/normal/exponential fills, vector chance/ratio/range/normal/exponential APIs, vector-slice chance/ratio fill APIs, scalar-ziggurat single-vector and vector-slice normal/exponential fills, and a dedicated `vectorbench` step. Latest local vector rows are about 484M normal f32x8, 486M normal f64x4, 436M exponential f32x8, and 437M exponential f64x4 lanes/s; deeper true-SIMD distribution kernels are still open. |
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `ScalarPrng` reaches about 440M standard normal, 416M standard exponential, 68M direct Poisson fill, 157M cached Gamma, about 122M direct LogNormal, and direct-source bulk fills for core discrete and compound distributions; Geometric failures and StandardGeometric now cover local `rand_distr` semantics, and Hypergeometric HIN closes the benchmarked `(100, 30, 10)` gap, while large-parameter H2PE-equivalent Hypergeometric remains open, has an isolated `hypergeo-h2pe-probe` harness with a 24.8M-sample/s production candidate, and is covered by a large-parameter `distcheck` mean gate; single-vector normal/exponential rows improved to about 432M and 393M lanes/s; `fillSampleFrom` improves direct reusable bulk sampling; cached ChiSquared/Beta/Fisher/StudentT and inverse-Gaussian-family sampler paths are faster than their single-shot equivalents; alphanumeric generation exceeds current local Rust evidence; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
