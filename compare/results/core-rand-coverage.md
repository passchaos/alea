...
| S4-M2 | Longer external statistical validation | Run and check in longer PractRand/TestU01-compatible evidence beyond the current local 64GiB engine reports for the primary Linux engines. | In progress: `fast` / `Alea4x64`, `wyhash64`, and `xoshiro256++` have clean 128GiB PractRand passes; `default` / `Xoshiro256` and `pcg64` each have one default-seed `unusual` at 128GiB plus a clean alternate-seed rerun; remaining primary engines still need longer-than-64GiB evidence |
