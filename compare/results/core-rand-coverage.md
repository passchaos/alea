...
| S4-M3 | SIMD/vector sampling design | Design and prototype Zig-native vector/SIMD sampling APIs for high-volume scalar distributions without copying Rust `std::simd` surface shapes. | In progress: `Rng` now has Zig-native vector APIs, direct/static vector helpers, packed-lane prototypes for bool/integer/`f32` vectors, vector-slice fill, vector range/normal/exponential fills, and vector-math `f32` normal/exponential kernels; deeper SIMD-optimized distribution kernels are still open |
