...
| Distribution algorithms | Normal samplers cache paired Box-Muller output, exponential samplers precompute inverse rate, binomial has exact small-n, p=0.5, and large-n rejection paths, and an explicit Poisson approximation helper exists for large sparse binomials. Ziggurat-style normal/exponential algorithms remain future work. | Low |
