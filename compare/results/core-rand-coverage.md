...
| S4-M3 | SIMD/vector sampling design | Design and prototype Zig-native vector/SIMD sampling APIs for high-volume scalar distributions without copying Rust `std::simd` surface shapes. | In progress: `Rng` now has Zig-native vector APIs, direct/static vector helpers, direct/static vector fill helpers, packed-lane prototypes for bool/integer/`f32` vectors, vector-slice fill, vector range/normal/exponential fills, and vector-math `f32` normal/exponential kernels; deeper SIMD-optimized distribution kernels are still open |
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: medium-lambda Poisson now uses an Ahrens-Dieter rejection path and reaches about 52M samples/s at `lambda=20`, up from about 26M; normal direct benchmarking shows the remaining normal gap is in the stdlib ziggurat path, not Alea facade overhead; bulk `fillRange`, typed integer fill, packed `f32` and `f32` range fill, and bool fill APIs now have native benchmark rows; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
...
| Performance | Fast paths should be competitive with or faster than local Rust evidence for comparable workloads, with facade/direct overhead separated. | Follow `compare/results/performance-triage.md`; recent work improved ziggurat facade distributions and packed integer/f32/bool fill paths. |
