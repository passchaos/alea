...
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `ScalarPrng` reaches about 427M normal, 426M exponential, and 76M Poisson samples/s for scalar-heavy distribution workloads; medium-lambda Poisson uses an Ahrens-Dieter rejection path; `WeightedIntTree` reaches about 52M update+sample ops/s for unsigned integer weights; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
