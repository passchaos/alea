...
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `Rng.normal` uses the direct ziggurat path and reaches about 386-390M samples/s, up from about 210-224M; medium-lambda Poisson uses an Ahrens-Dieter rejection path and reaches about 53M samples/s at `lambda=20`, up from about 26M; `WeightedIntTree` reaches about 52M update+sample ops/s for unsigned integer weights; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
