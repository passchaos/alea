...
| Distribution algorithms | Normal samplers cache paired Box-Muller output, exponential samplers precompute inverse rate, binomial has exact small-n plus p=0.5 fast paths, and an explicit Poisson approximation helper exists for large sparse binomials. Current native benchmark evidence shows exact large sparse binomial around 1.1M samples/s versus 25.0M samples/s for the approximation helper; exact BTPE/BTRD-class large-n binomial and ziggurat-style normal/exponential algorithms remain future work. | Medium |
