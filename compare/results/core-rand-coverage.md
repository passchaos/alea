...
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `Rng.normal` and `Rng.exponential` use ziggurat-backed stdlib paths; bulk `fillRange`, typed integer fill, packed `f32` and `f32` range fill, and bool fill APIs now have native benchmark rows; `fillNormal(f32)` and `fillExponential(f32)` stay on faster ziggurat scalar paths after vector kernels underperformed; `compare/results/performance-triage.md` tracks open gaps and rejected attempts |
