...
| Area | Gap | Priority |
| --- | --- | --- |
| Full statistical validation | `zig build statcheck` adds a longer in-repo statistical smoke harness; `zig build stream` and `tools/practrand.sh` export raw bytes for PractRand/TestU01-style external testing. `FastPrng` has a checked-in PractRand 0.96 64MiB smoke pass; longer and multi-engine PractRand/TestU01 reports are still pending. | Medium |
