...
| S4-M3 | SIMD/vector sampling design | Design and prototype Zig-native vector/SIMD sampling APIs for high-volume scalar distributions without copying Rust `std::simd` surface shapes. | In progress: `Rng` now has Zig-native vector APIs, direct/static vector helpers, direct/static vector fill helpers, packed-lane prototypes for bool/integer/`f32` vectors, vector-slice fill, vector range/normal/exponential fills, vector-math `f32` normal/exponential kernels, paired `fillVectorNormal(f32)` generation, and a dedicated `vectorbench` step; deeper SIMD-optimized distribution kernels are still open |
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `ScalarPrng` reaches about 427M normal, 426M exponential, and 76M Poisson samples/s for scalar-heavy distribution workloads; reusable `Normal(T)` now uses direct ziggurat and raises log-normal throughput to about 104M samples/s; medium-lambda Poisson uses an Ahrens-Dieter rejection path; `WeightedIntTree` reaches about 52M update+sample ops/s for unsigned integer weights; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
