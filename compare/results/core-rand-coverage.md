...
| S4-M2 | Longer external statistical validation | Run and check in longer PractRand/TestU01-compatible evidence beyond the current local 64GiB engine reports for the primary Linux engines. | In progress: `fast` / `Alea4x64` has a clean 128GiB PractRand pass; `default` / `Xoshiro256` has one default-seed `unusual` at 128GiB and a clean alternate-seed rerun, recorded in `compare/results/2026-06-28-practrand-default-128gib.md`; remaining primary engines still need longer-than-64GiB evidence |
