...
| Area | Gap | Priority |
| --- | --- | --- |
| Full statistical validation | `zig build statcheck` adds a longer in-repo statistical smoke harness; `zig build stream` and `tools/practrand.sh` export raw bytes for PractRand/TestU01-style external testing. Stream exporter smoke evidence is checked in; a long-run external PractRand/TestU01 report is still pending because `RNG_test` is not available locally. | Medium |
