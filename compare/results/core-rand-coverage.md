...
| Distribution algorithms | Normal samplers cache paired Box-Muller output, exponential samplers precompute inverse rate, binomial has exact small-n plus p=0.5 fast paths, and an explicit Poisson approximation helper exists for large sparse binomials; exact large-n general binomial and ziggurat-style normal/exponential algorithms remain future work. | Medium |
