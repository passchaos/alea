...
| S4-M4 | Performance follow-up from parity benchmarks | Use the completed feature matrix to choose targeted performance work where Alea trails local Rust in comparable core workloads. | In progress: `Rng.normal` with `FastPrng` reaches about 386-390M samples/s, while scalar-fast `wyhash64` reaches about 425M; medium-lambda Poisson uses an Ahrens-Dieter rejection path and reaches about 53M samples/s at `lambda=20`; `WeightedIntTree` reaches about 52M update+sample ops/s for unsigned integer weights; `compare/results/performance-triage.md` tracks remaining gaps and rejected attempts |
