...
| S4-M3 | SIMD/vector sampling design | Design and prototype Zig-native vector/SIMD sampling APIs for high-volume scalar distributions without copying Rust `std::simd` surface shapes. | In progress: `Rng` now has Zig-native vector APIs, direct/static vector helpers, direct/static vector fill helpers, packed-lane prototypes for bool/integer/`f32` vectors, vectorized `f32` and `f64` uniform/open/open-closed helpers and bulk fills, direct-source scalar/vector bulk `fillFrom` / `fillRangeFrom`, single-pass vector-scaled float range fills, vector-slice fill, vector range/chance/ratio/normal/exponential fills, vector chance/ratio/range/normal/exponential APIs, vector-slice chance/ratio fill APIs, vector-math `f32` normal/exponential kernels, experimental `f64` vector normal/exponential kernels, scalar-ziggurat vector-slice normal/exponential fills, and a dedicated `vectorbench` step; deeper SIMD-optimized distribution kernels are still open |
