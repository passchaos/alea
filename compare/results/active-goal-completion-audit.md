...
| S4-M1242 next post-S4-M1241 product bar | `core-rand-coverage.md`, future audits | Not complete; S4-M1241 closes the nextU32-only source fallback hardening, while exact/default dense SIMD, broader runtime evidence, longer validation, further semantics-preserving performance work, and future newly discovered gaps remain open tracks. |
...
The goal remains active under the new S4-M1249 bar until at least one of these
happens:

- exact/default-compatible dense SIMD normal/exponential: a default/exact-compatible dense SIMD normal/exponential candidate beats scalar lane-fill in the real vector-slice harness for f32x8 while preserving or deliberately versioning rejected-lane stream shape (f64x4 mask-rejection SIMD ziggurat landed in S4-M1248; f32x8 remains open);
