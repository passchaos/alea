...
| No proxy signal is accepted as whole-goal completion | `zig build validate-all` plus roadmap/audit files | Validation passes are necessary but not sufficient; the goal remains active for the next raised product bar (Nakagami-m, Noncentral chi/chi-squared, Bingham/Matrix-vMF/Kent directional distributions, Rice vector/SIMD sampling, copula methods, SIMD spherical bulk sampling, weighted/alias advances, string generation, longer validation, broader runtime, or newly discovered gaps). |
