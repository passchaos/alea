...
| No proxy signal is accepted as whole-goal completion | `zig build validate-all` plus roadmap/audit files | Validation passes are necessary but not sufficient; the goal remains active for the next raised product bar (directional/spherical extensions, longer validation, broader runtime, or newly discovered gaps). |
