...
| S4-M1142 next post-S4-M1141 product bar | `core-rand-coverage.md`, future audits | Not complete; S4-M1141 is closed for the current bar, but the next stricter product improvement has not yet been selected. |
...
path drift, S4-M1140 refreshes status after that fix, and S4-M1141 extends f64x4 facade standard vector fills, so the next active
product bar is S4-M1142.
...
map drift, S4-M1140 refreshes status again, and S4-M1141 extends f64x4 facade standard vector fills, so the next active product bar
is S4-M1142.

## Required Next Work Before Completion

The goal remains active under the new S4-M1142 bar until at least one of these
happens:
