...
and S4-M1125 refreshes the status snapshots after that restoration. S4-M1126 adds fresh f32x8 dense SIMD probe evidence, and S4-M1127 closes a checksum-preserving f64x4 standard-normal direct-source fill optimization, and S4-M1128 closes the matching narrow standard-exponential direct-source fill call-shape improvement, and S4-M1129 refreshes current status after those closures, and S4-M1130 refreshes full `validate-all` evidence, and S4-M1131 refreshes current status after that validation, and S4-M1132 records refreshed f32x8 direct-source negative evidence, and S4-M1133 routes rate-one vector exponential fills through standard-exponential fills, and S4-M1134 applies the same delegation to single-vector helpers, and S4-M1135 routes scalar rate-one exponential fills through standard-exponential fills, and S4-M1136 routes scalar rate-one exponential single samples through standard-exponential samples, and S4-M1137 routes standard-parameter scalar normal single samples through standard-normal samples, and S4-M1138 refreshes current status after those delegation fixes, and S4-M1139 repairs roadmapcheck evidence-path drift, and S4-M1140 refreshes status after that fix, and S4-M1141 extends f64x4 standard vector fill specializations to facade standard-parameter workflows, and S4-M1142 extends the f64x4 call-shape specialization to parameterized vector fills, S4-M1143 aligns zero-rate exponential semantics with local rand_distr, S4-M1144 aligns negative normal/log-normal stddev semantics, S4-M1145 aligns unrestricted normal/log-normal log-space mean semantics, S4-M1146 aligns mean/CV edge semantics, S4-M1147 aligns Gamma-family infinity semantics, S4-M1148 aligns FisherF infinity semantics, S4-M1149 aligns StudentT infinity semantics, S4-M1150 aligns Cauchy non-finite parameter semantics, S4-M1151 aligns Pareto/Weibull infinite-scale semantics, S4-M1152 aligns Beta infinity semantics, S4-M1153 aligns Triangular non-finite bound semantics, and S4-M1154 aligns PERT infinite-shape semantics, S4-M1155 aligns Poisson max-lambda semantics, S4-M1156 aligns Geometric zero-probability semantics, S4-M1157 aligns InverseGaussian infinity semantics, S4-M1158 aligns SkewNormal unrestricted-location semantics, S4-M1159 aligns NormalInverseGaussian alpha-infinity semantics, S4-M1160 aligns Hypergeometric large-population semantics, S4-M1161 aligns Dirichlet subnormal-alpha semantics, S4-M1162 aligns Beta/Dirichlet tiny-shape semantics, S4-M1163 aligns AliasTable max-weight semantics, S4-M1164 aligns WeightedTree zero-total checked sampling semantics, S4-M1165 aligns WeightedIntTree integer overflow diagnostics, S4-M1166 refreshes full validate-all evidence, and S4-M1167 adds weighted sampler clone/equality helpers, and S4-M1168 adds weighted sampler format helpers, and S4-M1169 adds weighted-tree constructor/default helpers, and S4-M1170 adds weighted-tree trySample aliases, and S4-M1171 refreshes validate-all evidence, and S4-M1172 adds weighted iterator clone/format helpers, and S4-M1173 refreshes validate-all evidence, and S4-M1174 adds weighted error message helpers, and S4-M1175 refreshes validate-all evidence, and S4-M1176 adds root/prelude weighted error aliases, and S4-M1177 refreshes validate-all evidence, and S4-M1178 refreshes weighted manifest coverage, and S4-M1179 refreshes validate-local evidence, and S4-M1180 adds typed static weighted diagnostics, and S4-M1181 refreshes validate-all evidence, and S4-M1182 refreshes weighted public-surface manifests for those typed diagnostics, S4-M1183 extends typed weighted diagnostics through `seq.WeightedChoice`, S4-M1184 refreshes validate-all evidence, S4-M1185 refreshes dense SIMD vectorbench evidence, S4-M1186 refreshes validate-local evidence, and S4-M1187 adds dynamic weighted-tree typed diagnostics, and S4-M1188 refreshes validate-all evidence after that dynamic weighted-tree API change, and S4-M1189 adds dynamic weighted-tree typed total diagnostics, and S4-M1190 refreshes validate-all evidence after that change, and S4-M1191 repairs rand-status post-bar drift, and S4-M1192 repairs follow-up rand-status post-bar drift, and S4-M1193 refreshes validate-local evidence after those status repairs, and S4-M1194 adds weighted namespace aliases for local rand_distr discovery names, and S4-M1195 adds root weighted sampler aliases plus prelude weighted sampler aliases, and S4-M1196 refreshes weighted manifest root-alias evidence, and S4-M1197 exposes Poisson max-lambda public constants, and S4-M1198 refreshes validate-all evidence after that public API change, and S4-M1199 expands ziggurat-table surface guards for public const/static names, and S4-M1200 refreshes validate-all evidence after that verifier change, and S4-M1201 refreshes f32x8 vectorbench evidence, and S4-M1202 refreshes f64x4 vectorbench evidence, and S4-M1203 refreshes parameterized vectorbench evidence, S4-M1204 repairs vectorbench status drift, S4-M1205 refreshes validate-local evidence, S4-M1206 adds the roadmap evidence-path guard, S4-M1207 refreshes validate-all evidence after that guard, S4-M1208 refreshes validate-local evidence after the full validation refresh, S4-M1209 refreshes validate-all evidence after the local comparison update, S4-M1210 refreshes inverse-CDF tail vectorbench evidence, S4-M1211 refreshes validate-local evidence after the dense-SIMD evidence refresh, S4-M1212 refreshes validate-all evidence after the local comparison update, S4-M1213 refreshes validate-all evidence after the S4-M1212 status update, and S4-M1214 refreshes exponential vectorbench evidence, and S4-M1215 refreshes validate-local evidence after the exponential vectorbench probe, and S4-M1216 refreshes validate-all evidence after the local comparison update, and S4-M1217 refreshes minimum vectorbench evidence, and S4-M1218 refreshes validate-local evidence after the vectorbench gate, and S4-M1219 refreshes validate-all evidence after the local comparison update, and S4-M1220 adds full-covariance multivariate normal sampling, and S4-M1221 improves multivariate-normal small-dimension throughput, and S4-M1222 adds static multivariate-normal samplers, and S4-M1223 fixes f64 StandardUniform to the correct 53-bit grid, and S4-M1224/S4-M1225 recover f64 throughput and widen f64 vector fills, and S4-M1226/S4-M1227/S4-M1228/S4-M1229 harden distribution vector lane load/store, S4-M1230 hardens Rng normal affine lane stores, S4-M1231/S4-M1232 harden fallible byte/owned-byte fallbacks, S4-M1233 fixes fromRandom nextU32 byte shape, S4-M1234 tightens root string preallocation, S4-M1235/S4-M1236/S4-M1237/S4-M1238/S4-M1239/S4-M1240/S4-M1241 align direct-source raw-alias dispatch, S4-M1243 fixes stable iterator choice semantics, S4-M1244 converts StandardNormal/StandardExponential to polymorphic unit structs with Exp1 alias and vector support, S4-M1245 adds Standard alias, N-dimensional unit sphere/ball, and StandardCauchy/StandardLogistic unit structs, S4-M1246 adds Von Mises circular distribution with Best–Fisher rejection sampling, S4-M1247 adds Wrapped Cauchy circular distribution with closed-form scalar/SIMD inverse-CDF sampling, S4-M1248 lands true mask-rejection SIMD f64x4 ziggurat for standard normal/exponential closing the long-standing S4-M11 exact/default dense-SIMD blocker for f64x4, and S4-M1249 extends true mask-rejection SIMD ziggurat to native f32x8 precision profiles, wiring the kernel into `*NativeF32*` vector/fill APIs while preserving default f32 bitstream backward compatibility. S4-M11 is closed for the current bar for both f64x4 and native f32x8. S4-M1250 adds a Truncated Normal distribution with public normPdf/normCdf/probit mathematical helpers, a complete 24-function scalar/vector sample/fill family, VectorTruncatedNormal SIMD struct, promotion of vonMises/wrappedCauchy to full public API with missing Checked/vector/fill variants and vector structs, and correct moments/edge-case handling for degenerate/one-sided/unbounded truncations. S4-M1251 adds a Von Mises-Fisher (vMF) spherical directional distribution on Sⁿ⁻¹ for arbitrary comptime dimension n≥2 using Wood (1994) Ulrich-transform rejection sampling for n≥3 (Beta-distributed w + uniform equatorial tangent point + Householder reflection), S¹ delegation to the existing Von Mises for n=2, fast paths for κ=0 (uniform sphere) and κ→∞ (point mass), exact mean-resultant-length for n=2,3 with first-order continued-fraction approximation for higher dimensions, a complete scalar/vector/fill free-function family, initNormalized for caller-guaranteed unit directions, and comprehensive tests covering 2D concentration, 3D sphere-surface/unit-norm properties, κ=0 uniformity, high-κ degeneracy, arbitrary mean Householder reflection, 4D unit vectors, and f32 support. S4-M1252 adds a Watson axial spherical directional distribution on Sⁿ⁻¹ for arbitrary comptime dimension n≥2, the antipodally symmetric (x↔−x) counterpart to vMF, supporting bipolar (κ>0), girdle (κ<0), and uniform (κ=0) regimes, using Beta((n-1)/2,(n-1)/2) rejection sampling with log-ratio envelope tests, a shared `householderReflectFromE1` helper factored out from vMF, fast paths for κ→±∞ (±μ point mass and equator great circle), a Kummer confluent hypergeometric ₁F₁ series for mean cosine-squared moments with asymptotic expansions for large |κ|, a complete scalar/fill free-function family, and comprehensive tests covering bipolar/girdle concentration, κ=0 uniformity (1/n mean cos²), high-κ degeneracies, 2D/3D/4D cases, f32 support, and kummerM numerical sanity. S4-M1253 adds a Rice (Rician) fading distribution — noncentral chi with 2 degrees of freedom — modeling √((N(ν,σ²))²+(N(0,σ²))²), a high-impact distribution for Rician fading channels, MRI Rician noise, radar, and optics, with rejection-free two-Gaussian sampling, internal besselI0/logBesselI0 helpers (Abramowitz & Stegun approximations), K-factor accessor for fading-channel conventions, exact Bessel-form mean with log-exp overflow safety, fast ν=0 (Rayleigh) and σ=0 (point mass) paths, a complete scalar/fill free-function family, and tests covering Rayleigh reduction, point mass, non-negativity, high-SNR concentration, free functions, f32 support, and besselI0 sanity. S4-M1254 adds a Nakagami-m fading distribution — the canonical multipath fading envelope model for wireless/RF communications — with shape parameter m ≥ 0.5 (m=1 Rayleigh, m=0.5 half-normal/severe fading, m>1 approximates Rician, m→∞ Gaussian), rejection-free sampling via √(Gamma(m, Ω/m)) leveraging the existing Gamma sampler, exact moments using lgamma ratios (Γ(m+1/2)/Γ(m) computed in log-space), m=1 Rayleigh fast paths for all moments, mode formula (√(Ω·(m−1)/m) for m≥1, 0 for 0.5≤m<1), equivalent Rayleigh scale accessor for comparison with Rice/Rayleigh parameterizations, Ω=0 point mass fast path, a complete scalar/fill free-function family, and tests covering parameter validation (including the m≥0.5 boundary), Rayleigh equivalence (m=1), point mass at Ω=0, non-negativity across 6 shape values, m=0.5 half-normal moments with Monte Carlo verification, large-m concentration near √Ω, free-function/fill correctness, and f32 support. S4-M1255 adds an Inverse Gamma distribution — the conjugate prior for normal variance in Bayesian statistics, used throughout regression, Gaussian processes, hierarchical models, MCMC, and finance for inverse volatility priors — with shape α>0 and scale β>0, trivial rejection-free reciprocal-Gamma sampling (X=1/Gamma(α, 1/β)), correct infinite mean/variance behavior for α≤1 and α≤2 respectively, closed-form mode β/(α+1), a complete scalar/fill free-function family, and tests covering parameter validation, analytic moments (including infinite mean for α=0.5 and infinite variance for α=1.5), positivity/finiteness, reciprocal-Gamma identity Monte Carlo verification, 5000-sample mean/variance MC check, free-function/fill correctness, and f32 support. S4-M1256 adds an Exponentially Modified Gaussian (ExGaussian / EMG) distribution — the sum of a normal N(μ,σ²) and an exponential with mean τ>0 — the canonical response-time distribution in cognitive psychology (ex-Gaussian RT model), also used for chromatography peak shapes and inter-arrival times with additive Gaussian noise; sampling is trivially compositional and rejection-free (one normal + one exponential + sum); moments are exact sums: mean μ+τ, variance σ²+τ², skewness 2τ³/(σ²+τ²)^{3/2} (always right-skewed), with σ=0 producing a shifted exponential and rateValue = 1/τ accessor; tests cover parameter validation, analytic moments (including σ=0 exponential case with skewness=2), rate reciprocity, finiteness, σ=0 lower bound, 5000-sample Monte Carlo mean/variance, free-function/fill correctness, and f32 support. S4-M1257 adds a Generalized Pareto Distribution (GPD) with location μ, scale σ>0, and shape ξ (tail index) — the canonical peaks-over-threshold model in extreme value theory for hydrology, finance/VaR, insurance, and environmental risk — unifying three regimes (ξ=0 exponential, ξ>0 heavy Pareto-like tail with infinite mean for ξ≥1 and infinite variance for ξ≥0.5, ξ<0 bounded short tail on [μ, μ−σ/ξ]); inverse-CDF sampling is rejection-free via X=μ+σ·(U^{−ξ}−1)/ξ (ξ≠0) or X=μ−σ·lnU (ξ=0). S4-M1258 adds a Scaled Inverse Chi-Squared distribution — the Gelman BDA/BUGS/Stan conjugate prior on normal variance — which is exactly InverseGamma(ν/2, ντ²/2); providing it as a first-class distribution spares Bayesian statistics users the manual parameter mapping, delegating sampling to the existing InverseGamma helper. S4-M1259 adds a Hoyt (Nakagami-q) fading distribution — the complementary multipath fading model to Nakagami-m for channels with unequal scattered-power components — with fading parameter q∈(0,1] (q=1→Rayleigh, q→0→severe single-path), spread Ω=E[X²]>0, rejection-free two-Gaussian product sampling X=σ·√(Z₁²+(q·Z₂)²) with σ=√(Ω/(1+q²)), closed-form mean via Bessel functions (reusing besselI0 and adding a new standalone besselI1 A&S approximation), and mode=σ·q. S4-M1260 adds a Noncentral Chi-Squared distribution χ'²(k,λ) — the fundamental noncentral distribution for chi-squared/ANOVA power analysis and Rician fading power — with rejection-free Poisson-mixture sampling K~Poisson(λ/2), X~χ²(k+2K) composing existing Poisson and ChiSquared samplers, closed-form moments E[X]=k+λ, Var=2(k+2λ), mode=max(0,k+λ−2). S4-M1261 adds a Noncentral t distribution t'(ν,μ) — for one-sample/two-sample t-test power analysis and detection theory — with rejection-free ratio sampling (Z+μ)/√(V/ν) composing Normal and ChiSquared, Γ-function ratio moments via std.math.lgamma, ν=∞ normal limit, infinite mean for ν≤1 and infinite variance for ν≤2. S4-M1262 adds a Noncentral F distribution F'(d1,d2,λ) — for ANOVA/regression F-test power analysis — with rejection-free ratio sampling (X/d1)/(Y/d2) where X~χ'²(d1,λ) and Y~χ²(d2), closed-form moments with infinite mean for d2≤2 and infinite variance for d2≤4. S4-M1263 adds a Noncentral Chi distribution χ'(k,λ) — the sqrt of noncentral chi-squared generalizing Rayleigh/Rice to arbitrary dof — with kummerM/lgamma-based expected value reusing the existing ₁F₁ helper and a folded-normal k=0 branch using erfAbramowitzStegun. Additional directional/multivariate distributions (Bingham, Matrix vMF, Kent), Noncentral/Bingham/Hoyt/GPD/ScaledInvChiSq vector/SIMD sampling, copula methods, weighted/alias sampling advances, string generation expansion, SIMD spherical bulk sampling, longer statistical validation runs, broader runtime/architecture evidence, and other newly discovered improvements remain as the next raised product bar. Therefore do not call
...
| S4-M1032 MappedSampler facade direct paths | `src/distributions.zig`, `compare/results/s4-m1032-mapped-sampler-facade-direct.md` | Closed for the current bar: reusable MappedSampler facade sample/fill helpers now avoid From wrappers. |
| S4-M1033 Choice pointer fill facade direct path | `src/distributions.zig`, `compare/results/s4-m1033-choice-pointer-fill-facade-direct.md` | Closed for the current bar: distribution-layer reusable Choose pointer fill now avoids From wrappers. |
| S4-M1034 Normal fill facade direct path | `src/distributions.zig`, `compare/results/s4-m1034-normal-fill-facade-direct.md` | Closed for the current bar: reusable scalar Normal facade fill now avoids From wrappers. |
...
| S4-M1045 Native f32 LogNormal facade direct paths | `src/distributions.zig`, `compare/results/s4-m1045-native-f32-log-normal-facade-direct.md` | Closed for the current bar: reusable scalar/vector native-f32 LogNormal facade helpers now avoid From wrappers. |
| S4-M1046 LogNormalLibmvec facade direct paths | `src/distributions.zig`, `compare/results/s4-m1046-log-normal-libmvec-facade-direct.md` | Closed for the current bar: reusable LogNormalLibmvec facade sample/fill helpers now avoid From wrappers. |
| S4-M1047 LogNormalDlsymExp facade direct paths | `src/distributions.zig`, `compare/results/s4-m1047-log-normal-dlsym-exp-facade-direct.md` | Closed for the current bar: reusable LogNormalDlsymExp facade sample/fill helpers now avoid From wrappers. |
| S4-M1048 Native exp2 f32 LogNormal facade direct paths | `src/distributions.zig`, `compare/results/s4-m1048-native-exp2-log-normal-facade-direct.md` | Closed for the current bar: reusable scalar/vector native-exp2-f32 LogNormal facade helpers now avoid From wrappers. |
| S4-M1049 VectorLogNormal facade direct paths | `src/distributions.zig`, `compare/results/s4-m1049-vector-log-normal-facade-direct.md` | Closed for the current bar: reusable generic VectorLogNormal facade helpers now avoid From wrappers. |
| S4-M1050 Approx f32 LogNormal facade direct paths | `src/distributions.zig`, `compare/results/s4-m1050-approx-f32-log-normal-facade-direct.md` | Closed for the current bar: reusable scalar approximate-f32 LogNormal facade helpers now avoid From wrappers. |
...
| S4-M1055 AliasTable fill facade direct path | `src/distributions.zig`, `compare/results/s4-m1055-alias-table-fill-facade-direct.md` | Closed for the current bar: reusable AliasTable facade fill now avoids From wrappers. |
| S4-M1056 Seq Choice pointer fill facade direct path | `src/seq.zig`, `compare/results/s4-m1056-seq-choice-pointer-fill-facade-direct.md` | Closed for the current bar: sequence-layer reusable Choice pointer fill now avoids From wrappers. |
| S4-M1057 Seq WeightedChoice pointer fill facade direct path | `src/seq.zig`, `compare/results/s4-m1057-seq-weightedchoice-pointer-fill-facade-direct.md` | Closed for the current bar: sequence-layer reusable WeightedChoice pointer fill now avoids From wrappers. |
| S4-M1058 Seq WeightedChoice owned pointer facade direct path | `src/seq.zig`, `compare/results/s4-m1058-seq-weightedchoice-owned-ptrs-facade-direct.md` | Closed for the current bar: sequence-layer reusable WeightedChoice owned pointer batches now avoid From wrappers. |
...
| S4-M1063 Seq WeightedChoice checked u32 index sample facade direct path | `src/seq.zig`, `compare/results/s4-m1063-seq-weightedchoice-checked-u32-index-sample-facade-direct.md` | Closed for the current bar: sequence-layer reusable WeightedChoice checked u32 index sample now avoids From wrappers. |
| S4-M1064 Seq WeightedChoice checked value sample facade direct path | `src/seq.zig`, `compare/results/s4-m1064-seq-weightedchoice-checked-value-sample-facade-direct.md` | Closed for the current bar: sequence-layer reusable WeightedChoice checked value sample now avoids From wrappers. |
...
| S4-M1093 Distribution Choose index fill facade direct path | `src/distributions.zig`, `compare/results/s4-m1093-distribution-choose-index-fill-facade-direct.md` | Closed for the current bar: distribution-layer reusable Choose index fill now avoids From wrappers. |
| S4-M1094 ASCII Charset fill facade direct path | `src/ascii.zig`, `compare/results/s4-m1094-ascii-charset-fill-facade-direct.md` | Closed for the current bar: reusable ASCII Charset fill now avoids From wrappers. |
| S4-M1095 UnicodeCharset fill facade direct path | `src/ascii.zig`, `compare/results/s4-m1095-unicode-charset-fill-facade-direct.md` | Closed for the current bar: reusable UnicodeCharset fill now avoids From wrappers. |
| S4-M1096 ASCII Charset sample facade direct path | `src/ascii.zig`, `compare/results/s4-m1096-ascii-charset-sample-facade-direct.md` | Closed for the current bar: reusable ASCII Charset sample now avoids From wrappers. |
| S4-M1097 ASCII Charset checked sample facade direct path | `src/ascii.zig`, `compare/results/s4-m1097-ascii-charset-checked-sample-facade-direct.md` | Closed for the current bar: reusable ASCII Charset checked sample now avoids From wrappers. |
| S4-M1098 UnicodeCharset sample facade direct path | `src/ascii.zig`, `compare/results/s4-m1098-unicode-charset-sample-facade-direct.md` | Closed for the current bar: reusable UnicodeCharset sample now avoids From wrappers. |
...
| S4-M1106 UnicodeCharset sampleString facade direct path | `src/ascii.zig`, `compare/results/s4-m1106-unicode-charset-sample-string-facade-direct.md` | Closed for the current bar: reusable UnicodeCharset sampleString now avoids From wrappers. |
| S4-M1107 UnicodeCharset checked sampleString facade direct path | `src/ascii.zig`, `compare/results/s4-m1107-unicode-charset-checked-sample-string-facade-direct.md` | Closed for the current bar: reusable UnicodeCharset checked sampleString now avoids From wrappers. |
...
| S4-M1125 post-S4-M1124 rand-status refresh | `tools/rand_status.zig`, `compare/results/s4-m420-current-rand-status.md`, `compare/results/s4-m450-rand-status-command-matrix.md`, `compare/results/s4-m455-rand-status-direct-matrix.md`, `compare/results/s4-m1125-post-s4-m1124-rand-status-refresh.md` | Closed for the current bar: status command output and checked-in status matrices now report S4-M1124 closure and point at S4-M1126. |
| S4-M1126 f32x8 dense SIMD probe refresh | `compare/results/s4-m1126-f32x8-dense-simd-probe.md`, `bench/vector.zig`, `compare/results/simd-distribution-kernel-notes.md` | Closed for the current bar as probe evidence: fresh focused f32x8 vectorbench evidence shows checksum-preserving dense candidates still do not beat direct scalar lane-fill. |
| S4-M1127 f64x4 standard normal direct fill specialization | `src/rng.zig`, `compare/results/s4-m1127-f64x4-standard-normal-direct-fill.md` | Closed for the current bar: direct-source f64x4 standard-normal and parameterized-normal fills use the faster checksum-preserving call shape. |
| S4-M1128 f64x4 standard exponential direct fill specialization | `src/rng.zig`, `compare/results/s4-m1128-f64x4-standard-exponential-direct-fill.md` | Closed for the current bar: direct-source f64x4 standard-exponential fills use the checksum-preserving specialized call shape. |
| S4-M1129 post-S4-M1128 rand-status refresh | `tools/rand_status.zig`, `compare/results/s4-m1129-post-s4-m1128-rand-status-refresh.md`, `compare/results/s4-m420-current-rand-status.md` | Closed for the current bar: status tooling and snapshots now report S4-M1127/S4-M1128 closure and point at S4-M1130. |
| S4-M1130 post-S4-M1129 validate-all refresh | `compare/results/s4-m1130-post-s4-m1129-validate-all.md`, `zig build validate-all` | Closed for the current bar: full validate-all passed after S4-M1127/S4-M1128 code changes and S4-M1129 status synchronization. |
| S4-M1131 post-S4-M1130 rand-status refresh | `tools/rand_status.zig`, `compare/results/s4-m1131-post-s4-m1130-rand-status-refresh.md`, `compare/results/s4-m420-current-rand-status.md` | Closed for the current bar: status tooling now reports S4-M1130 validate-all closure and points at S4-M1132. |
| S4-M1132 f32x8 direct-source probe refresh | `compare/results/s4-m1132-f32x8-direct-source-probe.md`, `bench/vector.zig` | Closed for the current bar as refreshed negative evidence: checksum-preserving f32x8 candidates still do not beat direct exact/default baselines. |
| S4-M1133 vector exponential rate-one delegate | `src/rng.zig`, `compare/results/s4-m1133-vector-exponential-rate-one-delegate.md` | Closed for the current bar: rate-one vector exponential fills reuse standard-exponential fill paths while preserving checksums. |
| S4-M1134 single-vector exponential rate-one delegate | `src/rng.zig`, `compare/results/s4-m1134-single-vector-exponential-rate-one-delegate.md` | Closed for the current bar: single-vector rate-one vector exponential sampling now delegates to standard exponential sampling. |
| S4-M1135 scalar exponential rate-one fill delegate | `src/rng.zig`, `compare/results/s4-m1135-scalar-exponential-rate-one-fill.md` | Closed for the current bar: scalar rate-one exponential fills now reuse standard-exponential fill paths. |
| S4-M1136 scalar exponential rate-one sample delegate | `src/rng.zig`, `compare/results/s4-m1136-scalar-exponential-rate-one-sample.md` | Closed for the current bar: scalar rate-one exponential single samples now reuse standard-exponential sample paths. |
...
| S4-M1145 non-finite normal mean compatibility | `src/rng.zig`, `src/distributions.zig`, `compare/results/s4-m1145-nonfinite-normal-mean.md` | Closed for the current bar: unrestricted normal/log-normal log-space means are accepted across exact/default scalar/vector/checked/reusable workflows while non-finite stddev remains invalid and stream shape is preserved. |
...
| S4-M1150 cauchy non-finite parameter compatibility | `src/distributions.zig`, `compare/results/s4-m1150-cauchy-nonfinite.md` | Closed for the current bar: Cauchy scalar/vector helpers now accept local `rand_distr` non-finite median and infinite-scale parameter edges while preserving Alea zero-scale point masses. |
| S4-M1151 pareto/weibull infinite-scale compatibility | `src/distributions.zig`, `compare/results/s4-m1151-pareto-weibull-infinite-scale.md` | Closed for the current bar: Pareto/Weibull scalar/vector helpers now accept local `rand_distr` infinite-scale edges while preserving finite-shape transform stream shape. |
| S4-M1152 beta infinity compatibility | `src/distributions.zig`, `compare/results/s4-m1152-beta-infinity.md` | Closed for the current bar: Beta scalar/vector helpers now accept local `rand_distr` one-sided and both-sided infinite-shape edges while preserving endpoint/NaN outputs and Cheng draw shape. |
| S4-M1153 triangular non-finite bounds compatibility | `src/distributions.zig`, `compare/results/s4-m1153-triangular-nonfinite.md` | Closed for the current bar: Triangular scalar/vector helpers now accept local `rand_distr` ordered non-finite bounds and preserve NaN outputs plus uniform draw shape. |
| S4-M1154 pert infinite-shape compatibility | `src/distributions.zig`, `compare/results/s4-m1154-pert-infinite-shape.md` | Closed for the current bar: finite-range PERT shape-infinity now preserves local `rand_distr` Beta `(inf, inf)` NaN output and draw shape while collapsed range remains an Alea extension. |
| S4-M1155 poisson max-lambda compatibility | `src/distributions.zig`, `compare/results/s4-m1155-poisson-max-lambda.md` | Closed for the current bar: scalar/vector Poisson checked/reusable helpers now reject finite `lambda > 1.844e19` like local `rand_distr` while retaining Alea zero-lambda point masses. |
| S4-M1156 geometric zero-probability compatibility | `src/distributions.zig`, `compare/results/s4-m1156-geometric-zero-probability.md` | Closed for the current bar: GeometricFailures scalar/vector checked/reusable helpers now accept local `rand_distr` p=0/-0.0 max-`u64` saturation while one-based Geometric keeps p=0 invalid. |
| S4-M1157 inverse-gaussian infinity compatibility | `src/distributions.zig`, `compare/results/s4-m1157-inverse-gaussian-infinity.md` | Closed for the current bar: InverseGaussian scalar/vector helpers now accept positive infinite mean/shape and preserve local `rand_distr` NaN output plus StandardNormal/uniform draw shape while retaining the zero-mean Alea extension. |
| S4-M1158 skew-normal unrestricted-location compatibility | `src/distributions.zig`, `compare/results/s4-m1158-skew-normal-location.md` | Closed for the current bar: SkewNormal scalar/vector helpers now accept local `rand_distr` unrestricted locations and propagate NaN/±inf while preserving normal draw shape and Alea zero-scale point masses. |
| S4-M1159 normal-inverse-gaussian alpha-infinity compatibility | `src/distributions.zig`, `compare/results/s4-m1159-nig-alpha-infinity.md` | Closed for the current bar: NormalInverseGaussian scalar/vector checked/reusable helpers now reject local `rand_distr` alpha-infinity without consuming randomness while preserving large finite-alpha construction. |
| S4-M1160 hypergeometric large-population compatibility | `src/distributions.zig`, `compare/results/s4-m1160-hypergeometric-large-population.md` | Closed for the current bar: Hypergeometric scalar/vector checked/reusable helpers now reject local `rand_distr` HIN large-population underflow without consuming randomness while preserving accepted sparse huge-population cases. |
| S4-M1161 dirichlet subnormal-alpha compatibility | `src/distributions.zig`, `compare/results/s4-m1161-dirichlet-subnormal-alpha.md` | Closed for the current bar: Dirichlet and `multi.Dirichlet` now reject local `rand_distr` positive subnormal alphas while preserving documented Alea one-dimensional and single-infinite-alpha point-mass extensions. |
| S4-M1162 beta/dirichlet tiny-shape stability | `src/distributions.zig`, `compare/results/s4-m1162-beta-dirichlet-tiny-shape.md` | Closed for the current bar: finite tiny-shape Beta now uses the local `rand_distr` Cheng sampler and all-small finite Dirichlet uses Beta stick-breaking, avoiding Gamma-ratio NaN for tiny normal shapes. |
| S4-M1163 alias-table max-weight compatibility | `src/distributions.zig`, `compare/results/s4-m1163-alias-table-max-weight.md` | Closed for the current bar: AliasTable/WeightedIndex now reject local `rand_distr::WeightedAliasIndex` per-weight overflow before scaling, including update paths, while preserving valid maximum-boundary tables. |
| S4-M1164 weighted-tree zero-total compatibility | `src/distributions.zig`, `compare/results/s4-m1164-weighted-tree-zero-total.md` | Closed for the current bar: checked sampling-style WeightedTree/WeightedIntTree APIs now match local `rand_distr::WeightedTreeIndex::try_sample` zero-total `InsufficientNonZero` diagnostics while preserving probability/export InvalidWeight diagnostics. |
| S4-M1165 weighted-int-tree overflow compatibility | `src/distributions.zig`, `compare/results/s4-m1165-weighted-int-tree-overflow.md` | Closed for the current bar: WeightedIntTree construction and update paths now report local `rand_distr::WeightedTreeIndex` style `Overflow` for integer total overflow while preserving wider-than-`u64` `InvalidWeight` diagnostics. |
| S4-M1166 post-S4-M1165 validate-all refresh | `compare/results/s4-m1166-post-s4-m1165-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after the WeightedIntTree overflow diagnostic change, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1167 weighted-sampler clone/equality helpers | `src/distributions.zig`, `compare/results/s4-m1167-weighted-sampler-clone-eql.md` | Closed for the current bar: AliasTable/WeightedIndex, WeightedTree, and WeightedIntTree now expose Zig-native clone/eql helpers matching local Rust weighted sampler Clone/PartialEq workflows. |
| S4-M1168 weighted-sampler format helpers | `src/distributions.zig`, `compare/results/s4-m1168-weighted-sampler-format.md` | Closed for the current bar: AliasTable/WeightedIndex, WeightedTree, and WeightedIntTree now expose Zig-native `{f}` formatter output matching local Rust weighted sampler Debug-style diagnostics. |
| S4-M1169 weighted-tree default constructors | `src/distributions.zig`, `compare/results/s4-m1169-weighted-tree-default.md` | Closed for the current bar: WeightedTree and WeightedIntTree now expose Rust-discoverable `new` aliases plus Zig-native `initEmpty` / `default` constructors matching local Rust WeightedTreeIndex new/default workflows. |
| S4-M1170 weighted-tree trySample aliases | `src/distributions.zig`, `compare/results/s4-m1170-weighted-tree-try-sample.md` | Closed for the current bar: WeightedTree and WeightedIntTree now expose Zig-native `trySample` / `trySampleFrom` aliases matching local Rust WeightedTreeIndex try_sample workflows. |
| S4-M1171 post-S4-M1170 validate-all refresh | `compare/results/s4-m1171-post-s4-m1170-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1170, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1172 weighted-iterator clone/format helpers | `src/distributions.zig`, `compare/results/s4-m1172-weighted-iterator-clone-format.md` | Closed for the current bar: weighted sampler weight/probability iterators now expose Zig-native `clone` and `{f}` format helpers matching local Rust WeightedIndexIter clone/debug workflows. |
| S4-M1173 post-S4-M1172 validate-all refresh | `compare/results/s4-m1173-post-s4-m1172-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1172, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1174 weighted-error message helpers | `src/distributions.zig`, `compare/results/s4-m1174-weighted-error-message.md` | Closed for the current bar: weightedErrorMessage and `weighted.errorMessage` now expose local Rust weighted Error Display-style diagnostics. |
| S4-M1175 post-S4-M1174 validate-all refresh | `compare/results/s4-m1175-post-s4-m1174-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1174, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1176 root weighted-error aliases | `src/root.zig`, `compare/results/s4-m1176-root-weighted-error-aliases.md` | Closed for the current bar: root and prelude now expose `WeightedError` and `weightedErrorMessage` aliases for distribution weighted diagnostics workflows. |
| S4-M1177 post-S4-M1176 validate-all refresh | `compare/results/s4-m1177-post-s4-m1176-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1176, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1178 weighted-manifest refresh | `compare/results/s4-m288-local-rand-public-surface-manifest.md`, `compare/results/s4-m294-rand-distr-public-surface-manifest.md`, `compare/results/s4-m1178-weighted-manifest-refresh.md` | Closed for the current bar: local Rust/rand_distr manifests now cite recent weighted iterator/error/root alias closures. |
| S4-M1179 post-S4-M1178 validate-local refresh | `compare/results/s4-m1179-post-s4-m1178-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after the weighted manifest refresh, covering local Rust surface scans and comparison smoke checks. |
| S4-M1180 typed static weighted diagnostics | `src/distributions.zig`, `compare/results/s4-m1180-typed-static-weighted-diagnostics.md` | Closed for the current bar: AliasTable/WeightedIndex now expose typed weight/total diagnostics and typed weight iterators matching local Rust WeightedIndex typed accessor workflows. |
| S4-M1181 post-S4-M1180 validate-all refresh | `compare/results/s4-m1181-post-s4-m1180-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1180, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1182 weighted-manifest typed-diagnostics refresh | `compare/results/s4-m288-local-rand-public-surface-manifest.md`, `compare/results/s4-m294-rand-distr-public-surface-manifest.md`, `compare/results/s4-m1182-weighted-manifest-refresh.md` | Closed for the current bar: local Rust/rand_distr manifests now cite typed static weighted diagnostics for `WeightedIndex` / `AliasTable` accessor workflows. |
| S4-M1183 weighted-choice typed diagnostics | `src/seq.zig`, `compare/results/s4-m1183-weighted-choice-typed-diagnostics.md` | Closed for the current bar: `seq.WeightedChoice(T, Weight)` now exposes typed total, single-weight, bulk-weight, and typed iterator diagnostics matching the underlying `AliasTable` typed accessors. |
| S4-M1184 post-S4-M1183 validate-all refresh | `compare/results/s4-m1184-post-s4-m1183-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1183, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1185 dense SIMD probe refresh | `compare/results/s4-m1185-dense-simd-probe-refresh.md`, `compare/results/performance-triage.md`, `compare/results/simd-distribution-kernel-notes.md` | Closed for the current bar as refreshed research evidence: focused real-harness vectorbench rows still do not justify replacing exact/default scalar lane-fill vector normal/exponential paths. |
| S4-M1186 post-S4-M1185 validate-local refresh | `compare/results/s4-m1186-post-s4-m1185-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1185, covering local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, and current status output. |
| S4-M1187 dynamic weighted-tree typed diagnostics | `src/distributions.zig`, `compare/results/s4-m1187-dynamic-tree-typed-diagnostics.md` | Closed for the current bar: dynamic weighted trees now expose typed single/bulk/pop/iterator diagnostics matching local Rust `WeightedTreeIndex::get` style workflows. |
| S4-M1188 post-S4-M1187 validate-all refresh | `compare/results/s4-m1188-post-s4-m1187-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1187, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1189 dynamic weighted-tree typed totals | `src/distributions.zig`, `compare/results/s4-m1189-dynamic-tree-typed-total.md` | Closed for the current bar: dynamic weighted trees now expose typed total diagnostics matching Alea static weighted typed-total workflows. |
| S4-M1190 post-S4-M1189 validate-all refresh | `compare/results/s4-m1190-post-s4-m1189-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1189, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1191 rand-status post-bar drift repair | `tools/rand_status.zig`, `tools/roadmapcheck.zig`, `compare/results/s4-m1191-rand-status-post-bar-drift.md` | Closed for the current bar: status JSON now reports the correct post-S4-M1190 blocker suffix and roadmapcheck maps S4-M1190 evidence to S4-M1190. |
| S4-M1192 rand-status post-bar drift repair follow-up | `tools/rand_status.zig`, `compare/results/s4-m1192-rand-status-post-bar-drift.md` | Closed for the current bar: status JSON no longer reports a self-referential S4-M1192 post-bar suffix. |
| S4-M1193 post-S4-M1192 validate-local refresh | `compare/results/s4-m1193-post-s4-m1192-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1192, covering local Rust surface scans, rand-bench parser/smoke checks, runtime availability checks, and current status output. |
| S4-M1194 weighted namespace alias refresh | `src/distributions.zig`, `compare/results/s4-m1194-weighted-namespace-aliases.md` | Closed for the current bar: `distributions.weighted` now exposes `WeightedAliasIndex`, `WeightedTreeIndex`, and `WeightedIntTreeIndex` aliases. |
| S4-M1195 root weighted sampler aliases | `src/root.zig`, `compare/results/s4-m1195-root-weighted-sampler-aliases.md` | Closed for the current bar: root and prelude now expose direct weighted sampler aliases. |
| S4-M1196 weighted manifest root-alias refresh | `compare/results/s4-m288-local-rand-public-surface-manifest.md`, `compare/results/s4-m1196-weighted-manifest-root-alias-refresh.md` | Closed for the current bar: local rand weighted manifest now cites root/prelude weighted sampler aliases. |
| S4-M1197 Poisson max-lambda public constants | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m294-rand-distr-public-surface-manifest.md`, `compare/results/s4-m1197-poisson-max-lambda-constants.md` | Closed for the current bar: Alea exposes the local `rand_distr::Poisson::MAX_LAMBDA` threshold through top-level, sampler, and vector constants. |
| S4-M1198 post-S4-M1197 validate-all refresh | `compare/results/s4-m1198-post-s4-m1197-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1197, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1199 ziggurat table surface guard | `tools/surfacecheck.zig`, `compare/results/s4-m294-rand-distr-public-surface-manifest.md`, `compare/results/s4-m1199-ziggurat-table-surface-guard.md` | Closed for the current bar: surfacecheck now extracts public const/static symbols and guards the full local rand_distr ziggurat table public surface. |
| S4-M1200 post-S4-M1199 validate-all refresh | `compare/results/s4-m1200-post-s4-m1199-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1199, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1201 f32x8 vectorbench refresh | `compare/results/s4-m1201-f32x8-vectorbench-refresh.md` | Closed for the current bar: focused f32x8 standard normal/exponential vectorbench rows still reject exact/default dense SIMD replacements for scalar lane-fill. |
| S4-M1202 f64x4 vectorbench refresh | `compare/results/s4-m1202-f64x4-vectorbench-refresh.md` | Closed for the current bar: focused f64x4 standard normal/exponential vectorbench rows still find no exact/default production replacement for scalar lane-fill. |
| S4-M1203 parameterized vectorbench refresh | `compare/results/s4-m1203-parameterized-vectorbench-refresh.md` | Closed for the current bar: focused parameterized f32x8/f64x4 normal/exponential vectorbench rows still find no exact/default production replacement for scalar lane-fill. |
| S4-M1204 vectorbench status drift repair | `compare/results/s4-m1204-vectorbench-status-drift.md` | Closed for the current bar: S4-M1202/S4-M1203 audit/status mappings now point to their own evidence files and roadmapcheck guards the chain. |
| S4-M1205 post-S4-M1204 validate-local refresh | `compare/results/s4-m1205-post-s4-m1204-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1204, covering local Rust comparison, status, runtime, and surface guards. |
| S4-M1206 roadmap evidence path guard | `compare/results/s4-m1206-roadmap-evidence-path-guard.md` | Closed for the current bar: roadmapcheck now rejects evidence table paths that do not contain their own milestone token. |
| S4-M1207 post-S4-M1206 validate-all refresh | `compare/results/s4-m1207-post-s4-m1206-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1206, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1208 post-S4-M1207 validate-local refresh | `compare/results/s4-m1208-post-s4-m1207-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1207, covering local Rust comparison, status, runtime, and surface guards. |
| S4-M1209 post-S4-M1208 validate-all refresh | `compare/results/s4-m1209-post-s4-m1208-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1208, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1210 inverse-CDF tail probe | `compare/results/s4-m1210-inverse-cdf-tail-probe.md` | Closed for the current bar: valid inverse-CDF tail-only f32x8/f64x4 normal candidates still trail same-host exact/default direct ziggurat lane-fill baselines, while central-only/tail-zero probes remain invalid diagnostics. |
| S4-M1211 post-S4-M1210 validate-local refresh | `compare/results/s4-m1211-post-s4-m1210-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1210, covering local Rust comparison, status, runtime, and surface guards. |
| S4-M1212 post-S4-M1211 validate-all refresh | `compare/results/s4-m1212-post-s4-m1211-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1211, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1213 post-S4-M1212 validate-all refresh | `compare/results/s4-m1213-post-s4-m1212-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1212, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1214 exponential vectorbench probe | `compare/results/s4-m1214-exponential-vectorbench-probe.md` | Closed for the current bar: approx-log and table-CDF exponential rows beat exact/default direct ziggurat lane-fill but remain checksum/output-contract-changing opt-ins. |
| S4-M1215 post-S4-M1214 validate-local refresh | `compare/results/s4-m1215-post-s4-m1214-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1214, covering local Rust comparison, status, runtime, and surface guards. |
| S4-M1216 post-S4-M1215 validate-all refresh | `compare/results/s4-m1216-post-s4-m1215-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1215, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1217 minimum vectorbench gate refresh | `compare/results/s4-m1217-minimum-vectorbench-gate.md` | Closed for the current bar: the minimum f32x8/f64x4 standard and parameterized vectorbench gate still found no exact/default dense SIMD replacement for scalar ziggurat lane-fill. |
| S4-M1218 post-S4-M1217 validate-local refresh | `compare/results/s4-m1218-post-s4-m1217-validate-local.md` | Closed for the current bar: `zig build validate-local` passed after S4-M1217, covering local Rust comparison, status, runtime, and surface guards. |
| S4-M1219 post-S4-M1218 validate-all refresh | `compare/results/s4-m1219-post-s4-m1218-validate-all.md` | Closed for the current bar: `zig build validate-all` passed after S4-M1218, covering native validation, crosscheck, Node WASI unit/dry/self tests, and the chained WASI report. |
| S4-M1220 full-covariance multivariate normal | `src/distributions.zig`, `tools/distcheck.zig`, `bench/throughput.zig`, `examples/multivariate_sampling.zig`, `compare/results/s4-m1220-multivariate-normal.md` | Closed for the current bar: reusable Cholesky-backed full-covariance multivariate normal sampling adds an allocation-free correlated-vector workflow beyond local `rand_distr`, with singular covariance, diagnostics, statistical gates, example, and benchmark coverage. |
| S4-M1221 multivariate-normal small-dimension throughput | `src/distributions.zig`, `bench/throughput.zig`, `compare/results/s4-m1221-multivariate-normal-throughput.md` | Closed for the current bar: corrected all-coordinate benchmarks expose real throughput, f64 1–3D transforms and full-rank 3D batch loops materially improve f64/f32 sampling, and repeated/batch stream tests preserve f32/f64 plus singular covariance behavior. |
| S4-M1222 static allocation-free multivariate normal | `src/distributions.zig`, `bench/throughput.zig`, `examples/multivariate_sampling.zig`, `compare/results/s4-m1222-static-multivariate-normal.md` | Closed for the current bar: comptime-dimension inline mean/factor storage removes allocator lifecycle, fixed-array outputs preserve dynamic sampler streams, and 8D real-harness throughput improves by about 44%. |
| S4-M1223 f64 StandardUniform 53-bit grid consistency | `src/rng.zig`, `compare/results/s4-m1223-f64-standard-uniform-grid.md` | Closed for the current bar: ordinary f64 StandardUniform now consistently uses the high-53-bit `n / 2^53` grid across facade, direct-source, bulk-fill, iterator/distribution-fill, and vector paths. |
| S4-M1224 f64 StandardUniform throughput recovery | `src/rng.zig`, `compare/results/s4-m1224-f64-standard-uniform-throughput.md` | Closed for the current bar: ordinary f64 StandardUniform uses a split bitcast-plus-half-ULP conversion that preserves S4-M1223 `n / 2^53` values while recovering much of the hot-path throughput. |
| S4-M1225 f64 uniform low-bit/vector8 recovery | `src/rng.zig`, `compare/results/s4-m1225-f64-uniform-lowbit-vector8.md` | Closed for the current bar: ordinary f64 uniform uses low-bit bitcast and 8-lane fill/range chunks while preserving S4-M1223 `n / 2^53` values and scalar stream shape. |
| S4-M1226 float fill lane-store refactor | `src/rng.zig`, `compare/results/s4-m1226-float-fill-lane-store-refactor.md` | Closed for the current bar: vectorized float slice fills now derive lane counts from `VectorType`, store through a single inline lane helper, and cover packed f32 plus f64 strict-open stream shapes. |
| S4-M1227 distribution lane-store refactor | `src/distributions.zig`, `compare/results/s4-m1227-distribution-lane-store-refactor.md` | Closed for the current bar: distribution staged slice transforms now derive/copy lane counts through shared helpers, covering standard f32 stream shape plus scale/abs/exp slice transforms. |
| S4-M1228 distribution transform lane-store sweep | `src/distributions.zig`, `compare/results/s4-m1228-distribution-transform-lane-store-sweep.md` | Closed for the current bar: remaining staged distribution transforms now derive/copy lane counts through shared helpers, with formula-equivalence and inverse-Gaussian stream-shape focused tests. |
| S4-M1229 vector sampler lane-count sweep | `src/distributions.zig`, `compare/results/s4-m1229-vector-sampler-lane-count-sweep.md` | Closed for the current bar: remaining distribution vector sampler wrappers now derive lane counts through `vectorInfo(VectorType).len`, and focused vector/checked-fill stream-shape tests pass. |
| S4-M1230 rng normal affine lane-store refactor | `src/rng.zig`, `compare/results/s4-m1230-rng-normal-affine-lane-store.md` | Closed for the current bar: `normalAffineInPlaceVector` now derives/copies lane counts through shared helpers, with f32/f64 scalar formula focused tests. |
| S4-M1231 rng reader try-fill/refill hardening | `src/rng.zig`, `compare/results/s4-m1231-rng-reader-tryfill-hardening.md` | Closed for the current bar: public `tryFillBytesFrom` now shares the fallible byte-fill fallback used by `RngReader`, zero-length fills avoid touching sources, and focused tests cover try-next-only sources plus buffered `std.Io.Reader` refills. |
| S4-M1232 owned byte allocation fallback hardening | `src/rng.zig`, `compare/results/s4-m1232-owned-byte-allocation-fallback.md` | Closed for the current bar: `bytesAllocFrom` now shares the fallible byte-fill fallback used by `tryFillBytesFrom` and `RngReader`, preserving allocation-before-consumption behavior and freeing owned buffers on source errors; focused tests cover try-next-only and SysRng success/failure. |
| S4-M1233 std.Random adapter nextU32 byte-shape hardening | `src/rng.zig`, `compare/results/s4-m1233-fromrandom-nextu32-byte-shape.md` | Closed for the current bar: `Rng.fromRandom(...).nextU32()` now uses `std.Random.int(u32)` so it consumes four bytes and matches native `std.Random` u32 output while `nextU64` remains unchanged. |
| S4-M1234 root string preallocation hardening | `src/root.zig`, `compare/results/s4-m1234-root-string-preallocation.md` | Closed for the current bar: root `string`, `sampleString`, and `appendString` now allocate or reserve caller-owned output before secure entropy and preserve visible list contents on entropy failure, with focused allocation/entropy/overflow tests. |
| S4-M1235 tryNextU32 direct-source native fallback | `src/rng.zig`, `compare/results/s4-m1235-try-next-u32-native-fallback.md` | Closed for the current bar: `Rng.tryNextU32From` now uses source-native `nextU32` when `tryNextU32` is absent, preserving u32 stream shape and matching `nextU32From` for native-u32 sources. |
| S4-M1236 nextU64 direct-source native fallback | `src/rng.zig`, `compare/results/s4-m1236-next-u64-native-fallback.md` | Closed for the current bar: `Rng.nextU64From` and `Rng.tryNextU64From` now use source-native `nextU64` before falling back to `next`, accepting Rust-style direct sources without forcing a Zig-only raw spelling. |
| S4-M1237 fillBytes direct-source fallback | `src/rng.zig`, `compare/results/s4-m1237-fillbytes-direct-source-fallback.md` | Closed for the current bar: `Rng.fillBytesFrom` now accepts source-native `fillBytes`, falls back through Zig-native `fill`, and then uses `nextU64From`; fallible byte helpers keep the shared source contract. |
| S4-M1238 generic nextFrom native-u64 fallback | `src/rng.zig`, `compare/results/s4-m1238-nextfrom-native-u64-fallback.md` | Closed for the current bar: `Rng.nextFrom` now preserves Zig-native `next` precedence while falling back to Rust-style `nextU64`, making generic value/fill helpers work with nextU64-only direct sources. |
| S4-M1239 fromRng native-u64 fallback | `src/source.zig`, `src/seed.zig`, `src/engines/*.zig`, `compare/results/s4-m1239-fromrng-native-u64-fallback.md` | Closed for the current bar: seed and engine `fromRng` / `tryFromRng` helpers now share native-u64 source dispatch, accepting `nextU64` / `tryNextU64` while preserving `next` / `tryNext` precedence. |
| S4-M1240 Rng.init raw-alias source fallback | `src/rng.zig`, `compare/results/s4-m1240-rng-init-raw-alias-source.md` | Closed for the current bar: `Rng.init` now accepts raw-alias-only sources and routes facade `next`, `nextU32`, and byte filling through the shared direct-source helpers. |
| S4-M1241 nextU32-only source fallback | `src/source.zig`, `src/rng.zig`, `compare/results/s4-m1241-next-u32-only-source-fallback.md` | Closed for the current bar: nextU32-only sources now feed u64-shaped raw/facade helpers through two little-endian u32 draws while preserving native u32 stream shape. |
| S4-M1243 stable iterator choice semantics | `src/sequence.zig`, `compare/results/s4-m1243-stable-iterator-choice-semantics.md` | Closed for the current bar: iterator `choice` produces the same stream as slice `choice` for the same seed. |
| S4-M1244 polymorphic standard normal/exponential unit structs | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1244-polymorphic-standard-normal-exponential.md` | Closed for the current bar: `StandardNormal{}` / `StandardExponential{}` with `Exp1` alias provide polymorphic scalar/vector zero-parameter distributions matching `rand_distr` unit-struct patterns. |
| S4-M1245 Rust API parity, N-dimensional geometry, standard Cauchy/Logistic | `src/distributions.zig`, `examples/*.zig`, `docs/api-reference.md`, `compare/results/s4-m1245-rust-api-parity-ndim-geometry-standard-distributions.md` | Closed for the current bar: `Standard` alias, arbitrary-dimensional `UnitSphereSurface{}`/`UnitBallVolume{}`, and `StandardCauchy{}`/`StandardLogistic{}` zero-parameter distributions are added with tests and documentation. |
| S4-M1246 Von Mises circular distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1246-von-mises-circular-directional-distribution.md` | Closed for the current bar: `VonMises(T)` parameterized and `StandardVonMises{}` unit-struct distributions implement exact Best–Fisher (1979) rejection sampling with fast squeeze, extending directional coverage beyond local `rand_distr`. |
| S4-M1247 Wrapped Cauchy circular distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1247-wrapped-cauchy-circular-distribution.md` | Closed for the current bar: `WrappedCauchy(T)` parameterized and `StandardWrappedCauchy{}` unit-struct distributions provide closed-form inverse-CDF sampling supporting both scalar and SIMD vector types without rejection loops, further extending directional coverage. |
| S4-M1248 Dense true-SIMD f64x4 ziggurat | `src/rng.zig`, `compare/results/s4-m1248-dense-simd-f64x4-ziggurat.md` | Closed for the current bar: mask-based lane-rejection SIMD ziggurat (`simdNormalZigguratF64x4`, `simdExponentialZigguratF64x4`) replaces scalar-per-lane unrolling in default f64x4 vector fills, producing exact ziggurat-distributed samples and closing the S4-M11 dense-SIMD blocker for f64x4. |
| S4-M1249 Dense true-SIMD f32x8 native ziggurat | `src/distributions.zig`, `compare/results/s4-m1249-dense-simd-f32x8-native-ziggurat.md` | Closed for the current bar: mask-based lane-rejection SIMD ziggurat (`simdNormalZigguratF32x8`, `simdExponentialZigguratF32x8`) is added for native f32 ziggurat tables and wired into explicit `*NativeF32*` opt-in APIs, delivering near-8x SIMD throughput for native f32 profiles while preserving default f32 bitstream compatibility. |
| S4-M1250 Truncated Normal distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1250-truncated-normal-distribution.md` | Closed for the current bar: `TruncatedNormal(T)` with public normPdf/normCdf/probit mathematical helpers provides deterministic-latency inverse-CDF sampling over one-sided/two-sided/degenerate intervals with exact closed-form moments, a complete 24-function scalar/vector/fill family, and VectorTruncatedNormal SIMD struct; vonMises/wrappedCauchy promoted to full public API. |
| S4-M1251 Von Mises-Fisher spherical directional distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1251-von-mises-fisher-spherical-distribution.md` | Closed for the current bar: `VonMisesFisher(T, n)` (comptime n≥2) implements Wood (1994) Ulrich-transform rejection sampling for n≥3 (Beta-distributed cosine w + uniform equatorial tangent point + Householder reflection to mean direction), S¹ delegation to existing Von Mises for n=2, fast paths for κ=0 (uniform sphere) and κ→∞ (point mass), exact mean-resultant-length for n=2,3 with first-order continued-fraction approximation for higher dimensions, a complete scalar/fill free-function family, initNormalized for caller-guaranteed unit directions, and tests covering 2D/3D/4D behavior, κ=0 uniformity, high-κ degeneracy, arbitrary mean directions, and f32 support; extends directional-statistics coverage from S¹ to arbitrary-dimensional spheres Sⁿ⁻¹ beyond local Rust `rand_distr`. |
| S4-M1252 Watson axial spherical directional distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1252-watson-axial-spherical-distribution.md` | Closed for the current bar: `Watson(T, n)` (comptime n≥2) implements Beta((n-1)/2,(n-1)/2) rejection sampling with log-ratio envelope tests for the cosine w coordinate, uniform Sⁿ⁻² equatorial tangent points, Householder reflection via shared `householderReflectFromE1` helper factored out of vMF, fast paths for κ=0 (uniform), κ>1e12 (±μ bipolar point mass), and κ<−1e12 (equator girdle), and Kummer confluent hypergeometric ₁F₁ series with asymptotic fallbacks for mean cosine-squared moments; supports bipolar (κ>0), girdle (κ<0), and uniform regimes on S¹, S², S³, and higher spheres; 10 tests cover parameter validation, bipolar/girdle concentration, κ=0 uniformity (1/n mean cos²), high-κ degeneracies, 2D/3D/4D cases, f32 support, and kummerM numerical sanity; extends directional-statistics coverage to the antipodally symmetric (axial) Watson family, absent from local Rust `rand_distr`. |
| S4-M1253 Rice (Rician) fading distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1253-rice-rician-distribution.md` | Closed for the current bar: `Rice(T)` provides rejection-free sampling √((N(ν,σ²))²+(N(0,σ²))²) for the magnitude of an offset 2D Gaussian vector (noncentral chi with 2 DOF), a high-impact distribution for Rician fading channels, MRI Rician noise, radar, and optics, plus internal `besselI0`/`logBesselI0` helpers (Abramowitz & Stegun approximations ~1e-7 accuracy) reusing existing `besselI1Ratio`, K-factor accessor K=ν²/(2σ²), exact mean via (1+2t)·I₀(t)+2t·I₁(t) closed-form with log-exp overflow safety, variance/mode/min/max accessors, fast ν=0→Rayleigh(σ) and σ=0→ν point-mass paths, a complete scalar/fill free-function family, and 8 new tests covering parameter validation, Rayleigh moment match, point-mass behavior, non-negativity, high-SNR concentration, free-function/fill, f32 support, and besselI0 numerical sanity (I₀(0)=1, I₀(1)≈1.266, evenness, large-z finiteness); absent from local Rust `rand_distr`. |
| S4-M1254 Nakagami-m fading distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1254-nakagami-m-fading-distribution.md` | Closed for the current bar: `Nakagami(T)` provides rejection-free sampling √(Gamma(m, Ω/m)) for the Nakagami-m multipath fading envelope — the canonical RF/wireless fading model with shape m ≥ 0.5 (m=1 Rayleigh, m=0.5 half-normal/severe fading, m>1 approximates Rician, m→∞ Gaussian), spread Ω = E[X²], exact mean (Γ(m+1/2)/Γ(m))·√(Ω/m) via `std.math.lgamma` in log-space with m=1→√(πΩ)/2 fast path, variance Ω·(1 − ratio²/m) with m=1 fast path Ω·(4−π)/4, mode √(Ω·(m−1)/m) for m≥1 (0 for m<1) with m=1→√(Ω/2) Rayleigh mode, equivalentRayleighScale accessor √(Ω/2), Ω=0 point-mass fast path, a complete scalar/fill free-function family, and 8 new tests covering parameter validation (m<0.5 rejection, Ω<0 rejection, NaN/Inf, m=0.5 boundary accepted), m=1 Rayleigh moment match, Ω=0 point mass, non-negativity across 6 shape values, m=0.5 half-normal moments with 5000-sample Monte Carlo verification (mean/variance within sampling tolerance), large-m (m=50) concentration near √Ω with variance bound and Monte Carlo check, free-function/fill correctness with error rejection, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1255 Inverse Gamma distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1255-inverse-gamma-distribution.md` | Closed for the current bar: `InverseGamma(T)` provides rejection-free sampling 1/Gamma(α, 1/β) for the Inverse Gamma distribution — the conjugate prior for normal variance in Bayesian statistics, used ubiquitously for variance priors in regression, Gaussian processes, hierarchical models, MCMC, and finance (inverse volatility) — with shape α > 0, scale β > 0, PDF f(x) = β^α/Γ(α) · x^{−(α+1)} · exp(−β/x), mean β/(α−1) for α>1 (+∞ for α≤1), variance β²/((α−1)²(α−2)) for α>2 (+∞ for α≤2), mode β/(α+1) for all α>0, a complete scalar/fill free-function family, and 8 new tests covering parameter validation (α≤0/β≤0/NaN/Inf rejection), analytic moments for IG(3,2) (mean=1, variance=1, mode=0.5) and IG(5,4) (mean=1, variance=1/3, mode=2/3), infinite mean/variance for small shape (α=0.5 both infinite, α=1.5 finite mean infinite variance), positivity/finiteness of 500 samples, reciprocal-Gamma identity via 2000-sample E[1/X] ≈ α/β = 1.5 within 0.1, 5000-sample Monte Carlo mean/variance for IG(5,4) within sampling tolerance, free-function/fill correctness with error rejection, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1256 Exponentially Modified Gaussian (ExGaussian) distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1256-exgaussian-distribution.md` | Closed for the current bar: `ExGaussian(T)` provides rejection-free compositional sampling X = μ + σ·Z + E (Z~N(0,1), E~Exp(1/τ)) for the Exponentially Modified Gaussian — the sum of a normal N(μ,σ²) and an exponential with mean τ>0, the canonical response-time distribution in cognitive psychology (ex-Gaussian RT model) also used for chromatography peak shapes and inter-arrival times with additive Gaussian noise — with exact analytic moments: mean μ+τ, variance σ²+τ², standard deviation √(σ²+τ²), skewness 2τ³/(σ²+τ²)^{3/2} (always right-skewed), rateValue = 1/τ, minValue = −∞, σ=0 accepted as a shifted exponential (all samples ≥ μ), mode returns μ as a stable central reference (exact mode is a transcendental root), a complete scalar/fill free-function family, and 8 new tests covering parameter validation (τ≤0/σ<0/NaN/Inf rejection, σ=0 accepted), analytic moments for (μ=10,σ=2,τ=3) (mean=13, var=13, sd=√13, skew≈1.152) and σ=0 pure exponential (mean=1, var=1, skew=2), rate reciprocity, finiteness of 500 samples, σ=0 shifted exponential lower bound (all samples ≥ μ), 5000-sample Monte Carlo mean ≈ 13 within 0.15 and variance ≈ 13 within 0.3, free-function/fill correctness with error rejection, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1257 Generalized Pareto distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1257-generalized-pareto-distribution.md` | Closed for the current bar: `GeneralizedPareto(T)` provides rejection-free inverse-CDF sampling for the three-regime Generalized Pareto Distribution (GPD) with location μ, scale σ>0, shape ξ (tail index) — the canonical peaks-over-threshold model in extreme value theory used in hydrology, finance/VaR, insurance, and environmental risk — covering ξ=0 (exponential), ξ>0 (heavy Pareto-like tail with infinite mean for ξ≥1 and infinite variance for ξ≥0.5), and ξ<0 (bounded short tail on [μ, μ−σ/ξ]); inverse-CDF sampling X=μ+σ·(U^{−ξ}−1)/ξ (ξ≠0) or X=μ−σ·lnU (ξ=0) is O(1) and rejection-free; moments: mean μ+σ/(1−ξ) for ξ<1, variance σ²/((1−ξ)²(1−2ξ)) for ξ<1/2, median μ+σ·ln2 for ξ=0 / μ+σ·(2^ξ−1)/ξ otherwise, mode=μ; a complete scalar/fill free-function family and 8 new tests cover parameter validation, analytic moments across all three regimes (including infinite mean/variance cases), support containment, 5000-sample Monte Carlo for ξ=0 exponential, free-function/fill correctness, and f32 support; extends beyond local Rust `rand_distr` which provides classical Pareto (Type I) but not the three-regime GPD. |
| S4-M1258 Scaled Inverse Chi-Squared distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1258-scaled-inverse-chi-squared-distribution.md` | Closed for the current bar: `ScaledInverseChiSquared(T)` provides the Gelman BDA/BUGS/Stan parameterization of the conjugate prior for normal variance as a thin adapter over InverseGamma (X~ScaledInvChiSq(ν,τ²) ⟺ X~InverseGamma(ν/2, ντ²/2)), with degrees of freedom ν>0, squared scale τ²>0, mean ντ²/(ν−2) for ν>2 (infinite otherwise), variance 2ν²τ⁴/((ν−2)²(ν−4)) for ν>4 (infinite otherwise), mode ντ²/(ν+2), a complete scalar/fill free-function family delegating to the existing inverseGammaPointFrom helper, and 6 new tests covering parameter validation, analytic moments matching InverseGamma identity, infinite mean/variance for small ν, positivity/finiteness, 5000-sample Monte Carlo mean, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr` (which provides InverseGamma but not this parameterization adapter). |
| S4-M1259 Hoyt (Nakagami-q) fading distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1259-hoyt-nakagami-q-fading-distribution.md` | Closed for the current bar: `Hoyt(T)` provides rejection-free two-Gaussian product sampling X=σ·√(Z₁²+(q·Z₂)²) with σ=√(Ω/(1+q²)) for the Hoyt (Nakagami-q) fading distribution — the complementary multipath fading model to Nakagami-m for channels with unequal scattered-power components, with fading parameter q∈(0,1] (q=1→Rayleigh(σ) exactly, q→0→severe near-single-path fading), spread Ω=E[X²]>0, exact mean via Bessel form σ·√(π/2)·(1+q)/2·exp(K)·(I₀(K)+I₁(K)) where K=(1−q)²/(4q), variance Ω−(E[X])², mode σ·q; adds a new standalone `besselI1(z)` (Abramowitz & Stegun polynomial/asymptotic approximations ~1e-7 accuracy) alongside existing besselI0; a complete scalar/fill free-function family and 6 new tests cover parameter validation, q=1 exact Rayleigh moment equivalence, non-negativity across q∈{0.2,0.5,0.8,1.0}, Ω=0 point mass, 5000-sample Monte Carlo for q=1 Rayleigh within sampling tolerance, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1260 Noncentral Chi-Squared distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1260-noncentral-chi-squared-distribution.md` | Closed for the current bar: `NoncentralChiSquared(T)` provides rejection-free Poisson-mixture sampling for χ'²(k,λ) with dof k≥0, noncentrality λ≥0 — the fundamental noncentral distribution for chi-squared/ANOVA power analysis, Rician fading power, radar, and communications — using K~Poisson(λ/2), X~χ²(k+2K) composed from existing Poisson and ChiSquared samplers; λ=0 reduces to central ChiSquared(k); closed-form moments E[X]=k+λ, Var=2(k+2λ), mode=max(0,k+λ−2); a complete scalar/fill free-function family and 7 new tests cover parameter validation, analytic moments (including λ=0 reduction to central χ² and k=0 case), non-negativity/finiteness, k=0 and k=3,λ=4 5000-sample Monte Carlo mean/variance, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1261 Noncentral t distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1261-noncentral-t-distribution.md` | Closed for the current bar: `NoncentralT(T)` provides rejection-free ratio sampling (Z+μ)/√(V/ν) for t'(ν,μ) with dof ν>0, noncentrality μ — the canonical distribution for one-sample/two-sample t-test power analysis and detection theory — composing existing Normal and ChiSquared samplers; ν=∞ reduces to N(μ,1); μ=0 reduces to central StudentT; moments use a Γ-function ratio c(ν)=√(ν/2)·Γ((ν−1)/2)/Γ(ν/2) via std.math.lgamma with infinite mean for ν≤1 and infinite variance for ν≤2; a complete scalar/fill free-function family and 7 new tests cover parameter validation, μ=0 central-t moment reductions (null mean for ν≤1, null variance for ν≤2), ν=∞ normal limit, finiteness, 5000-sample Monte Carlo for ν=∞ and μ=0,ν=10 cases, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1262 Noncentral F distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1262-noncentral-f-distribution.md` | Closed for the current bar: `NoncentralF(T)` provides rejection-free ratio sampling (X/d1)/(Y/d2) where X~χ'²(d1,λ), Y~χ²(d2) for F'(d1,d2,λ) with numerator df d1>0, denominator df d2>0, noncentrality λ≥0 — the canonical distribution for ANOVA/regression F-test power analysis under an alternative hypothesis; λ=0 reduces exactly to central FisherF; closed-form moments E[F]=d2/(d2−2)·(d1+λ)/d1 for d2>2, Var[F]=2·((d1+λ)²+(d1+2λ)(d2−2))·d2²/(d1·(d2−2)²·(d2−4)) for d2>4; a complete scalar/fill free-function family and 6 new tests cover parameter validation, λ=0 reduction to FisherF moments (null mean for d2≤2, null variance for d2≤4), non-negativity/finiteness, 5000-sample Monte Carlo for λ=0 and λ>0 cases, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr`. |
| S4-M1263 Noncentral Chi distribution | `src/distributions.zig`, `docs/api-reference.md`, `compare/results/s4-m1263-noncentral-chi-distribution.md` | Closed for the current bar: `NoncentralChi(T)` provides rejection-free sqrt-of-noncentral-chi-squared sampling for χ'(k,λ) with dof k≥0, noncentrality λ≥0 — the generalized Rician envelope for arbitrary dof used in communications, radar, and directional statistics; λ=0 reduces exactly to central Chi(k); k=2,λ=ν² gives Rice; k=1,λ=μ² gives folded normal |μ|; expected value uses confluent hypergeometric ₁F₁ (reusing the existing `kummerM` helper) combined with Γ-function ratios E[X]=√2·Γ((k+1)/2)/Γ(k/2)·exp(−λ/2)·₁F₁(1/2;k/2;λ/2) with a folded-normal k=0 branch using erfAbramowitzStegun; variance = k+λ−(E[X])²; a complete scalar/fill free-function family and 6 new tests cover parameter validation, λ=0 reduction to central Chi moments (E[χ₅] matches Chi.expectedValue, k=2 Rayleigh mean=√(π/2)), k=2,λ=ν² Rice non-negativity/variance consistency, non-negativity/finiteness, 5000-sample Monte Carlo second moment=k+λ, free-function/fill correctness, and f32 support; absent from local Rust `rand_distr`. |
| No proxy signal is accepted as whole-goal completion | `zig build validate-all` plus roadmap/audit files | Validation passes are necessary but not sufficient; the goal remains active for the next raised product bar (Bingham/Matrix-vMF/Kent directional distributions, Noncentral/GeneralizedPareto/Hoyt/ScaledInvChiSq/ExGaussian/Rice/Nakagami/InverseGamma vector/SIMD sampling, copula methods, SIMD spherical bulk sampling, weighted/alias advances, string generation expansion, longer validation, broader runtime, or newly discovered gaps). |
...
dense kernel wins in the real `vectorbench` slice-fill harness. The roadmap raised the next product bar to S4-M1124, which is closed by
portable oversized-u32 test guards and a passing `validate-all`; S4-M1125 then
refreshes the local status snapshots, S4-M1126 records f32x8 probe evidence, and
S4-M1127 lands a direct-source f64x4 standard-normal fill specialization and
S4-M1128 lands the adjacent direct-source f64x4 standard-exponential fill
specialization, and S4-M1129 refreshes status after those direct-fill closures, and S4-M1130
refreshes full validate-all evidence, and S4-M1131 refreshes status after that
validation, and S4-M1132 records refreshed f32x8 direct-source negative evidence, and S4-M1133
routes rate-one vector exponential fills through standard-exponential fills, and
S4-M1134 applies the same delegation to single-vector helpers, and S4-M1135
...
path drift, S4-M1140 refreshes status after that fix, and S4-M1141 extends f64x4 facade standard vector fills, S4-M1142 extends parameterized f64x4 vector fills, S4-M1143 aligns zero-rate exponential semantics, S4-M1144 aligns negative normal/log-normal stddev semantics, S4-M1145 aligns unrestricted normal/log-normal log-space mean semantics, S4-M1146 aligns mean/CV edge semantics, S4-M1147 aligns Gamma-family infinity semantics, S4-M1148 aligns FisherF infinity semantics, S4-M1149 aligns StudentT infinity semantics, S4-M1150 aligns Cauchy non-finite parameter semantics, S4-M1151 aligns Pareto/Weibull infinite-scale semantics, S4-M1152 aligns Beta infinity semantics, S4-M1153 aligns Triangular non-finite bound semantics, and S4-M1154 aligns PERT infinite-shape semantics, S4-M1155 aligns Poisson max-lambda semantics, S4-M1156 aligns Geometric zero-probability semantics, S4-M1157 aligns InverseGaussian infinity semantics, S4-M1158 aligns SkewNormal unrestricted-location semantics, S4-M1159 aligns NormalInverseGaussian alpha-infinity semantics, S4-M1160 aligns Hypergeometric large-population semantics, S4-M1161 aligns Dirichlet subnormal-alpha semantics, S4-M1162 aligns Beta/Dirichlet tiny-shape semantics, S4-M1163 aligns AliasTable max-weight semantics, S4-M1164 aligns WeightedTree zero-total checked sampling semantics, S4-M1165 aligns WeightedIntTree integer overflow diagnostics, S4-M1166 refreshes full validate-all evidence, and S4-M1167 adds weighted sampler clone/equality helpers, and S4-M1168 adds weighted sampler format helpers, and S4-M1169 adds weighted-tree constructor/default helpers, and S4-M1170 adds weighted-tree trySample aliases, and S4-M1171 refreshes validate-all evidence, and S4-M1172 adds weighted iterator clone/format helpers, and S4-M1173 refreshes validate-all evidence, and S4-M1174 adds weighted error message helpers, and S4-M1175 refreshes validate-all evidence, and S4-M1176 adds root/prelude weighted error aliases, and S4-M1177 refreshes validate-all evidence, and S4-M1178 refreshes weighted manifest coverage, and S4-M1179 refreshes validate-local evidence, and S4-M1180 adds typed static weighted diagnostics, and S4-M1181 refreshes validate-all evidence, and S4-M1182 refreshes weighted public-surface manifests, and S4-M1183 extends typed diagnostics through WeightedChoice, and S4-M1184 refreshes validate-all evidence, and S4-M1185 refreshes dense SIMD vectorbench evidence, and S4-M1186 refreshes validate-local evidence, and S4-M1187 adds dynamic weighted-tree typed diagnostics, and S4-M1188 refreshes validate-all evidence after that dynamic weighted-tree API change, and S4-M1189 adds dynamic weighted-tree typed total diagnostics, and S4-M1190 refreshes validate-all evidence after that change, and S4-M1191 repairs rand-status post-bar drift, and S4-M1192 repairs follow-up rand-status post-bar drift, and S4-M1193 refreshes validate-local evidence after those status repairs, and S4-M1194 adds weighted namespace aliases for local rand_distr discovery names, and S4-M1195 adds root weighted sampler aliases plus prelude weighted sampler aliases, and S4-M1196 refreshes weighted manifest root-alias evidence, and S4-M1197 exposes Poisson max-lambda public constants, and S4-M1198 refreshes validate-all evidence after that public API change, and S4-M1199 expands ziggurat-table surface guards for public const/static names, and S4-M1200 refreshes validate-all evidence after that verifier change, and S4-M1201 refreshes f32x8 vectorbench evidence, and S4-M1202 refreshes f64x4 vectorbench evidence, and S4-M1203 refreshes parameterized vectorbench evidence, S4-M1204 repairs vectorbench status drift, S4-M1205 refreshes validate-local evidence, S4-M1206 adds the roadmap evidence-path guard, S4-M1207 refreshes validate-all evidence after that guard, S4-M1208 refreshes validate-local evidence after the full validation refresh, S4-M1209 refreshes validate-all evidence after the local comparison update, S4-M1210 refreshes inverse-CDF tail vectorbench evidence, S4-M1211 refreshes validate-local evidence after the dense-SIMD evidence refresh, S4-M1212 refreshes validate-all evidence after the local comparison update, S4-M1213 refreshes validate-all evidence after the S4-M1212 status update, and S4-M1214 refreshes exponential vectorbench evidence, and S4-M1215 refreshes validate-local evidence after the exponential vectorbench probe, and S4-M1216 refreshes validate-all evidence after the local comparison update, and S4-M1217 refreshes minimum vectorbench evidence, and S4-M1218 refreshes validate-local evidence after the vectorbench gate, and S4-M1219 refreshes validate-all evidence after the local comparison update, S4-M1223 fixes ordinary f64 StandardUniform 53-bit grid consistency, S4-M1224 recovers that path's throughput, S4-M1225 improves the low-bit/vector8 hot path without changing the grid, S4-M1226 refactors core float fill lane stores, S4-M1227 refactors distribution lane stores, S4-M1228 completes the remaining distribution transform lane-store sweep, S4-M1229 completes the vector sampler lane-count sweep, S4-M1230 refactors the rng normal affine lane-store helper, S4-M1231 hardens rng byte-fill / reader refill correctness, S4-M1232 extends that hardening to owned byte allocation, S4-M1233 fixes `std.Random` adapter `nextU32` byte shape, S4-M1234 hardens root string preallocation before entropy, S4-M1235 aligns fallible direct-source raw-u32 fallback behavior, S4-M1236 aligns direct-source native-u64 fallback behavior, S4-M1237 aligns direct-source byte-helper fallback behavior, S4-M1238 aligns generic direct-source native-u64 fallback behavior, S4-M1239 aligns seed/fork native-u64 fallback behavior, S4-M1240 aligns `Rng.init` raw-alias source fallback behavior, and S4-M1241 aligns nextU32-only direct-source fallback behavior, so the next active
product bar is S4-M1242.
...
preserving stream shape, current S4-M1148 infinite-degree NaN/draw-shape behavior, and
zero-length checked fill semantics while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1009 additionally routes top-level scalar/vector FisherF facade helpers
through reusable facade samplers, preserving stream shape, current S4-M1148 infinite-degree
NaN/draw-shape behavior, and zero-length checked fill semantics while
avoiding `From` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
...
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1027 additionally routes reusable scalar/vector Pert facade sample/fill
helpers through direct cached-Beta transform sampling, preserving stream shape,
degenerate point-mass no-consume behavior, and zero-length checked fill semantics
while avoiding `sampleFrom` / `fillFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
...
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1029 additionally routes reusable scalar/vector NormalInverseGaussian facade
sample/fill helpers through direct inverse-Gaussian and standard-normal component
sampling, preserving stream shape, degenerate point-mass no-consume behavior, and
zero-length checked fill semantics while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
...
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1032 additionally routes reusable MappedSampler facade sample/fill helpers
through direct wrapped-sampler facade sampling plus mapper application, preserving
mapped output semantics while avoiding `sampleFrom` / `fillFrom` wrapper aliases;
it is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1033 additionally routes distribution-layer reusable Choose pointer fills
through direct facade index mapping, preserving stream shape and empty/singleton
behavior while avoiding `fillFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1034 additionally routes reusable scalar Normal facade fills through direct
facade `Rng.fillNormal` calls, preserving stream shape and degenerate no-consume
behavior while avoiding `fillFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1035 additionally routes reusable scalar/vector native-f32 Normal facade
sample/fill helpers through direct native-f32 facade helpers, preserving
snapshot-sensitive output semantics while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1036 additionally routes reusable vector table Normal facade sample/fill
helpers through direct table-normal facade helpers, preserving snapshot-sensitive
output semantics while avoiding `sampleFrom` / `fillFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1037 additionally routes reusable VectorNormal facade fills through direct
facade vector-normal fill helpers, preserving stream shape and degenerate
...
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1039 additionally routes reusable scalar/vector native-f32 Exponential facade
sample/fill helpers through direct native-f32 standard-exponential facade helpers,
...
S4-M11 or complete the long-term objective.
S4-M1040 additionally routes reusable vector approximate-log f32 Exponential
facade sample/fill helpers through direct approximate-log facade helpers,
preserving snapshot-sensitive output semantics while avoiding `sampleFrom` /
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
...
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1042 additionally routes reusable generic VectorExponential facade sample/fill
helpers through direct vector exponential facade helpers, preserving stream shape
and degenerate no-consume behavior while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1043 additionally routes reusable scalar LogNormal facade sample/fill helpers
through direct log-normal facade helpers, preserving stream shape and degenerate
...
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
S4-M1045 additionally routes reusable scalar/vector native-f32 LogNormal facade
sample/fill helpers through direct native-f32 log-normal facade helpers,
preserving snapshot-sensitive output semantics while avoiding `sampleFrom` /
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
S4-M1046 additionally routes reusable LogNormalLibmvec facade sample/fill helpers
through direct loaded-libmvec refills from facade `Rng`, preserving availability
checks, buffer semantics, and degenerate no-consume behavior while avoiding
`sampleFrom` / `fillFrom` wrapper aliases; it is reliability/ergonomics work and
does not resolve S4-M11 or complete the long-term objective.
S4-M1047 additionally routes reusable LogNormalDlsymExp facade sample/fill
helpers through direct loaded-libm refills from facade `Rng`, preserving
availability checks, buffer semantics, and degenerate no-consume behavior while
avoiding `sampleFrom` / `fillFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1048 additionally routes reusable scalar/vector native-exp2-f32 LogNormal
facade sample/fill helpers through direct native-exp2 log-normal facade helpers,
preserving snapshot-sensitive output semantics while avoiding `sampleFrom` /
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
S4-M1049 additionally routes reusable generic VectorLogNormal facade sample/fill
helpers through direct vector log-normal facade helpers, preserving stream shape
and degenerate no-consume behavior while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1050 additionally routes reusable scalar approximate-f32 LogNormal facade
sample/fill helpers through direct approximate-f32 log-normal facade helpers,
preserving snapshot-sensitive output semantics while avoiding `sampleFrom` /
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
S4-M1051 additionally routes reusable scalar exp2-f32 LogNormal facade
sample/fill helpers through direct exp2-f32 log-normal facade helpers, preserving
snapshot-sensitive output semantics while avoiding `sampleFrom` / `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1052 additionally routes reusable vector exp2-f32 LogNormal facade
sample/fill helpers through direct vector exp2-f32 log-normal facade helpers,
preserving snapshot-sensitive output semantics while avoiding `sampleFrom` /
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not resolve
S4-M11 or complete the long-term objective.
S4-M1053 additionally routes reusable vector approximate-f32 LogNormal facade
sample/fill helpers through direct vector approximate-f32 log-normal facade
...
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1055 additionally routes reusable AliasTable facade fills through direct
cached alias-table sampling, preserving stream shape, zero-length behavior, and
single-positive no-consume behavior while avoiding `fillFrom` wrapper aliases; it
is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1056 additionally routes sequence-layer reusable Choice pointer fills through
direct facade index mapping, preserving stream shape and empty/singleton behavior
while avoiding `fillFrom` wrapper aliases; it is reliability/ergonomics work and
does not resolve S4-M11 or complete the long-term objective.
S4-M1057 additionally routes sequence-layer reusable WeightedChoice pointer fills
through direct facade alias-index mapping, preserving stream shape and
empty/single-positive behavior while avoiding `fillFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the long-term
objective.
S4-M1058 additionally routes sequence-layer reusable WeightedChoice owned pointer
batches through direct facade pointer fills after allocation, preserving stream
shape, ownership, and single-positive behavior while avoiding `fillFrom` wrapper
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1059 additionally routes sequence-layer reusable WeightedChoice checked
pointer fills through direct facade alias-index sampling, preserving stream shape
and empty/single-positive behavior while avoiding the direct-source sample
wrapper alias; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1060 additionally routes sequence-layer reusable WeightedChoice value fills
through direct facade alias-index sampling, preserving stream shape and
empty/single-positive behavior while avoiding `fillValuesFrom` wrapper aliases;
it is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1061 additionally routes sequence-layer reusable WeightedChoice checked value
fills through direct facade alias-index sampling, preserving stream shape,
...
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1063 additionally routes sequence-layer reusable WeightedChoice checked u32
index sampling through the AliasTable checked u32 facade sample directly,
preserving stream shape, population validation, and single-positive behavior
while avoiding `sampleU32CheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1064 additionally routes sequence-layer reusable WeightedChoice checked value
sampling through the AliasTable checked facade sample directly, preserving stream
shape, empty-enum validation, and single-positive behavior while avoiding
`sampleCheckedFrom` wrapper aliases; it is reliability/ergonomics work and does
not resolve S4-M11 or complete the long-term objective.
S4-M1065 additionally routes sequence-layer reusable WeightedChoice index fills
through the AliasTable fill facade directly, preserving stream shape,
empty-output behavior, and single-positive behavior while avoiding
`fillIndicesFrom` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
S4-M1066 additionally routes sequence-layer reusable WeightedChoice checked index
fills through the AliasTable fill facade directly, preserving stream shape,
empty-output behavior, and single-positive behavior while avoiding `fillFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1067 additionally routes reusable AliasTable checked u32 fills through direct
facade cached-alias sampling, preserving stream shape, u32 population validation,
...
is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1069 additionally routes sequence-layer reusable WeightedChoice u32 index
fills through the AliasTable checked u32 fill facade directly, preserving stream
shape, population validation, empty-output behavior, and single-positive behavior
while avoiding `fillU32CheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1070 additionally routes sequence-layer reusable WeightedChoice checked u32
index fills through the AliasTable checked u32 fill facade directly, preserving
stream shape, population validation, empty-output behavior, and single-positive
behavior while avoiding `fillU32CheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1071 additionally routes sequence-layer reusable WeightedChoice owned
iterator facade construction directly, preserving iterator stream shape and
...
and does not resolve S4-M11 or complete the long-term objective.
S4-M1073 additionally routes top-level sequence partialShuffle through direct
facade range sampling, preserving stream shape and zero-count behavior while
avoiding `partialShuffleFrom` wrapper aliases; it is reliability/ergonomics work
and does not resolve S4-M11 or complete the long-term objective.
S4-M1074 additionally routes top-level sequence partialShuffleChecked through
validation plus the direct facade partialShuffle path, preserving stream shape,
...
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1076 additionally routes top-level sequence partialShuffleSplitChecked
through validation plus the direct facade partialShuffleSplit path, preserving
stream shape, invalid-count no-consume validation, selected/rest partitioning,
and zero-count behavior while avoiding `partialShuffleSplitCheckedFrom` wrapper
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1077 additionally routes top-level sequence partialShuffleTail through direct
facade range sampling, preserving stream shape and zero-count behavior while
avoiding `partialShuffleTailFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1078 additionally routes top-level sequence partialShuffleTailChecked through
validation plus the direct facade partialShuffleTail path, preserving stream
shape, invalid-count no-consume validation, tail-selected semantics, and
...
and does not resolve S4-M11 or complete the long-term objective.
S4-M1080 additionally routes top-level sequence partialShuffleTailSplitChecked
through validation plus the direct facade partialShuffleTailSplit path,
preserving stream shape, invalid-count no-consume validation,
selected-tail/rest-prefix partitioning, and zero-count behavior while avoiding
`partialShuffleTailSplitCheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1081 additionally routes top-level sequence reservoirSampleInto through a
direct facade reservoir loop, preserving stream shape, invalid-count no-consume
validation, empty-output behavior, and empty-value validation while avoiding
`reservoirSampleIntoFrom` wrapper aliases; it is reliability/ergonomics work and
does not resolve S4-M11 or complete the long-term objective.
S4-M1082 additionally routes top-level sequence reservoirSample through direct
facade allocation plus reservoirSampleInto filling, preserving stream shape,
...
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1084 additionally routes top-level sequence reservoirSamplePtrsInto through a
direct facade pointer reservoir loop, preserving stream shape, invalid-count
...
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1086 additionally routes top-level sequence reservoirSamplePtrsChecked
through validation plus the direct facade reservoirSamplePtrs path, preserving
stream shape, invalid-count no-consume validation, allocation ownership,
empty-output behavior, and pointer identity while avoiding
`reservoirSamplePtrsCheckedFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1087 additionally routes top-level sequence reservoirSampleMutPtrsInto
through a direct facade mutable-pointer reservoir loop, preserving stream shape,
invalid-count no-consume validation, empty-output behavior, and mutable pointer
identity while avoiding `reservoirSampleMutPtrsIntoFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1088 additionally routes top-level sequence reservoirSampleMutPtrs through
direct facade allocation plus reservoirSampleMutPtrsInto filling, preserving
stream shape, allocation ownership, empty-output behavior, and mutable pointer
...
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1090 additionally routes reusable sequence Choice value fills through direct
facade index sampling, preserving stream shape, empty-output behavior,
empty-value behavior, and singleton no-consume behavior while avoiding
`fillValuesFrom` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
S4-M1091 additionally routes reusable sequence Choice index fills through direct
facade index sampling, preserving stream shape, empty-output behavior, and
...
`fillValuesFrom` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
S4-M1093 additionally routes distribution-layer reusable Choose index fills
through direct facade index sampling, preserving stream shape, empty-output
behavior, and singleton no-consume behavior while avoiding `fillIndicesFrom`
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1094 additionally routes reusable ASCII Charset byte fills through direct
facade charset-index sampling, preserving stream shape, caller-owned output
behavior, and singleton no-consume behavior while avoiding `fillFrom` wrapper
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1095 additionally routes reusable UnicodeCharset scalar fills through direct
facade scalar-index sampling, preserving stream shape, caller-owned output
behavior, validation assumptions, and singleton no-consume behavior while avoiding
`fillFrom` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
S4-M1096 additionally routes reusable ASCII Charset scalar-byte sampling through
direct facade charset-index sampling, preserving stream shape, scalar output
behavior, and singleton no-consume behavior while avoiding `sampleFrom` wrapper
aliases; it is reliability/ergonomics work and does not resolve S4-M11 or
complete the long-term objective.
S4-M1097 additionally routes reusable ASCII Charset checked scalar-byte sampling
through validation plus the direct sample facade path, preserving stream shape,
empty validation, scalar output behavior, and singleton no-consume behavior while
avoiding `sampleCheckedFrom` wrapper aliases; it is reliability/ergonomics work
and does not resolve S4-M11 or complete the long-term objective.
S4-M1098 additionally routes reusable UnicodeCharset scalar sampling through
direct facade scalar-index sampling, preserving stream shape, scalar output
behavior, validation assumptions, and singleton no-consume behavior while avoiding
`sampleFrom` wrapper aliases; it is reliability/ergonomics work and does not
resolve S4-M11 or complete the long-term objective.
S4-M1099 additionally routes reusable UnicodeCharset checked scalar sampling
through validation plus the direct sample facade path, preserving stream shape,
empty/invalid validation, scalar output behavior, and singleton no-consume
behavior while avoiding `sampleCheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1100 additionally routes reusable ASCII Charset checked byte fills through
validation plus the direct fill facade path, preserving stream shape, empty
validation, caller-owned output behavior, and singleton no-consume behavior while
avoiding `fillCheckedFrom` wrapper aliases; it is reliability/ergonomics work and
does not resolve S4-M11 or complete the long-term objective.
S4-M1101 additionally routes reusable UnicodeCharset checked scalar fills through
...
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1103 additionally routes reusable ASCII Charset checked allocation-returning
byte batches through validation plus the direct alloc facade path, preserving
stream shape, allocation ownership, empty validation, zero-length behavior, and
singleton no-consume behavior while avoiding `allocCheckedFrom` wrapper aliases;
it is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1104 additionally routes reusable ASCII Charset allocation-returning strings
through the direct alloc facade, preserving stream shape, allocation ownership,
...
is reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1106 additionally routes reusable UnicodeCharset allocation-returning UTF-8
strings through a direct facade allocation/encoding path, preserving stream
shape, allocation ownership, empty/invalid validation, zero-length behavior,
UTF-8 encoding, and singleton no-consume behavior while avoiding
`sampleStringFrom` wrapper aliases; it is reliability/ergonomics work and does
not resolve S4-M11 or complete the long-term objective.
S4-M1107 additionally routes reusable UnicodeCharset checked
allocation-returning UTF-8 strings through validation plus the direct facade
string path, preserving stream shape, allocation ownership, empty/invalid
validation, zero-length behavior, UTF-8 encoding, and singleton no-consume
behavior while avoiding `sampleStringCheckedFrom` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1108 additionally routes reusable UnicodeCharset caller-owned UTF-8 append
generation through a direct facade reserve/encoding path, preserving stream
shape, caller-owned output behavior, empty/invalid validation, zero-length
behavior, UTF-8 encoding, and singleton no-consume behavior while avoiding
`appendStringFrom` wrapper aliases; it is reliability/ergonomics work and does
not resolve S4-M11 or complete the long-term objective.
S4-M1109 additionally routes reusable UnicodeCharset checked caller-owned UTF-8
append generation through validation plus the direct facade append path,
preserving stream shape, caller-owned output behavior, empty/invalid validation,
zero-length behavior, UTF-8 encoding, and singleton no-consume behavior while
avoiding `appendStringCheckedFrom` wrapper aliases; it is reliability/ergonomics
work and does not resolve S4-M11 or complete the long-term objective.
S4-M1110 additionally routes top-level ASCII alphanumeric byte sampling through
the reusable direct `Charset.sample` facade path, preserving stream shape and
...
resolve S4-M11 or complete the long-term objective.
S4-M1112 additionally routes top-level ASCII allocation-returning sample string
helpers through the reusable direct `Charset.sampleString` facade path,
preserving stream shape, allocation ownership, zero-length behavior,
allocation-failure no-consume behavior, and alphanumeric sampling behavior while
avoiding `sampleStringFrom` wrapper aliases; it is reliability/ergonomics work
and does not resolve S4-M11 or complete the long-term objective.
S4-M1113 additionally routes reusable ASCII Charset caller-owned string append
generation through a direct facade resize/fill path, preserving stream shape,
caller-owned output behavior, empty validation, zero-length behavior,
allocation-failure no-consume behavior, and singleton no-consume behavior while
avoiding `appendStringFrom` wrapper aliases; it is reliability/ergonomics work
and does not resolve S4-M11 or complete the long-term objective.
S4-M1114 additionally routes reusable ASCII Charset checked caller-owned string
append generation through validation plus the direct facade append path,
...
not resolve S4-M11 or complete the long-term objective.
S4-M1117 additionally routes bounded `Rng` Unicode scalar range helpers through
direct facade compressed-range sampling plus surrogate-gap mapping, preserving
stream shape, Unicode scalar validity, invalid-range behavior, and degenerate
...
wrapper aliases; it is reliability/ergonomics work and does not resolve S4-M11
or complete the long-term objective.
S4-M1119 additionally routes `Rng.fillUnicodeScalar` through direct facade
scalar draws, preserving stream shape, caller-owned output behavior, Unicode
scalar validity, and downstream UTF-8 string behavior while avoiding
`fillUnicodeScalarFrom` wrapper aliases; it is reliability/ergonomics work and
does not resolve S4-M11 or complete the long-term objective.
S4-M1120 additionally routes bounded `Rng` Unicode scalar range fills through
direct facade compressed-range sampling plus surrogate-gap mapping, preserving
stream shape, caller-owned output behavior, Unicode scalar validity,
empty-output no-consume behavior, and degenerate no-consume behavior while
avoiding `fillUnicodeScalarRange*From` wrapper aliases; it is
reliability/ergonomics work and does not resolve S4-M11 or complete the
long-term objective.
S4-M1121 additionally routes checked bounded `Rng` Unicode scalar range fills
through validation plus direct facade range fills, preserving stream shape,
caller-owned output behavior, Unicode scalar validity, invalid-range no-consume
...
portability aggregate, S4-M1125 refreshed the status snapshots, S4-M1126
records fresh f32x8 dense SIMD probe evidence, and S4-M1127 lands a
checksum-preserving f64x4 standard-normal direct fill specialization and S4-M1128
lands the adjacent standard-exponential direct fill specialization, and S4-M1129
refreshes status after those closures, and S4-M1130 refreshes full validate-all
evidence, and S4-M1131 refreshes status after that validation, and S4-M1132 records refreshed
f32x8 direct-source negative evidence, and S4-M1133 routes rate-one vector
exponential fills through standard-exponential fills, and S4-M1134 applies the
same delegation to single-vector helpers, and S4-M1135 routes scalar rate-one
...
map drift, S4-M1140 refreshes status again, and S4-M1141 extends f64x4 facade standard vector fills, S4-M1142 extends parameterized f64x4 vector fills, S4-M1143 aligns zero-rate exponential semantics, S4-M1144 aligns negative normal/log-normal stddev semantics, S4-M1145 aligns unrestricted normal/log-normal log-space mean semantics, S4-M1146 aligns mean/CV edge semantics, S4-M1147 aligns Gamma-family infinity semantics, S4-M1148 aligns FisherF infinity semantics, S4-M1149 aligns StudentT infinity semantics, S4-M1150 aligns Cauchy non-finite parameter semantics, S4-M1151 aligns Pareto/Weibull infinite-scale semantics, S4-M1152 aligns Beta infinity semantics, S4-M1153 aligns Triangular non-finite bound semantics, and S4-M1154 aligns PERT infinite-shape semantics, S4-M1155 aligns Poisson max-lambda semantics, S4-M1156 aligns Geometric zero-probability semantics, S4-M1157 aligns InverseGaussian infinity semantics, S4-M1158 aligns SkewNormal unrestricted-location semantics, S4-M1159 aligns NormalInverseGaussian alpha-infinity semantics, S4-M1160 aligns Hypergeometric large-population semantics, S4-M1161 aligns Dirichlet subnormal-alpha semantics, S4-M1162 aligns Beta/Dirichlet tiny-shape semantics, S4-M1163 aligns AliasTable max-weight semantics, S4-M1164 aligns WeightedTree zero-total checked sampling semantics, S4-M1165 aligns WeightedIntTree integer overflow diagnostics, S4-M1166 refreshes full validate-all evidence, and S4-M1167 adds weighted sampler clone/equality helpers, and S4-M1168 adds weighted sampler format helpers, and S4-M1169 adds weighted-tree constructor/default helpers, and S4-M1170 adds weighted-tree trySample aliases, and S4-M1171 refreshes validate-all evidence, and S4-M1172 adds weighted iterator clone/format helpers, and S4-M1173 refreshes validate-all evidence, and S4-M1174 adds weighted error message helpers, and S4-M1175 refreshes validate-all evidence, and S4-M1176 adds root/prelude weighted error aliases, and S4-M1177 refreshes validate-all evidence, and S4-M1178 refreshes weighted manifest coverage, and S4-M1179 refreshes validate-local evidence, and S4-M1180 adds typed static weighted diagnostics, and S4-M1181 refreshes validate-all evidence, and S4-M1182 refreshes weighted public-surface manifests, and S4-M1183 extends typed diagnostics through WeightedChoice, and S4-M1184 refreshes validate-all evidence, and S4-M1185 refreshes dense SIMD vectorbench evidence, and S4-M1186 refreshes validate-local evidence, and S4-M1187 adds dynamic weighted-tree typed diagnostics, and S4-M1188 refreshes validate-all evidence after that dynamic weighted-tree API change, and S4-M1189 adds dynamic weighted-tree typed total diagnostics, and S4-M1190 refreshes validate-all evidence after that change, and S4-M1191 repairs rand-status post-bar drift, and S4-M1192 repairs follow-up rand-status post-bar drift, and S4-M1193 refreshes validate-local evidence after those status repairs, and S4-M1194 adds weighted namespace aliases for local rand_distr discovery names, and S4-M1195 adds root weighted sampler aliases plus prelude weighted sampler aliases, and S4-M1196 refreshes weighted manifest root-alias evidence, and S4-M1197 exposes Poisson max-lambda public constants, and S4-M1198 refreshes validate-all evidence after that public API change, and S4-M1199 expands ziggurat-table surface guards for public const/static names, and S4-M1200 refreshes validate-all evidence after that verifier change, and S4-M1201 refreshes f32x8 vectorbench evidence, and S4-M1202 refreshes f64x4 vectorbench evidence, and S4-M1203 refreshes parameterized vectorbench evidence, S4-M1204 repairs vectorbench status drift, S4-M1205 refreshes validate-local evidence, S4-M1206 adds the roadmap evidence-path guard, S4-M1207 refreshes validate-all evidence after that guard, S4-M1208 refreshes validate-local evidence after the full validation refresh, S4-M1209 refreshes validate-all evidence after the local comparison update, S4-M1210 refreshes inverse-CDF tail vectorbench evidence, S4-M1211 refreshes validate-local evidence after the dense-SIMD evidence refresh, S4-M1212 refreshes validate-all evidence after the local comparison update, S4-M1213 refreshes validate-all evidence after the S4-M1212 status update, and S4-M1214 refreshes exponential vectorbench evidence, and S4-M1215 refreshes validate-local evidence after the exponential vectorbench probe, and S4-M1216 refreshes validate-all evidence after the local comparison update, and S4-M1217 refreshes minimum vectorbench evidence, and S4-M1218 refreshes validate-local evidence after the vectorbench gate, and S4-M1219 refreshes validate-all evidence after the local comparison update, S4-M1223 fixes ordinary f64 StandardUniform 53-bit grid consistency, S4-M1224 recovers that path's throughput, S4-M1225 improves the low-bit/vector8 hot path without changing the grid, S4-M1226 refactors core float fill lane stores, S4-M1227 refactors distribution lane stores, S4-M1228 completes the remaining distribution transform lane-store sweep, S4-M1229 completes the vector sampler lane-count sweep, S4-M1230 refactors the rng normal affine lane-store helper, S4-M1231 hardens rng byte-fill / reader refill correctness, S4-M1232 extends that hardening to owned byte allocation, S4-M1233 fixes `std.Random` adapter `nextU32` byte shape, S4-M1234 hardens root string preallocation before entropy, S4-M1235 aligns fallible direct-source raw-u32 fallback behavior, S4-M1236 aligns direct-source native-u64 fallback behavior, S4-M1237 aligns direct-source byte-helper fallback behavior, S4-M1238 aligns generic direct-source native-u64 fallback behavior, S4-M1239 aligns seed/fork native-u64 fallback behavior, S4-M1240 aligns `Rng.init` raw-alias source fallback behavior, S4-M1241 aligns nextU32-only direct-source fallback behavior, S4-M1242 fixes stable iterator choice semantics, S4-M1243 corrects reservoir sampling implementation, S4-M1244 adds polymorphic standard normal/exponential unit structs, S4-M1245 adds Rust API parity, N-dimensional geometry sampling, and standard Cauchy/Logistic distributions, and S4-M1246 adds the Von Mises circular/directional distribution extending beyond local `rand_distr` core continuous coverage, so the next active
product bar is S4-M1247.

## Required Next Work Before Completion

The goal remains active under the next raised product bar until at least one of these
happens:

- exact/default-compatible dense SIMD normal/exponential: while f64x4 default paths now use true SIMD ziggurat (S4-M1248) and native f32x8 opt-in paths use true SIMD ziggurat (S4-M1249), default f32 paths deliberately retain the f64-ziggurat-cast-to-f32 output mapping for backward compatibility; a default/exact-compatible dense SIMD normal/exponential candidate beats scalar lane-fill in the real vector-slice harness for default f32 only if it preserves the f64-cast bitstream, which the native f32ziggurat deliberately does not (different algorithm, different tables);
- another non-WASI OS/architecture runtime executes the accepted profile long
  sweep or stronger validation;
- additional directional/spherical distributions (e.g., von Mises-Fisher for
  N-spheres, wrapped normal) extend the circular/directional coverage beyond
  the existing S¹ Von Mises and Wrapped Cauchy;
...
Note: S4-M1248 closed the long-standing S4-M11 exact/default dense-SIMD blocker
for f64x4 (default API paths use true mask-rejection SIMD ziggurat), and S4-M1249
extends the same mask-rejection SIMD ziggurat to native f32x8 (opt-in `*NativeF32*`
APIs use 8-lane native f32 SIMD ziggurat while default f32 paths retain f64-cast
bitstream compatibility). Exact/default-compatible dense SIMD for default f32
paths is not a blocker because default f32 semantics are explicitly defined as
f64 ziggurat cast to f32; native f32 SIMD acceleration is available through the
opt-in APIs as documented in the native f32 profile guidance.
...
S4-M1205 is closed after refreshing validate-local evidence in `compare/results/s4-m1205-post-s4-m1204-validate-local.md`.
...
S4-M1209 is closed after refreshing validate-all evidence in `compare/results/s4-m1209-post-s4-m1208-validate-all.md`.

S4-M1210 is closed after refreshing inverse-CDF tail vectorbench evidence in `compare/results/s4-m1210-inverse-cdf-tail-probe.md`.
...
S4-M1215 is closed after refreshing validate-local evidence in `compare/results/s4-m1215-post-s4-m1214-validate-local.md`.

S4-M1216 is closed after refreshing validate-all evidence in `compare/results/s4-m1216-post-s4-m1215-validate-all.md`.
...
S4-M1220 is closed after adding and validating full-covariance multivariate
normal sampling in `compare/results/s4-m1220-multivariate-normal.md`.
...
S4-M1223 is closed after fixing ordinary f64 StandardUniform 53-bit grid consistency in `compare/results/s4-m1223-f64-standard-uniform-grid.md`.

S4-M1224 is closed after recovering ordinary f64 StandardUniform throughput while preserving the S4-M1223 grid in `compare/results/s4-m1224-f64-standard-uniform-throughput.md`.
...
S4-M1226 is closed after refactoring vectorized float fill lane stores in `compare/results/s4-m1226-float-fill-lane-store-refactor.md`.
...
S4-M1229 is closed after completing the vector sampler lane-count sweep in `compare/results/s4-m1229-vector-sampler-lane-count-sweep.md`.
...
S4-M1236 is closed after aligning direct-source native-u64 fallback behavior in `compare/results/s4-m1236-next-u64-native-fallback.md`.
...
S4-M1245 is closed after adding Rust `rand` API parity via the `Standard` alias for `StandardUniform`, implementing generalized N-dimensional unit sphere surface and unit ball volume sampling using Marsaglia's algorithm with correct radial scaling `u^(1/n)`, adding `StandardCauchy{}` and `StandardLogistic{}` polymorphic unit structs matching Rust `rand_distr` API conventions, and adding full SIMD vector support for both new standard distributions (`compare/results/s4-m1245-rust-api-parity-ndim-geometry-standard-distributions.md`).

S4-M1246 is closed after adding the Von Mises circular distribution (the maximum-entropy circular normal distribution on S¹), including parameterized `VonMises(T)` and `StandardVonMises{}` unit struct with exact Best & Fisher (1979) rejection sampling, validating constructor validation, range, κ=0 uniformity, large-κ concentration, and Bessel function ratio properties, extending alea beyond local `rand_distr` core continuous distribution coverage (`compare/results/s4-m1246-von-mises-circular-directional-distribution.md`).

S4-M1247 is closed after adding the Wrapped Cauchy circular distribution (a heavy-tailed circular distribution obtained by wrapping a Cauchy around the unit circle), including parameterized `WrappedCauchy(T)` and `StandardWrappedCauchy{}` unit struct with closed-form inverse-CDF sampling that natively supports both scalar f32/f64 and SIMD vector types without rejection loops, exact variance = 1-ρ, a generic `wrapAngleGeneric` helper for branchless per-lane SIMD angle wrapping, and tests for parameter validation, (-π, π] bounds, ρ=0 uniformity (mean resultant length <0.05), ρ=1 point mass at μ, high-ρ concentration (mean resultant length ~ρ), standard parameters, and SIMD vector lane validity, extending directional-statistics coverage beyond Von Mises and beyond local `rand_distr` (`compare/results/s4-m1247-wrapped-cauchy-circular-distribution.md`).

S4-M1248 remains active after S4-M1247 raises the next correctness/performance/product bar.
