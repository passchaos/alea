# S4-M617 Root Checked Mutable-Pointer Choose Batch Prevalidation

## Gap

Root `choosePtrBatchChecked` allocated its output buffer before validating
...
This milestone aligns checked mutable-pointer choose batch behavior with root
fill helpers and with earlier unchecked batch prevalidation work.
...
- Non-zero empty-item requests return `error.EmptyRange` before allocation or
  entropy.
- Singleton mutable item slices still allocate and fill deterministic repeated
  pointers before entropy is requested.
...
S4-M617 is closed for the current bar: root checked mutable-pointer choose batch
