# S4-M597 Root Weighted Value Batch Prevalidation

## Gap

Root `chooseWeightedBatch` and `chooseWeightedBatchChecked` allocated their output
buffers before checking length mismatches, invalid weights, and deterministic
empty/single weighted-index states. Those failures should be reported before
random-output allocation and before root secure-engine construction.

This milestone aligns the allocation-returning weighted value batch helpers with
the validated weighted-index root helpers.
...
- Empty/all-zero weights still return nullable `null` batches for unchecked
  requests and `error.EmptyRange` for checked requests before entropy.
- Single-positive weights still allocate and fill repeated values before entropy
  is requested.
...
S4-M597 is closed for the current bar: root weighted value batch helpers now
prevalidate deterministic and invalid paths before random-output allocation and
