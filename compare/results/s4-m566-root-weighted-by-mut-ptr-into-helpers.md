# S4-M566 Root Item-Accessor Weighted Mutable-Pointer Into Helpers

## Gap

S4-M565 added root item-accessor weighted const-pointer buffer helpers, but
callers that wanted caller-owned no-replacement weighted `*T` buffers from
mutable item-local weights still had to construct a secure engine manually and
call `seq.sampleWeightedMutPtrsByInto*`.
...
for mutable-pointer buffers without adding Rust trait-shaped API machinery.

## API Added

`src/root.zig` now exposes:

- `sampleWeightedMutPtrsByInto`
- `sampleWeightedMutPtrsByIntoChecked`

Both helpers take a mutable item slice, caller-owned mutable-pointer output
buffer, caller-owned index/key scratch buffers, and a comptime item-weight
accessor. The unchecked helper fills up to `out.len` positive-weight item
pointers and returns the filled count; the checked helper requires enough
positive-weight items to fill the output and rejects insufficient/all-zero
requests with `error.InvalidParameter`.
...
- `examples/basic.zig` demonstrates `byMutPtrInto=` in the dedicated
...
Runnable example excerpt showing the guarded root item-accessor weighted mutable-pointer-into token:

```text
$ zig build run-basic | grep "root weighted by no-replacement helpers"
root weighted by no-replacement helpers: bySample=[green, blue], byInto=[green, blue], byPtrInto=[red, blue], byPtrSample=[blue, red], byMutPtrInto=[blue, green], byMutPtrSample=[blue, green]
```
...
S4-M566 is closed for the current bar: root system-entropy callers can fill
caller-owned no-replacement weighted mutable-pointer buffers directly from a
