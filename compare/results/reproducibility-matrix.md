...
| Table-quantile f32 vector normal profile | `VectorStandardNormalTableF32` / `VectorNormalTableF32` intentionally use midpoint quantile-table lookup for f32 vector lanes. They are vector-only, discrete/truncated throughput opt-ins with checked snapshot evidence and do not match default ziggurat or native-f32 normal output mappings. |
