...
| `src/rng.zig`, `src/seq.zig`, `src/distributions.zig`: invalid checked/no-consume tests | Invalid checked/error paths return before consuming randomness, including nested empty-enum `valueCheckedFrom` cases and empty/all-zero weighted iterator choices |
