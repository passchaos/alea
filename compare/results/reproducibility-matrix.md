...
| Native f32 standard distribution profiles | `StandardNormalNativeF32` / `VectorStandardNormalNativeF32` / `NormalNativeF32` and `StandardExponentialNativeF32` / `VectorStandardExponentialNativeF32` / `ExponentialNativeF32` intentionally use f32-native ziggurat candidates and therefore do not match the default f64-backed f32 output mapping. They are explicit opt-in throughput profiles with checked snapshot evidence. |
