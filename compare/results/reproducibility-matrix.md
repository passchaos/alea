...
| `Rng.unicodeScalarFrom`, `Rng.fillUnicodeScalarFrom`, `Rng.unicodeScalarBatchFrom`, `ascii.unicodeScalarFrom`, `unicodeUtf8Capacity`, `unicodeUtf8AllocFrom`, `unicodeUtf8IntoFrom` | Same Unicode scalars and UTF-8 bytes for fixed engine stream and requested scalar count; `fillUnicodeScalarFrom` and `unicodeScalarBatchFrom` preserve repeated scalar stream shape; `unicodeUtf8Capacity(len)` is stable as checked `len * 4`. |
...
| `src/ascii.zig`: `ascii helpers have stable snapshots`; `src/rng.zig`: `unicode scalar fills and batches preserve scalar stream shape` | ASCII charset/string and Unicode scalar UTF-8 generation for a fixed stream; charset sampling uses a u64 bounded draw for target-width-independent snapshots on 32-bit and 64-bit `usize` targets; `fillUnicodeScalarFrom` and `unicodeScalarBatchFrom` preserve repeated scalar stream shape |
