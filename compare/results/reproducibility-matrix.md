...
| `src/ascii.zig`: `ascii helpers have stable snapshots` | ASCII charset/string and Unicode scalar UTF-8 generation for a fixed stream; charset sampling uses a u64 bounded draw for target-width-independent snapshots on 32-bit and 64-bit `usize` targets |
