...
| Deterministic point-mass helpers | Valid collapsed ranges, single-positive weighted-index/static weighted-sampler choices, single-root weighted trees, and degenerate scalar/vector distribution parameter sets return or fill their single possible value without consuming randomness. |
...
| `src/rng.zig`: `degenerate range helpers do not consume random stream`, `single-positive weighted index does not consume random stream`; `src/distributions.zig`: `single-positive alias table does not consume random stream`, `single-root weighted trees do not consume random stream`; `src/seq.zig`: `single-positive weighted choice does not consume random stream`; `src/distributions.zig`: `degenerate normal and log-normal helpers do not consume random stream`, `degenerate uniform distribution helpers do not consume random stream`, `degenerate discrete distribution helpers do not consume random stream` | Point-mass scalar/vector range, weighted-index/static weighted-choice/weighted-tree, Normal/LogNormal, uniform, and discrete distribution helpers return/fill deterministic values without consuming randomness |
