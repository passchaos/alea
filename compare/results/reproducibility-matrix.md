...
| `src/rng.zig`, `src/seq.zig`, `src/distributions.zig`, `src/ascii.zig`: invalid checked/no-consume, zero-length, and allocation-failure tests | Invalid checked/error paths that can validate before drawing return before consuming randomness, including nested empty-enum `valueCheckedFrom` cases, empty/all-zero weighted iterator choices, length-mismatched `WeightedChoice.update` calls, invalid charset construction, zero-length ASCII/Unicode string allocation, invalid multivariate checked output lengths, initial sequence/string/multivariate allocation failure, and Unicode UTF-8 allocation length overflow. Single-pass streaming helpers may already have consumed randomness for earlier valid entries before a later invalid entry is discovered. |
