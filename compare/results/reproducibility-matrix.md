...
| `src/distributions.zig`: `native f32 standard samplers have stable snapshots` | Current `StandardNormalNativeF32` / `standardNormalNativeF32*` and `StandardExponentialNativeF32` / `standardExponentialNativeF32*` opt-in output mappings for native f32 ziggurat profiles |
...
| Native f32 standard distribution profiles | `StandardNormalNativeF32` and `StandardExponentialNativeF32` intentionally use f32-native ziggurat candidates and therefore do not match the default f64-backed f32 output mapping. They are explicit opt-in throughput profiles with checked snapshot evidence. |
