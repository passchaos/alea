...
| Table-quantile vector normal profiles | `VectorStandardNormalTableF32` / `VectorNormalTableF32` and `VectorStandardNormalTableF64` / `VectorNormalTableF64` intentionally use midpoint quantile-table lookup for vector lanes. They are vector-only, discrete/truncated throughput opt-ins with checked snapshot evidence and do not match default ziggurat or native-f32 normal output mappings. |
