...
| `src/distributions.zig`: `native f32 log-normal has stable snapshots` | Current `LogNormalNativeF32` / `logNormalNativeF32*` opt-in output mapping for native-f32-standard-normal plus exact f32 `@exp` |
...
| Native f32 LogNormal profile | `LogNormalNativeF32` uses `StandardNormalNativeF32` plus exact f32 `@exp`, so it intentionally differs from default `LogNormal(f32)` output mapping while preserving a named opt-in throughput profile. |
