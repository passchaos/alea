...
| Additional architecture/runtime runner | Refreshed `command -v` audit found `node`, `cargo`, and `rustc`, but still not `qemu-aarch64`, `qemu-riscv64`, `qemu-x86_64`, `wine`, `wine64`, `wasmtime`, or `wasmer`; `zig build runtimecheck` now automates this availability check and currently reports `runtimecheck ok: no additional runtime runner available`. Current executed targets remain native glibc Linux, x86_64-linux-musl via Zig's runnable target, and Node WASI. Other targets remain compile-only through `zig build crosscheck`. |
