...
| --- | --- | --- |
| Local Rust baseline exists for comparable core workloads | `compare/rand_bench/src/main.rs`, focused Rust rows in `performance-triage.md`, and local `~/Work/rand` / cached `rand_distr` scope in `linux-no-known-gaps-audit.md` | Covered for current Linux scope; new missing rows have been added as found, including skew-large Hypergeometric and weighted alias f64/u32. |
...
| Functionality and validation gates cover distribution changes | `zig build validate-all`, `zig build distcheck`, focused unit tests listed in `performance-triage.md` and `reproducibility-matrix.md` | Covered for implemented changes; recent commits ran relevant focused validation before commit. |
...
| Weighted dynamic/static sampling | Integer dynamic weights are covered by `WeightedIntTree`; generic `WeightedTree(f64)` now has matching local Rust f64 evidence and exceeds it in focused update+sample rows. Static `AliasTable(f64)` and `AliasTable(u32)` now have explicit local Rust `WeightedAliasIndex` evidence and exceed it in ScalarPrng direct/fill rows after the power-of-two one-word threshold fast path. |
