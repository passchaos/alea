...
| f32 vector normal opt-in | `VectorStandardNormalTableF32` / `VectorNormalTableF32` provide a vector-only f32 table-quantile profile with real-harness rows around 1.28-1.30B f32x8 lanes/s, far above current f32x8 ziggurat lane-fill. This narrows the normal side for users who accept the explicit discrete/truncated table-output contract, but it is not a default replacement and does not close f64/default dense-kernel requirements. |
