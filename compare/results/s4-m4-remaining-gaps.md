...
| Exact default LogNormal matches or exceeds local Rust | `lognormal-transform-notes.md`, `performance-triage.md`, focused LogNormal rows | Not complete; see blocking gap below. |
...
Conclusion: S4-M4 is not complete. The remaining work is not unverified generic
coverage; it is the two concrete hard blockers below.

## Current Blocking Gaps

| Area | Current evidence | Why not closed |
| --- | --- | --- |
| Exact default LogNormal transform/codegen | Focused exact-default f64 single-sample rows remain around 118M for facade/FastPrng/raw versus local Rust f64 around 145-146M. Exact f32 defaults improved in direct/fill workflows, but primary exact f32 single-sample evidence remains below local Rust f32 around 158-159M. Wider `stddev=1` exact-default rows remain around 65M f64 and 60-64M f32 single-sample versus Rust around 80M/79M. | Exact default `@exp` transform/codegen is still the bottleneck. `lognormal-codegen-audit.md` confirms local exact paths call compiler-rt exp/expf; `BufferedLogNormal`, `LogNormalDlsymExp`, `LogNormalLibmvec`, and f32/native/exp2 profiles cover opt-in throughput/accuracy niches but do not close the exact-default gap because they change refill contract, output mapping, accuracy contract, or platform availability. |
| Dense SIMD normal/exponential distribution kernels | Vector APIs and vectorbench coverage are complete, but production vector normal/exponential kernels still use scalar ziggurat lane-fill. f32x8/f64x4 repair, same-candidate repair, all-accepted repair, block-fallback, flat-slice routing, FastPrng repair, Alea4x64 lane-local repair, Marsaglia polar normal, libmvec vector-log exponential, and f64 direct-next/noinline codegen probes have all trailed or failed to transfer into current direct rows in the real vector-slice harness. | No genuinely dense SIMD candidate has beaten scalar lane-fill while preserving or explicitly versioning rejected-lane stream shape; `simd-distribution-kernel-notes.md` now lists the minimum real-harness vectorbench rows required for any future candidate. |
