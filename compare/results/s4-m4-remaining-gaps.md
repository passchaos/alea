...
| Weighted dynamic sampling | Integer weights are covered by `WeightedIntTree`; generic `WeightedTree(f64)` now has matching local Rust f64 evidence and exceeds it in focused update+sample rows. |
...
3. Produce new local Rust evidence showing an untracked comparable core workload
   where Alea trails, then add a targeted fix or explicit exclusion.
