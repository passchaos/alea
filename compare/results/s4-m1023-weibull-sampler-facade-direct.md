# S4-M1023 Weibull Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `Weibull` facade sample/fill helpers still routed through
`sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw standard
exponentials for `shape == 1` or strict-open uniforms for the generic transform;
the facade paths can now do the same through facade `Rng` while preserving
collapsed `scale == 0` and `shape == inf` point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while Alea's Weibull
...
- `src/distributions.zig` updates `Weibull(T).sample` and `Weibull(T).fill` to
  draw standard exponentials or strict-open uniforms directly through facade
  `Rng`, with no-consume point-mass fast paths.
- `src/distributions.zig` updates `VectorWeibull(VectorType).sample` and
  `VectorWeibull(VectorType).fill` to draw vector standard exponentials or
  strict-open uniforms directly through facade `Rng` and call the vector generic
  transform helper, with the same point-mass no-consume fast paths.
- Direct-source `sampleFrom` / `fillFrom` remain unchanged for explicit
  direct-source workflows and top-level helpers.

## Validation

Focused Weibull tests:
...
$ zig test src/distributions.zig --test-filter "degenerate weibull helpers do not consume random stream"
1/2 distributions.test.degenerate weibull helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1023 is closed for the current bar: reusable scalar/vector Weibull facade
