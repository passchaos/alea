# S4-M611 Root Checked Inclusive Integer Batch Prevalidation

## Gap

Root `rangeAtMostBatchChecked` allocated its output buffer before validating
invalid inclusive integer ranges. Invalid checked parameters should be reported
before random-output allocation and before root secure-engine construction for
non-zero requests.

This milestone aligns checked inclusive integer range batch behavior with the
root fill helper and with earlier checked batch prevalidation work.
...
- Zero-count batches still return empty allocations before validating parameters
  or drawing entropy.
- Invalid inclusive ranges return `error.EmptyRange` before allocation or
  entropy for non-zero requests.
- Collapsed deterministic valid paths, where `min == max`, still allocate and
  fill deterministic values before entropy is requested.
...
S4-M611 is closed for the current bar: root checked inclusive integer range batch
