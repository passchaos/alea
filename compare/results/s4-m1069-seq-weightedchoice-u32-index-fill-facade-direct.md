# S4-M1069 Seq WeightedChoice U32 Index Fill Facade Direct Path

## Gap

Sequence-layer reusable `WeightedChoice(T).fillIndicesU32` still called
`self.table.fillU32CheckedFrom(rng, dest)` after population validation. Reusable
`AliasTable.fillU32Checked` now has a direct facade implementation, so the
...
behavior. Alea exposes compact caller-owned u32 weighted index fills on reusable
sequence choices; this change tightens that facade helper so it drives the
supplied facade RNG directly through the alias-table checked u32 fill facade.

## Implementation

- `src/seq.zig` updates `WeightedChoice(T).fillIndicesU32` to call
  `self.table.fillU32Checked(rng, dest)` after the existing population-size
  validation.
- `WeightedChoice(T).fillIndicesU32From` remains unchanged for explicit
...
S4-M1069 is closed for the current bar: sequence-layer reusable
`WeightedChoice(T).fillIndicesU32` now avoids the direct-source wrapper alias
while preserving stream shape, population validation, empty-output behavior, and
single-positive behavior. This is reliability/ergonomics work only; it does not
