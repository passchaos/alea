# S4-M1026 SkewNormal Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `SkewNormal` facade sample/fill helpers still routed
through `sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw
standard-normal components and apply the symmetric, min/max, or generic skew
transform directly; the facade paths can now do the same through facade `Rng`
while preserving collapsed `scale == 0` point-mass behavior.

## Local `rand` / `rand_distr` Baseline

The local `rand` checkout remains the primary baseline, while Alea's SkewNormal
...
- `src/distributions.zig` updates `SkewNormal(T).sample` and
  `SkewNormal(T).fill` to draw standard-normal components directly through
  facade `Rng` and apply the symmetric, min/max, or generic skew transform, with
  a `scale == 0` no-consume fast path.
- `src/distributions.zig` updates `VectorSkewNormal(VectorType).sample` and
  `VectorSkewNormal(VectorType).fill` to draw vector standard-normal components
  directly through facade `Rng` and call `skewNormalFromStandardNormalVectors`,
  with the same point-mass no-consume fast path.
- Direct-source `sampleFrom` / `fillFrom` remain unchanged for explicit
  direct-source workflows and top-level helpers.

## Validation

Focused SkewNormal tests:
...
$ zig test src/distributions.zig --test-filter "degenerate skew-normal helpers do not consume random stream"
1/2 distributions.test.degenerate skew-normal helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1026 is closed for the current bar: reusable scalar/vector SkewNormal facade
