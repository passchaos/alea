# S4-M606 Root Item-Accessor Weighted Index Batch Prevalidation

## Gap

Root `weightedIndexBatchBy` and `weightedIndexBatchByChecked` allocated output
...
This milestone aligns the allocation-returning item-accessor weighted `usize`
...
- Empty/all-zero item weights still return nullable `null` index batches for
  unchecked requests and `error.EmptyInput` for checked requests before entropy.
- Single-positive item weights still allocate and fill repeated indices before
...
S4-M606 is closed for the current bar: root item-accessor weighted `usize` index
