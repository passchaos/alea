# S4-M1021 Maxwell Sampler Facade Direct Paths

## Gap

Reusable scalar/vector `Maxwell` facade sample/fill helpers still routed through
`sampleFrom` / `fillFrom` wrappers. The direct-source paths already draw three
normal components and combine them with the Maxwell radius transform; the facade
...
`rand_distr` surface evidence records Maxwell as product-breadth beyond the
...
- `src/distributions.zig` updates `Maxwell(T).sample` and `Maxwell(T).fill` to
  draw normal components directly through facade `Rng` and apply the Maxwell
  radius transform, with a `scale == 0` no-consume fast path.
- `src/distributions.zig` updates `VectorMaxwell(VectorType).sample` and
  `VectorMaxwell(VectorType).fill` to draw vector normal components directly
  through facade `Rng` and call `maxwellFromNormalVectors`, with the same
...
$ zig test src/distributions.zig --test-filter "degenerate maxwell helpers do not consume random stream"
1/2 distributions.test.degenerate maxwell helpers do not consume random stream...OK
2/2 root.test_0...OK
All 2 tests passed.
...
S4-M1021 is closed for the current bar: reusable scalar/vector Maxwell facade
