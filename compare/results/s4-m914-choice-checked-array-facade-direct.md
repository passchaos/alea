# S4-M914 Choice Checked Array Facade Direct Paths

## Gap

Reusable `Choice` checked fixed-array facade helpers still routed through
...
pointer, value, `usize` index, and compact `u32` index outputs. The checked facade
variants should fill the fixed array directly through the facade `Rng`.

## Implementation

- `src/seq.zig` updates `Choice.ptrArrayChecked` to allocate the fixed pointer
  array and call checked facade pointer fill directly.
- `src/seq.zig` updates `Choice.valueArrayChecked` to allocate the fixed value
  array and call checked facade value fill directly.
- `src/seq.zig` updates `Choice.indexArrayChecked` and `Choice.indexArrayU32Checked`
  to allocate fixed index arrays and call checked facade index fills directly.
...
S4-M914 is closed for the current bar: reusable `Choice` checked fixed-array
facade helpers now avoid direct-source checked array wrapper aliases while
