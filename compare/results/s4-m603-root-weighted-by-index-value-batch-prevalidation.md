# S4-M603 Root By-Index Weighted Value Batch Prevalidation

## Gap

Root `chooseWeightedBatchByIndex` and `chooseWeightedBatchByIndexChecked`
allocated output buffers before checking invalid weights and deterministic
empty/single by-index weighted-index states. Those failures should be reported
before random-output allocation and before root secure-engine construction.

This milestone aligns the allocation-returning by-index weighted value batch
...
- Empty/all-zero index weights still return nullable `null` batches for
  unchecked requests and `error.EmptyInput` for checked requests before entropy.
- Single-positive index weights still allocate and fill repeated values before
...
S4-M603 is closed for the current bar: root by-index weighted value batch helpers
now prevalidate deterministic and invalid paths before random-output allocation
