...
fn main() {
    ...
    bench_distr_hypergeometric_skew_large("rand_distr hypergeometric skew-large", bytes / 256);
...
fn bench_weighted_alias_u32(name: &str, count: usize) {
    if !include_bench(name) {
        return;
    }
    let weights = vec![1u32, 2, 3, 0, 5, 8, 13, 21];
    let mut best_million_per_s = 0.0;
    let mut best_checksum = 0usize;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0xa11a);
...
fn bench_weighted_alias_f64(name: &str, count: usize) {
    if !include_bench(name) {
        return;
    }
    let weights = vec![1.0f64, 2.0, 3.0, 0.0, 5.0, 8.0, 13.0, 21.0];
    let mut best_million_per_s = 0.0;
    let mut best_checksum = 0usize;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0xa11b);
        let dist = rand_distr::weighted::WeightedAliasIndex::new(weights.clone()).unwrap();
        let start = Instant::now();
        let mut checksum: usize = 0;
...
fn bench_weighted_tree_f64(name: &str, count: usize) {
    if !include_bench(name) {
        return;
    }
    let initial = [1.0f64, 2.0, 3.0, 0.0, 5.0, 8.0, 13.0, 21.0];
    let mut best_million_per_s = 0.0;
    let mut best_checksum = 0usize;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0x77ed);
        let mut dist = rand_distr::weighted::WeightedTreeIndex::new(initial).unwrap();
        let start = Instant::now();
        let mut checksum: usize = 0;
        ...
        for i in 0..count {
            let index = i & 7;
            dist.update(index, ((i % 17) + 1) as f64).unwrap();
            checksum = checksum.wrapping_add(dist.sample(&mut rng));
        }
...
fn bench_distr_hypergeometric(name: &str, count: usize) {
    ...
    let mut best_checksum = 0u64;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0x4965);
        let start = Instant::now();
        let mut checksum = 0u64;
...
fn bench_distr_hypergeometric_large(name: &str, count: usize) {
    ...
    let mut best_checksum = 0u64;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0x4966);
        let start = Instant::now();
        let mut checksum = 0u64;
...
fn bench_distr_hypergeometric_skew_large(name: &str, count: usize) {
    if !include_bench(name) {
        return;
    }
    let dist = rand_distr::Hypergeometric::new(10_000, 1_000, 2_000).unwrap();
    let mut best_million_per_s = 0.0;
    let mut best_checksum = 0u64;
    for _ in 0..TRIALS {
        let mut rng = SmallRng::seed_from_u64(0x4967);
        let start = Instant::now();
        let mut checksum = 0u64;
